...
def parse_lidar_camera(param_str: str, batch_dims: tuple, width: int, height: int, device: torch.device):
    ...
    params.angles_to_columns_map = compute_lidar_angles_to_columns_map(LidarCameraParameters(**vars(params)))
...
def image_points(batch_dims, image_dims, ref_camera):
    ...
    if isinstance(ref_camera, _LidarCameraModel):
        ...
        if ref_camera.spinning_direction == SpinningDirection.CLOCKWISE:
            ...
            azimuth = ref_camera.fov_horiz_rad.start - ref_camera.fov_horiz_rad.span*torch.rand(*shape, device=device)
        else:
            # Counter-clockwise: azimuth increases
            azimuth = ref_camera.fov_horiz_rad.start + ref_camera.fov_ohriz_rad.span*torch.rand(*shape, device=device)

        # Elevation decreases, always clockwise
        elevation = ref_camera.fov_vert_rad.start - ref_camera.fov_vert_rad.span*torch.rand(*shape, device=device)
