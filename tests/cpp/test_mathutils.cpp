...
struct SmallCountCase
{
    ...
    {
        return os << "{count=" << c.count << ", expected_bits=" << c.expected_bits << "}";
    }
};
...
TEST_P(BitsForCountSmall, MatchesExpectedWidth)
{
    ...
    EXPECT_EQ(gsplat::bits_for_count(param.count), param.expected_bits) << "count = " << param.count;
}
...
TEST(BitsForCount, PowersOfTwoNeedKBits)
{
    ...
    for(uint32_t k = 1; k <= 31; ++k)
    {
        ...
        EXPECT_EQ(gsplat::bits_for_count(count + 1), k + 1) << "count = 2^" << k << " + 1";
    }
}
