...
def test_isect_lidar(lidar_model, batch_dims: Tuple[int, ...]):
    ...
    test_data = {
        "means2d": torch.randn(C, N, 2, device=device) * torch.tensor([nrows, ncols], device=device),
        "radii": torch.randint(0, max(1,min(ncols, nrows)//2), (C, N, 2), device=device, dtype=torch.int32),
        "depths": torch.rand(C, N, device=device),
    }
    test_data = expand(test_data, batch_dims)
