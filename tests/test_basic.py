...
def test_isect_lidar(lidar_model, batch_dims: Tuple[int, ...]):
    from gsplat.cuda._torch_impl_lidar import _isect_tiles_lidar, ANGLE_TO_PIXEL_SCALING_FACTOR
...
def lidar_param(hfov_span_deg, ray_location, n_dense_tiles_azimuth):
    ...
    hfov_span_rad = math.ceil((hfov_span_deg/180*math.pi)*ANGLE_TO_PIXEL_SCALING_FACTOR)/ANGLE_TO_PIXEL_SCALING_FACTOR
    vfov_span_rad = math.ceil((vfov_span_deg/180*math.pi)*ANGLE_TO_PIXEL_SCALING_FACTOR)/ANGLE_TO_PIXEL_SCALING_FACTOR
    ...
    # clockwise
    row_elevations_rad = torch.tensor([vfov_span_rad/2, 0.0, -vfov_span_rad/2], dtype=torch.float32, device=device)
    ...
    dense_mask = torch.zeros(
        (cdf_resolution_azimuth, cdf_resolution_elevation), dtype=torch.int32, device=device
    )
    ...
    cdf = torch.zeros(
        (dense_mask.shape[0]+1, dense_mask.shape[1]+1), dtype=torch.int32, device=dense_mask.device
    )
...
def gaussian_param(lidar_param, gauss_start_pos, gauss_end_pos):
    ...
    if "exact" in gauss_start_pos or "exact" in gauss_end_pos:
        ...
        def calc_mean_az_pix(exact_edge: float, radius: float) -> float:
            ...
            gauss_mean_fp32 = struct.unpack("!f", struct.pack("!f", float(gauss_mean)))[0]
    ...
    assert not behind_sensor or (mean_az_pix < hfov_start_pix or mean_az_pix >= hfov_end_pix), \
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (120, "min_inside_exact", "max_outside",       "none", 21, 21), # beg=0, end=span -> full_cover -> 21
        (120, "min_outside",      "max_outside_exact", "none", 21, 21), # beg<0, end=span -> full_cover -> 21
        (360, "min_inside_exact", "max_outside",       "none", 21, 21), # beg=0, end=span -> full_cover -> 21
        (360, "min_outside",      "max_outside_exact", "none", 21, 21), # beg<0, end=span -> full_cover -> 21
        ...
        (120, "min_inside_exact", "max_outside",       "min",  21, 21), # full_cover + ray@min -> still 21
        (120, "min_inside_exact", "max_outside",       "both", 21, 21), # full_cover + both rays -> still 21
        (360, "min_inside_exact", "max_outside",       "max",  21, 21), # full_cover + ray@max -> still 21
        ...
        (360, "min_outside",      "max_outside_exact", "max",  21, 21), # A=[0,fc-~0.02), B=[fc-~0.02,fc); tile 20 only counted once -> 21
        ...
        (120, "min_inside_exact", "max_inside",        "none", -1,  0), # end=span-1 < span -> not full_cover, no rays -> 0
        (120, "min_outside",      "max_inside_exact",  "none", -1,  0), # end=span-1 < span -> not full_cover, no rays -> 0
        (360, "min_inside_exact", "max_inside",        "none", -1,  0), # end=span-1 < span -> not full_cover, no rays -> 0
        (360, "min_outside",      "max_inside_exact",  "none", -1,  0), # end=span-1 < span -> not full_cover, no rays -> 0

        # Normal inside gaussians.
        (120, "low_inside",       "high_inside",       "middle", 21, 11), # A covers [0.25,0.75)*span -> tiles [5,16); ray@cell10 hit -> 11
        (120, "high_inside",      "low_inside",        "middle", 21, 0),  # behind sensor: tips at min/max edges, ray@middle in gap -> 0

        # 360deg normal inside: same geometry but periodic.
        (360, "low_inside",       "high_inside",       "middle", 21, 11), # same as 120deg, normal middle coverage -> 11
        ...
        (360, "high_inside",      "low_inside",        "middle", 21, 0),  # ray@middle in gap between A and B -> 0
        (360, "high_inside",      "low_inside",        "both",   21, 12), # A=6 tiles (hit@max), B=6 tiles (hit@min) -> 12
        ...
        (120, "max_inside_exact",  "min_inside",       "min",  21, 1), # ray@min in B(tile 0) -> 1
        (120, "max_inside_exact",  "min_inside",       "max",  21, 1), # ray@max in A(tile 20) -> 1
        (120, "max_inside_exact",  "min_inside",       "both", 21, 2), # ray@min in B + ray@max in A -> 2
        # start@max_inside, end_exact@min_inside:
        #   end exact@0 -> B=[0,0)=empty; A=[span-1,span)=tile 20
        (120, "max_inside",  "min_inside_exact",       "min",  21, 0), # B empty; ray@min not in A -> 0
        (120, "max_inside",  "min_inside_exact",       "max",  21, 1), # B empty; ray@max in A(tile 20) -> 1
        (120, "max_inside",  "min_inside_exact",       "both", 21, 1), # B empty; only ray@max in A -> 1

        # start_exact@max_outside: start at span boundary -> A=[span,span)=empty.
        # Only B (overflow tip at min) can contribute.
        (120, "max_outside_exact", "min_inside",       "min",  21, 1), # A empty, B=tile 0; ray@min in B -> 1
        (120, "max_outside_exact", "min_inside",       "max",  21, 0), # A empty, B=tile 0; ray@max not in B -> 0
        (120, "max_outside_exact", "min_inside",       "both", 21, 1), # A empty, B=tile 0; only ray@min in B -> 1
        ...
        (120, "max_inside_exact", "min_outside",       "min",  21, 0), # ray@min not in A(tile 20) -> 0
        (120, "max_inside_exact", "min_outside",       "max",  21, 1), # ray@max in A(tile 20) -> 1
        (120, "max_inside_exact", "min_outside",       "both", 21, 1), # B empty; only ray@max in A -> 1
        (120, "max_inside",       "min_outside_exact", "min",  21, 0), # A=tile 20; ray@min not in A -> 0
        (120, "max_inside",       "min_outside_exact", "max",  21, 1), # A=tile 20; ray@max in A -> 1
        (120, "max_inside",       "min_outside_exact", "both", 21, 1), # A=tile 20; only ray@max in A -> 1
        ...
        (120, "min_outside_exact", "min_inside",       "min", 21, 1), # underflow: A=[0,1)=cell 0, B clamped empty; ray@min in A -> 1
        (120, "min_outside",       "min_outside_exact","min", 21, 0), # entirely outside FOV (near fc), clamped empty -> 0
        (120, "min_inside_exact",  "min_inside",       "min", 21, 1), # A=[0,2)=cell 0; ray@min in A -> 1
        (120, "min_outside",       "min_inside_exact", "min", 21, 0), # mean outside, end exact@0 -> clamped empty -> 0
        # 1.1.2 ray on max boundary
        (120, "min_outside_exact", "min_inside",       "max", 21, 0), # A=[0,1)=cell 0; ray@max(cell 20) not in A -> 0
        (120, "min_outside",       "min_outside_exact","max", 21, 0), # entirely outside -> 0
        (120, "min_inside_exact",  "min_inside",       "max", 21, 0), # A=[0,2)=cell 0; ray@max not in A -> 0
        ...
        (120, "min_outside_exact", "min_inside",       "both", 21, 1), # A=cell 0; only ray@min hit -> 1
        (120, "min_outside",       "min_outside_exact","both", 21, 0), # entirely outside -> 0
        (120, "min_inside_exact",  "min_inside",       "both", 21, 1), # A=cell 0; only ray@min hit -> 1
        ...
        (120, "max_outside_exact", "max_outside",      "min", 21, 0), # both at span, clamped to [span,span+2) -> empty -> 0
        (120, "max_inside",        "max_outside_exact","min", 21, 0), # A=[span-2,span)=cell 20; ray@min not in A -> 0
        (120, "max_inside_exact",  "max_outside",      "min", 21, 0), # A=[span-1,span)=cell 20; ray@min not in A -> 0
        (120, "max_inside",        "max_inside_exact", "min", 21, 0), # A near max=cell 20; ray@min not in A -> 0
        # 1.2.2 ray on max boundary
        (120, "max_outside_exact", "max_outside",      "max", 21, 0), # entirely outside (clamped empty) -> 0
        (120, "max_inside",        "max_outside_exact","max", 21, 1), # A=[span-2,span)=cell 20; ray@max in A -> 1
        (120, "max_inside_exact",  "max_outside",      "max", 21, 1), # A=[span-1,span)=cell 20; ray@max in A -> 1
        (120, "max_inside",        "max_inside_exact", "max", 21, 1), # A near max=cell 20; ray@max in A -> 1
        # 1.2.3 ray on both boundaries (non-periodic B is empty, only max side matters)
        (120, "max_outside_exact", "max_outside",      "both", 21, 0), # entirely outside -> 0
        (120, "max_inside",        "max_outside_exact","both", 21, 1), # A=cell 20; only ray@max hit -> 1
        (120, "max_inside_exact",  "max_outside",      "both", 21, 1), # A=cell 20; only ray@max hit -> 1
        (120, "max_inside",        "max_inside_exact", "both", 21, 1), # A=cell 20; only ray@max hit -> 1

        # 2 periodic azimuth
        # 2.1. Gaussian edge on min hfov boundary
        # 2.1.1 ray on min hfov boundary
        (360, "min_outside_exact", "min_inside",       "min", 21, 1), # underflow: A=[0,1)=cell 0, B=[fc-1,fc)=cell 20; ray@min in A -> 1
        (360, "min_outside",       "min_outside_exact","min", 21, 0), # A=[fc-2,fc)=cell 20; ray@min(cell 0) not in A -> 0
        (360, "min_inside_exact",  "min_inside",       "min", 21, 1), # A=[0,2)=cell 0; ray@min in A -> 1
        (360, "min_outside",       "min_inside_exact", "min", 21, 0), # A near fc=cell 20; ray@min not in A -> 0
        # 2.1.2 ray on max boundary
        (360, "min_outside_exact", "min_inside",       "max", 21, 1), # A=cell 0, B=cell 20; ray@max in B -> 1
        (360, "min_outside",       "min_outside_exact","max", 21, 1), # A=[fc-2,fc)=cell 20; ray@max in A -> 1
        (360, "min_inside_exact",  "min_inside",       "max", 21, 0), # A=[0,2)=cell 0; ray@max not in A -> 0
        (360, "min_outside",       "min_inside_exact", "max", 21, 1), # A near fc=cell 20; ray@max in A -> 1
        # 2.1.3 ray on both boundaries
        (360, "min_outside_exact", "min_inside",       "both", 21, 2), # A=cell 0 (hit@min), B=cell 20 (hit@max) -> 2
        (360, "min_outside",       "min_outside_exact","both", 21, 1), # A=cell 20; ray@max hit, ray@min miss -> 1
        (360, "min_inside_exact",  "min_inside",       "both", 21, 1), # A=cell 0; ray@min hit, ray@max miss -> 1
        (360, "min_outside",       "min_inside_exact", "both", 21, 1), # A=cell 20; ray@max hit -> 1

        # 2.2. Gaussian edge on max hfov boundary
        (360, "max_outside_exact", "max_outside",      "min", 21, 1), # wraps to A=[0,2)=cell 0; ray@min in A -> 1
        (360, "max_outside_exact", "min_inside",       "min", 21, 1), # same angles for 360: wraps to cell 0; ray@min in A -> 1
        (360, "max_inside",        "max_outside_exact","min", 21, 0), # A=[fc-2,fc)=cell 20; ray@min not in A -> 0
        (360, "max_inside",        "min_inside_exact", "min", 21, 0), # same geometry as above -> 0
        (360, "max_inside_exact",  "max_outside",      "min", 21, 1), # underflow: A=[0,1), B=[fc-1,fc); ray@min in A -> 1
        (360, "max_inside_exact",  "min_inside",       "min", 21, 1), # same geometry as above -> 1
        (360, "max_inside",        "max_inside_exact", "min", 21, 0), # A=[fc-3,fc-1)=cells 19-20; ray@min not in A -> 0
        (360, "max_inside",        "min_outside_exact","min", 21, 0), # same geometry as above -> 0
        # 2.2.2 ray on max boundary
        (360, "max_outside_exact", "max_outside",      "max", 21, 0), # wraps to A=[0,2)=cell 0; ray@max not in A -> 0
        (360, "max_inside",        "max_outside_exact","max", 21, 1), # A=[fc-2,fc)=cell 20; ray@max in A -> 1
        (360, "max_inside_exact",  "max_outside",      "max", 21, 1), # A=cell 0, B=cell 20; ray@max in B -> 1
        (360, "max_inside",        "max_inside_exact", "max", 21, 1), # A=[fc-3,fc-1)=cells 19-20; ray@max(cell 20) in A -> 1
        # 2.2.3 ray on both boundaries
        (360, "max_inside_exact",  "max_outside",      "both", 21, 2), # A=cell 0 (hit@min), B=cell 20 (hit@max) -> 2
        (360, "max_outside_exact", "min_outside",      "both", 21, 0), # extent wraps >full_circle -> negative radius -> culled -> 0
        (360, "max_outside",       "min_outside_exact","both", 21, 1), # radius rounds to 1, A covers cell 20; ray@max hit -> 1
        ...
        (120, "min_outside_exact", "min_inside",       "min", 7, 1),  # coarser dense: A dense=[0,1), ray@min hit -> 1
        (120, "max_inside",        "max_outside_exact","max", 7, 1),  # coarser dense: A dense=[6,7), ray@max hit -> 1
        (120, "low_inside",        "high_inside",      "middle", 7, 11), # coarser dense doesn't change tile count -> 11
        (360, "min_outside_exact", "min_inside",       "min", 7, 1),  # periodic with coarser dense -> 1
        ...
        (120, "far_min_outside", "far_max_outside", "both", 21, 21), # extent=1.5*span; full_cover -> 21
        (120, "far_min_outside", "far_max_outside", "none", 21, 21), # full_cover triggers conservative shortcut -> 21
        (200, "far_min_outside", "far_max_outside", "both", 21, 21), # full_cover -> 21
        (200, "far_min_outside", "far_max_outside", "none", 21, 21), # full_cover triggers conservative shortcut -> 21
        (360, "far_min_outside", "far_max_outside", "middle", 21, 21), # periodic, extent=1.5*span; region B covers full FOV -> 21
        (360, "far_min_outside", "far_max_outside", "none",   21, 21), # periodic, region B triggers conservative full-cover shortcut -> 21
    ]
)
def test_isect_lidar_corner_cases(
    ...
    def abs_to_rel_az(abs_az_pix: float) -> float:
        return relative_angle(hfov_start_pix, abs_az_pix, lidar.spinning_direction, scale=ANGLE_TO_PIXEL_SCALING_FACTOR)
...
def _expected_hit_distance_canonical_ray_distance(
    ...
    gro = torch.matmul(iscl_rot, (ray_o - xyz).unsqueeze(0).unsqueeze(-1)).squeeze(-1).squeeze(0)
...
def _pixel_ray_dir_pinhole(px: float, py: float, fx: float, fy: float, cx: float, cy: float, device: torch.device) -> torch.Tensor:
...
def test_rasterize_to_pixels_hit_distance_principal_axis(
    ...
    ray_d = _pixel_ray_dir_pinhole(pixel_center_x, pixel_center_y, fx, fy, cx, cy, device)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(*params, marks=[
            ...
            pytest.mark.skipif(params[4] and not has_camera_wrappers(), reason="Camera wrapper support isn't built in")
        ])
        for params in
            chain(
                ...
                product(
                    [3],                          # channels
                    [(), (2,), (1, 2)],            # batch_dims
                    [RollingShutterType.GLOBAL, RollingShutterType.ROLLING_TOP_TO_BOTTOM],  # rs_type
...
def test_rasterize_to_pixels_eval3d(
    ...
    if use_rays:
        ...
        grid = grid.expand(*batch_shape, *grid.shape).reshape(*batch_shape, width*height, 2)
        ...
        rori, rdir, rvalid = camera.image_point_to_world_ray_shutter_pose(grid, pose_start, pose_end)
    ...
    render_colors, render_alphas, render_last_ids, render_sample_counts, render_normals = rasterize_to_pixels_eval3d_extra(
        means,
        quats,
        scales,
        colors,
    ...
    _render_colors, _render_alphas, _render_last_ids, _render_sample_counts, *_render_normals = _rasterize_to_pixels_eval3d(
        means,
        quats,
        scales,
        colors,
    ...
    if return_normals:
        assert render_normals is not None, "CUDA render_normals should not be None when return_normals=True"
        assert _render_normals is not None, "PyTorch render_normals should not be None when return_normals=True"
        ...
        assert render_normals is None, "CUDA render_normals should be None when return_normals=False"
        assert _render_normals is None, "PyTorch render_normals should be None when return_normals=False"
    ...
    loss_cuda = randperm(render_colors * v_render_colors + render_alphas * v_render_alphas + (render_normals * v_render_normals if return_normals else 0)).sum()
    loss_ref = randperm(_render_colors * v_render_colors + _render_alphas * v_render_alphas + (_render_normals * v_render_normals if return_normals else 0)).sum()

    if use_rays:
        (v_means, v_quats, v_scales, v_colors, v_opacities, v_backgrounds, v_rays) = torch.autograd.grad(
            loss_cuda,
            (means, quats, scales, colors, opacities_broadcast, backgrounds, rays),
            retain_graph=True,
        )

        (_v_means, _v_quats, _v_scales, _v_colors, _v_opacities, _v_backgrounds, _v_rays) = torch.autograd.grad(
            loss_ref,
            (means, quats, scales, colors, opacities_broadcast, backgrounds, rays),
            retain_graph=True,
        )
...
def _render_alpha(data, quats, scales, means2d, radii, depths):
    ...
    from gsplat.cuda._wrapper import (
        isect_offset_encode, isect_tiles, rasterize_to_pixels_eval3d_extra,
    )
    ...
    _, ra, _, _, _ = rasterize_to_pixels_eval3d_extra(
        data["means"], quats, scales, colors, opac_bc,
        data["viewmats"], data["Ks"], W, H, tile_size, ioff, fids,
    )
    return ra
...
def nan_test_data():
    ...
    Ks = torch.tensor([
        ...
        [0.0, 200.0, 24.0],
        [0.0, 0.0, 1.0],
    ], device=device).unsqueeze(0).expand(C, 3, 3).contiguous()
...
def test_projection_ut_zero_quaternion(nan_test_data):
    ...
    radii_ref, _, _, _, _ = fully_fused_projection_with_ut(
        data["means"], data["quats"], data["scales"], data["opacities"],
        data["viewmats"], data["Ks"], W, H,
    )
    assert (radii_ref[..., 0, :] > 0).all(), "Gaussian 0 must be visible with valid quat"
    ...
    proj_params = dict(
        means=data["means"], quats=quats, scales=data["scales"],
        opacities=data["opacities"], viewmats=data["viewmats"],
        Ks=data["Ks"], width=W, height=H,
    )

    # CUDA implementation
    radii_gpu, means2d_gpu, depths_gpu, conics_gpu, _ = fully_fused_projection_with_ut(**proj_params)
    # Python reference implementation
    radii_ref, means2d_ref, depths_ref, conics_ref, _ = _fully_fused_projection_with_ut(**proj_params)

    # Both must cull the zero-quat Gaussian (both x and y radii)
    assert (radii_gpu[..., 0, :] == 0).all(), \
        f"CUDA: zero-quat Gaussian should have radii=0, got {radii_gpu[..., 0, :].tolist()}"
    assert (radii_ref[..., 0, :] == 0).all(), \
        f"Ref: zero-quat Gaussian should have radii=0, got {radii_ref[..., 0, :].tolist()}"
    ...
    if sel.any():
        torch.testing.assert_close(means2d_gpu[sel], means2d_ref[sel], rtol=0.5, atol=0.05)
        torch.testing.assert_close(depths_gpu[sel], depths_ref[sel], rtol=1e-6, atol=2e-6)
        torch.testing.assert_close(conics_gpu[sel], conics_ref[sel], rtol=1e-2, atol=1e-2)
...
def test_projection_ut_zero_scale_single_axis(nan_test_data):
    ...
    assert (radii_ref[..., 0, :] > 0).all(), "Gaussian 0 must be visible with valid scales"
    ...
    proj_params = dict(
        means=data["means"], quats=data["quats"], scales=scales,
        opacities=data["opacities"], viewmats=data["viewmats"],
        Ks=data["Ks"], width=W, height=H,
    )
    ...
    assert (radii_gpu[..., 0, :] == 0).all(), \
        f"CUDA: single-axis zero-scale should have radii=0, got {radii_gpu[..., 0, :].tolist()}"
    assert (radii_ref[..., 0, :] == 0).all(), \
        f"Ref: single-axis zero-scale should have radii=0, got {radii_ref[..., 0, :].tolist()}"
...
def test_rasterize_eval3d_degenerate_gaussians_culled(nan_test_data):
    ...
    radii, means2d, depths, _, _ = fully_fused_projection_with_ut(
        data["means"], quats_bad, scales_bad, data["opacities"],
        data["viewmats"], data["Ks"], W, H,
    )
    ...
    rc_gpu, ra_gpu, _, _, _ = rasterize_to_pixels_eval3d_extra(
        data["means"], quats_safe, scales_safe, colors, opac_bc,
        data["viewmats"], data["Ks"], W, H, tile_size, ioff, fids,
    )
    ...
    ref_outputs = _rasterize_to_pixels_eval3d(
        data["means"], quats_safe, scales_safe, colors, opac_bc,
        data["viewmats"], data["Ks"], W, H,
        tile_size=tile_size, isect_offsets=ioff, flatten_ids=fids,
    )
...
def test_projection_ut_python_ref_no_nan(nan_test_data):
    ...
    proj_params = dict(
        means=data["means"], quats=quats, scales=scales,
        opacities=data["opacities"], viewmats=data["viewmats"],
        Ks=data["Ks"], width=data["width"], height=data["height"],
        ut_params=ut_params,
    )
...
def test_backward_high_opacity_no_nan():
    ...
    # CUDA forward
    render_colors, render_alphas, _, _, _ = rasterize_to_pixels_eval3d_extra(
        means, quats, scales, colors, opac_bc,
        viewmats, Ks, W, H, tile_size, ioff, fids,
    )
    ...
    for name, param in [("means", means), ("quats", quats), ("scales", scales), ("colors", colors)]:
        assert param.grad is not None, f"No gradient for {name}"
        assert torch.isfinite(param.grad).all(), \
            f"NaN/Inf in {name}.grad (max={param.grad.abs().max().item():.2e})"
