...
pure-torch reference ``gsplat.cuda._torch_impl._isect_tiles_sparse``. The
reference has been cross-checked against fvdb's
``intersect_gaussian_tiles_sparse`` for equivalence.
"""
...
pytestmark = pytest.mark.skipif(
    not cuda_is_available(), reason="isect_tiles_sparse is CUDA-only"
)
