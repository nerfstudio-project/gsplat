...
class MCMCStrategy(Strategy):
    ...
    def step_post_backward(self, step: int, info: Dict[str, Any], lr: float):
        ...
        if (
            ...
            # add new GSs
            n_new_gs = self._add_new_gs(self.cap_max)
            if self.verbose:
                print(
    ...
    def _relocate_gs(self, min_opacity: float = 0.005) -> int:
        dead_mask = torch.sigmoid(self.params["opacities"]) <= min_opacity
        ...
        probs = torch.sigmoid(self.params["opacities"])[alive_indices]
        probs = probs / (probs.sum() + eps)
        sampled_idxs = torch.multinomial(probs, n_gs, replacement=True)
        sampled_idxs = alive_indices[sampled_idxs]
        new_opacities, new_scales = compute_relocation(
            opacities=torch.sigmoid(self.params["opacities"])[sampled_idxs],
            scales=torch.exp(self.params["scales"])[sampled_idxs],
    ...
    def _add_new_gs(self, cap_max: int, min_opacity: float = 0.005) -> int:
        current_n_points = len(self.params["means3d"])
        ...
        # Sample for new GSs
        eps = torch.finfo(torch.float32).eps
        probs = torch.sigmoid(self.params["opacities"])
        probs = probs / (probs.sum() + eps)
        sampled_idxs = torch.multinomial(probs, n_gs, replacement=True)
        new_opacities, new_scales = compute_relocation(
            opacities=torch.sigmoid(self.params["opacities"])[sampled_idxs],
        ...
        # Update splats and optimizers
        for k in self.params.keys():
            self.params[k] = torch.cat([self.params[k], self.params[k][sampled_idxs]])
        for name, optimizer in self.optimizers.items():
            for i, param_group in enumerate(optimizer.param_groups):
                p = param_group["params"][0]
                p_state = optimizer.state[p]
