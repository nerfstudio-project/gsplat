...
from .ops import duplicate, remove
...
class ScaffoldStrategy(Strategy):
    ...
    colors_mlp: torch.nn.Sequential = torch.nn.Sequential(
        torch.nn.Linear(mean_feat_dim + 3, mean_feat_dim),
        torch.nn.ReLU(True),
        torch.nn.Linear(mean_feat_dim, 3 * n_feat_offsets),
        torch.nn.Sigmoid()
    ).cuda()
    opacities_mlp: torch.nn.Sequential = torch.nn.Sequential(
        torch.nn.Linear(mean_feat_dim + 3, mean_feat_dim),
        torch.nn.ReLU(True),
        torch.nn.Linear(mean_feat_dim, n_feat_offsets),
        torch.nn.Tanh()
    ).cuda()
    scale_rot_mlp: torch.nn.Sequential = torch.nn.Sequential(
        torch.nn.Linear(mean_feat_dim + 3, mean_feat_dim),
        torch.nn.ReLU(True),
        torch.nn.Linear(mean_feat_dim, 7 * n_feat_offsets)
    ).cuda()
    ...
    def step_post_backward(
        ...
        if (
            ...
            # grow GSs
            n_dupli, n_split = self._grow_anchors(params, optimizers, state, step)
            if self.verbose:
                print(
    ...
    def _update_state(
        ...
        for key in ["anchors2d", "width", "height", "n_cameras", "radii", "gaussian_ids"]:
            assert key in info, f"{key} is required but missing."

        # normalize grads to [-1, 1] screen space
        if self.absgrad:
            grads = info["anchors2d"].absgrad.clone()
        else:
            grads = info["anchors2d"].grad.clone()
        grads[..., 0] *= info["width"] / 2.0 * info["n_cameras"]
        grads[..., 1] *= info["height"] / 2.0 * info["n_cameras"]

        # initialize state on the first run
        n_gaussian = len(list(params["anchors"].shape[0]))
