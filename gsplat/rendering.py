...
from .cuda._wrapper import (
    ...
    fully_fused_projection_3dcs,
    isect_offset_encode,
    isect_tiles,
    rasterize_to_pixels,
    rasterize_to_pixels_2dgs,
    rasterize_to_pixels_3dcs,
    spherical_harmonics,
...
### 3DCS ###
def rasterization_3dcs(
    convex_points: Tensor,  # [N, K, 3]
    delta: Tensor,          # [N]
    sigma: Tensor,          # [N]
    num_points_per_convex: int, # K, default is 6
    cumsum_of_points_per_convex: Tensor,
    opacities: Tensor,  # [N]
    colors: Tensor,  # [(C,) N, D] or [(C,) N, K, 3]
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [C, 3, 3]
    ...
    """Rasterize a set of 3D Convex Points (N) to a batch of image planes (C).

    This function provides a handful features for 3D Convex rasterization, which
    we detail in the following notes. A complete profiling of the these features
    can be found in the :ref:`profiling` page.
    ...
    .. note::
        **Sparse Gradients**: If `sparse_grad` is True, the gradients for {convex_points, quats, scales}
        will be stored in a `COO sparse layout <https://pytorch.org/docs/stable/generated/torch.sparse_coo_tensor.html>`_.
        This can be helpful for saving memory
        for training when the scene is large and each iteration only activates a small portion
        of the Gaussians. Usually a sparse optimizer is required to work with sparse gradients,
    ...
    Args:
        convex_points: The 3D Points representing the 3D convex structure. [N, K, 3]
        delta: The delta values of the 3D convex. [N]
        sigma: The sigma values of the 3D convex. [N]
        num_points_per_convex: The number of points per convex. Default is 6.
        cumsum_of_points_per_convex: The cumsum of points per convex. [N]
        opacities: The opacities of the Gaussians. [N]
        colors: The colors of the Gaussians. [(C,) N, D] or [(C,) N, K, 3] for SH coefficients.
        viewmats: The world-to-cam transformation of the cameras. [C, 4, 4]
        Ks: The camera intrinsics. [C, 3, 3]
        ...
        render_mode: The rendering mode. Supported modes are "RGB", "D", "ED", "RGB+D",
            and "RGB+ED". "RGB" renders the colored image, "D" renders the accumulated depth, and
            "ED" renders the expected depth. Default is "RGB".
        sparse_grad: If true, the gradients for {convex_points, quats, scales} will be stored in
            a COO sparse layout. This can be helpful for saving memory. Default is False.
        absgrad: If true, the absolute gradients of the projected 2D means
            will be computed during the backward pass, which could be accessed by
            `meta["means2d"].absgrad`. Default is False.
    ...
    .. code-block:: python
        ...
        >>> # define Gaussians
        >>> convex_points = torch.randn((100, 3), device=device)
        >>> colors = torch.rand((100, 3), device=device)
        >>> opacities = torch.rand((100,), device=device)
        >>> # define cameras
        >>> viewmats = torch.eye(4, device=device)[None, :, :]
        ...
        >>> # render
        >>> colors, alphas, meta = rasterization(
        >>>    convex_points, quats, scales, opacities, colors, viewmats, Ks, width, height
        >>> )
        >>> print (colors.shape, alphas.shape)
        torch.Size([1, 200, 300, 3]) torch.Size([1, 200, 300, 1])
        >>> print (meta.keys())
    ...
    N = convex_points.shape[0]
    C = viewmats.shape[0]
    device = convex_points.device
    assert convex_points.shape == (N, num_points_per_convex, 3), convex_points.shape
    assert opacities.shape == (N,), opacities.shape
    assert viewmats.shape == (C, 4, 4), viewmats.shape
    assert Ks.shape == (C, 3, 3), Ks.shape
    assert render_mode in ["RGB", "D", "ED", "RGB+D", "RGB+ED"], render_mode
    ...
    # Special for 3DCS
    scaling = torch.zeros_like(convex_points[:,0,0].squeeze(), dtype=convex_points.dtype, requires_grad=True, device="cuda").detach()

    # If in distributed mode, we distribute the projection computation over Gaussians
    # and the rasterize computation over cameras. So first we gather the cameras
    # from all ranks for projection.
    # FIXME: Probably rework this part.
    if distributed:
       world_rank = torch.distributed.get_rank()
       world_size = torch.distributed.get_world_size()
    ...
    # Project 3D convex points to 2D convex hull.
    proj_results = fully_fused_projection_3dcs(
        convex_points,
    ...
    if packed:
        ...
        # The results are with shape [C, N, ...]. Only the elements with radii > 0 are valid.
        normals, offsets, _, _, num_points_per_convex_view, _, radii, means2d, depths, conics, compensations = proj_results
        opacities = opacities.repeat(C, 1)  # [C, N]
        camera_ids, gaussian_ids = None, None
    ...
    if sh_degree is None:
        ...
        camtoworlds = torch.inverse(viewmats)  # [C, 4, 4]
        if packed:
            dirs = convex_points[None, gaussian_ids, :] - camtoworlds[camera_ids, None, :3, 3]  # [nnz, 3]
            masks = (radii > 0).all(dim=-1)  # [nnz]
            if colors.dim() == 3:
                # Turn [N, K, 3] into [nnz, 3]
                shs = colors[gaussian_ids, :, :]  # [nnz, K, 3]
                ...
                # Turn [C, N, K, 3] into [nnz, 3]
                shs = colors[camera_ids, gaussian_ids, :, :]  # [nnz, K, 3]
            colors = spherical_harmonics_3dcs(sh_degree, convex_points, dirs, shs, masks=masks)  # [nnz, 3]
        else:
            # Compute direction from the center of the 3D convex i.e means3d
            # FIXME: Not the most efficient? Should we do that in the cuda call?
            means3d = convex_points.mean(dim=1)
            dirs = means3d[None, :, :] - camtoworlds[:, None, :3, 3]  # [C, N, 3]
            masks = (radii > 0).all(dim=-1)  # [C, N]
            if colors.dim() == 3:
                # Turn [N, K, 3] into [C, N, K, 3]
                shs = colors.expand(C, -1, -1, -1)  # [C, N, K, 3]
                ...
                # colors is already [C, N, K, 3]
                shs = colors
        colors = spherical_harmonics_3dcs(sh_degree, convex_points, dirs, shs, masks=masks)  # [C, N, 3]
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_ = rasterize_to_pixels_3dcs(
                means2d,
                normals,
                offsets,
                num_points_per_convex_view,
        ...
        render_colors, render_alphas = rasterize_to_pixels_3dcs(
            means2d,
            normals,
            offsets,
            num_points_per_convex_view,
...
#def _rasterization_3dcs(
#    convex_points: Tensor,  # [N, 3]
#    opacities: Tensor,  # [N]
#    colors: Tensor,  # [(C,) N, D] or [(C,) N, K, 3]
#    viewmats: Tensor,  # [C, 4, 4]
#    Ks: Tensor,  # [C, 3, 3]
...
#    N = convex_points.shape[0]
#    C = viewmats.shape[0]
#    # FIXME: Replace 6 with the num_points_per_convex
#    assert convex_points.shape == (N, 6, 3), convex_points.shape
#    assert opacities.shape == (N,), opacities.shape
#    assert viewmats.shape == (C, 4, 4), viewmats.shape
#    assert Ks.shape == (C, 3, 3), Ks.shape
#    assert render_mode in ["RGB", "D", "ED", "RGB+D", "RGB+ED"], render_mode
...
#    # The results are with shape [C, N, ...]. Only the elements with radii > 0 are valid.
#    covars, _ = _quat_scale_to_covar_preci(quats, scales, True, False, triu=False)
#    radii, means2d, depths, conics, compensations = _fully_fused_projection_3dcs(
#        convex_points,
#        covars,
#        viewmats,
#        Ks,
...
#        # Colors are SH coefficients, with shape [N, K, 3] or [C, N, K, 3]
#        camtoworlds = torch.inverse(viewmats)  # [C, 4, 4]
#        dirs = convex_points[None, :, :] - camtoworlds[:, None, :3, 3]  # [C, N, 3]
#        masks = radii > 0  # [C, N]
#        if colors.dim() == 3:
#            # Turn [N, K, 3] into [C, N, 3]
#            shs = colors.expand(C, -1, -1, -1)  # [C, N, K, 3]
