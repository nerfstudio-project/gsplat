...
def rasterization_2dgs_hold(
    ...
    if distributed:
        if packed:
            ...
            (means2d, depths, ray_transformations, opacities, colors, normals) = all_to_all_tensor_list(
                world_size,
                [means2d, depths, ray_transformations, opacities, colors, normals],
                cnts,
                output_splits=collected_splits,
            )         
            ...
            (means2d, depths, ray_transformations, opacities, colors, normals) = all_to_all_tensor_list(
                ...
                [
                    means2d.flatten(0, 1),
                    depths.flatten(0, 1),
                    ray_transformations.flatten(0, 1),
                    opacities.flatten(0, 1),
                    colors.flatten(0, 1),
                    normals.flatten(0, 1),
                ],
            ...
            means2d = means2d.reshape(C, -1, 2)
            depths = depths.reshape(C, -1)
            ray_transformations = ray_transformations.reshape(C, -1, 3, 3)
            opacities = opacities.reshape(C, -1)
            colors = colors.reshape(C, -1, colors.shape[-1])
            normals = normals.reshape(C, -1, 3)
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_distort.append(render_distort_)
            render_median.append(render_median_)
        render_colors = torch.cat(render_colors, dim=-1)
        render_alphas = render_alphas[0] # discard the rest
        render_normals = torch.cat(render_colors, dim=-1)
        render_distort = torch.cat(render_distort, dim=-1)
        render_median = torch.cat(render_median, dim=-1)
    else:
        (
            render_colors,
            render_alphas,
