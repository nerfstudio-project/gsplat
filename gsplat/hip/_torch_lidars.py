...
# Note: The hierarchy should be rooted at new class SensorModel.
class _LidarModel:
...
# Note: Since we don't have SensorModel yet, we make StructuredLidarModel inherit from _BaseCameraModel,
...
class _StructuredLidarModel(_LidarModel, _BaseCameraModel):
    def __init__(self, params: StructuredLidarModelParameters) -> None:
        ...
        # Note: passing shutter type because the base expects it, but Lidar doesn't use it.
...
class _RowOffsetStructuredSpinningLidarModel(_StructuredSpinningLidarModel):
    ...
    def focal_lengths(self) -> Tensor:
        ...
        # Note: fix the class hierarchy to properly accomodate lidar sensors
    ...
    def element_to_image_point(self, row: Tensor, col: Tensor) -> Tensor:
        ...
        Mirrors the CUDA ``element_to_image_point`` in Lidars.hip.h: looks up
