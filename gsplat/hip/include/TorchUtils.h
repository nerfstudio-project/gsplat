...
namespace detail
{
    ...
    template<class V>
    inline constexpr bool is_optional_tensor_v = std::is_same_v<value_type_t<is_optional<V>>, at::Tensor>;

    template<class V>
    inline constexpr bool is_optional_intrusive_v = is_intrusive_ptr<value_type_t<is_optional<V>>>::value;
} // namespace detail
...
{
    return at::GradMode::is_enabled() && (tensor_requires_grad(tensors) || ...);
}
...
template<class T>
inline T *data_ptr_or_null(bool enabled, const at::optional<at::Tensor> &tensor)
{
...
{
    static_assert(!std::is_const_v<ScalarT>, "Put const on PointerT, not ScalarT");
    using TensorPointerT = std::conditional_t<std::is_const_v<PointerT>, const ScalarT, ScalarT>;
...
inline at::Tensor tensor_or_zeros(
    const at::optional<at::Tensor> &grad, at::IntArrayRef shape, const at::TensorOptions &options
)
{
    return grad.has_value() ? grad.value() : at::zeros(shape, options);
}

inline at::Tensor tensor_or_zeros_like(const at::optional<at::Tensor> &grad, const at::Tensor &reference)
{
    return grad.has_value() ? grad.value() : at::zeros_like(reference);
}

inline at::Tensor make_sparse_coo_grad(
    const at::Tensor &indices, const at::Tensor &values, at::IntArrayRef size, bool is_coalesced
)
{
...
{
    static int64_t to(T v)
    {
        return static_cast<int64_t>(v);
    }

    static T from(int64_t v)
    {
...
struct TorchArgDef<T>
{
    static double to(T v)
    {
        return static_cast<double>(v);
    }

    static T from(double v)
    {
        return static_cast<T>(v);
    }
};
...
struct TorchArgDef<std::tuple<Ts...>>
{
    ...
    {
        return std::apply([](auto &&...es) { return to_torch_args(std::forward<decltype(es)>(es)...); }, std::move(t));
    }
...
namespace detail
{
    ...
    struct ToNativeArgs<std::tuple<Args...>>
    {
        ...
        {
            constexpr std::size_t arity[sizeof...(Args)] = {std::tuple_size_v<as_tuple_t<torch_arg_t<Args>>>...};
            std::size_t off                              = 0;
        ...
        static decltype(auto) rebuild(Packed &&packed)
        {
            ...
            if constexpr(IsTuple<torch_arg_t<A>>)
            {
                ...
                {
                    return TA::from(std::forward_as_tuple(std::get<off + J>(std::forward<Packed>(packed))...));
    ...
    private:
        ...
        {
            decltype(auto) result = Fn(ToNativeArgs<std::tuple<Args...>>::template rebuild<Arg, Args>(packed)...);
            return TorchArgDef<std::decay_t<decltype(result)>>::to(std::forward<decltype(result)>(result));
        }
    };
    ...
    template<auto Fn>
    using ToTorchOp = ToTorchOpImpl<
        Fn,
        typename fn_traits<decltype(Fn)>::args,
...
private:
    ...
    static std::tuple<std::optional<std::tuple_element_t<Is, inner_t>>...> to_values(
        std::optional<T> v, std::index_sequence<Is...>
    )
    {
        if(!v.has_value())
        {
    ...
    static std::optional<T> from_values(Values &&vals, std::index_sequence<Is...>)
    {
        ...
        return TorchArgDef<T>::from(std::make_tuple(std::move(*std::get<Is>(std::forward<Values>(vals)))...));
    }
...
public:
    ...
    {
        return from_values(std::forward<Values>(vals), std::make_index_sequence<std::tuple_size_v<inner_t>>{});
    }
};
...
{
    static_assert(
        (detail::IsTorch<torch_arg_t<std::decay_t<Args>>> && ...),
...
namespace detail
{
    ...
    auto invoke_torch_op(TypedOp &op, Args &&...args)
    {
        ...
        auto ret  = [&]
        {
            ...
            {
                return std::apply(
                    [&](auto &&...f) { return op.call(std::forward<decltype(f)>(f)...); }, std::move(flat)
                );
            }
            else
            {
...
auto call_torch_op(const char *qualified_name, Args &&...args)
{
    ...
    using DispatchSig = detail::typed_sig_t<torch_arg_t<ResultT>, detail::flat_torch_types_t<typename Traits::args>>;

    // FwdFn is unique per op, so the resolved handle is cached in a static.
    static auto op = c10::Dispatcher::singleton().findSchemaOrThrow(qualified_name, "").typed<DispatchSig>();
...
auto call_torch_op(const char *qualified_name, Args &&...args)
{
    ...
    auto op = c10::Dispatcher::singleton().findSchemaOrThrow(qualified_name, "").typed<DispatchSig>();
...
namespace detail
{
    ...
    struct drop_last<std::tuple<Ts...>>
    {
        ...
        static auto pick(std::index_sequence<I...>) -> std::tuple<std::tuple_element_t<I, std::tuple<Ts...>>...>;
    ...
    template<auto BwdFn>
    using bwd_grad_bundle_t = std::tuple_element_t<
        std::tuple_size_v<typename fn_traits<decltype(BwdFn)>::decayed_args> - 1,
...
inline constexpr std::size_t fwd_input_count()
{
    ...
    static_assert(
        std::is_same_v<std::tuple_element_t<0, args>, torch::autograd::AutogradContext *>,
...
namespace detail
{
    ...
    inline void save_one(torch::autograd::AutogradContext *ctx, torch::autograd::variable_list &tensors, const V &v)
    {
    ...
    // `.to<>()` / `.toCustomClass<>()` call. hipcc's EDG frontend treats the
    ...
    template<std::size_t I, class V>
    inline V load_one(
        torch::autograd::AutogradContext *ctx, const torch::autograd::variable_list &tensors, std::size_t &t
    )
    {
        if constexpr(std::is_same_v<V, at::Tensor>)
        {
    ...
    inline void ctx_save_impl(torch::autograd::AutogradContext *ctx, const Tuple &vals, std::index_sequence<I...>)
    {
    ...
    inline bwd_saved_t<BwdFn> ctx_load_impl(torch::autograd::AutogradContext *ctx, std::index_sequence<I...>)
    {
...
{
    static_assert(
        detail::GradBundle<detail::bwd_grad_bundle_t<BwdFn>>,
        "ctx_save: backward's last parameter must be the grad bundle "
        "(variable_list, or a struct tagged is_grad_bundle)."
    );
    static_assert(
        std::is_same_v<std::tuple<std::decay_t<Vals>...>, detail::bwd_saved_t<BwdFn>>,
        "ctx_save values must match the backward function's saved-state "
        "parameters (types and order) exactly."
    );
    detail::ctx_save_impl<BwdFn>(ctx, std::forward_as_tuple(vals...), std::make_index_sequence<sizeof...(Vals)>{});
}
