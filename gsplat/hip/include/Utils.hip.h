...
{
    static_assert(std::is_floating_point_v<T>, "is_near_zero requires a floating-point type");
...
)
{
    ...
    v_R = gsplat_matadd(v_R, gsplat_outer_product(v_pC, pW));
    v_t  += v_pC;
    v_pW += gsplat_matvec(gsplat_transpose(R), v_pC);
}
...
{
    covarC = gsplat_matmul(gsplat_matmul(R, covarW), gsplat_transpose(R));
}
...
)
{
    ...
    v_R = gsplat_matadd(v_R, gsplat_matadd(gsplat_matmul(gsplat_matmul(v_covarC, R), gsplat_transpose(covarW)), gsplat_matmul(gsplat_matmul(gsplat_transpose(v_covarC), R), covarW)));
    v_covarW = gsplat_matadd(v_covarW, gsplat_matmul(gsplat_matmul(gsplat_transpose(R), v_covarC), R));
}
...
inline __device__ void quat_to_rotmat_vjp(const vec4 quat, const mat3 v_R, vec4 &v_quat)
{
    float w = quat[0], x = quat[1], y = quat[2], z = quat[3];
    // normalize
    float inv_norm  = rsqrt_parse_safe(x * x + y * y + z * z + w * w);
    ...
    v_quat      += (v_quat_n - gsplat_dot(v_quat_n, quat_n) * quat_n) * inv_norm;
}
...
)
{
    ...
    if(covar != nullptr)
    {
        // C = gsplat_matmul(R, S) * S * Rt
        mat3 S = mat3(scale[0], 0.f, 0.f, 0.f, scale[1], 0.f, 0.f, 0.f, scale[2]);
        mat3 M = gsplat_matmul(R, S);
        *covar = gsplat_matmul(M, gsplat_transpose(M));
    }
    if(preci != nullptr)
    {
...
)
{
    ...
    mat3 v_M = gsplat_matmul((v_covar + gsplat_transpose(v_covar)), M);
    mat3 v_R = gsplat_matmul(v_M, S);
...
)
{
    ...
    // for D = M * Mt,
    // df/dM = df/dM + df/dMt = G * M + (Mt * G)t = G * M + Gt * M
    mat3 v_M = gsplat_matmul((v_preci + gsplat_transpose(v_preci)), M);
    mat3 v_R = gsplat_matmul(v_M, S);
...
)
{
    ...
    if(preci_half != nullptr)
    {
        ...
        // Gaussians (scale < epsilon) so they never reach this path.
        assert(scale[0] != 0.f && scale[1] != 0.f && scale[2] != 0.f);
        // P = gsplat_matmul(R, S)^-1
        mat3 S      = mat3(1.0f / scale[0], 0.f, 0.f, 0.f, 1.0f / scale[1], 0.f, 0.f, 0.f, 1.0f / scale[2]);
        *preci_half = gsplat_matmul(R, S);
    }
}
...
inline __device__ void inverse_vjp(const mat2 Minv, const mat2 v_Minv, mat2 &v_M)
{
    // P = M^-1
    // df/dM = -P * df/dP * P
    v_M = gsplat_matadd(v_M, gsplat_matmul(gsplat_matmul(gsplat_scale_mat(Minv, -1.0f), v_Minv), Minv));
}
...
inline __device__ void add_blur_vjp(
    const float eps2d, const mat2 conic_blur, const float compensation, const float v_compensation, mat2 &v_covar
)
{
...
)
{
    ...
    cov2d  = gsplat_matmul(gsplat_matmul(J, cov3d), gsplat_transpose(J));
...
)
{
    ...
    // -> df/dV = Jt * G * J
    // -> df/dJ = G * J * Vt + Gt * J * V
    v_cov3d = gsplat_matadd(v_cov3d, gsplat_matmul(gsplat_matmul(gsplat_transpose(J), v_cov2d), J));
...
)
{
    ...
    v_mean3d
        += vec3(fx * rz * v_mean2d[0], fy * rz * v_mean2d[1], -(fx * x * v_mean2d[0] + fy * y * v_mean2d[1]) * rz2);
    ...
    float rz3  = rz2 * rz;
    mat3x2 v_J = gsplat_matadd(gsplat_matmul(gsplat_matmul(v_cov2d, J), gsplat_transpose(cov3d)), gsplat_matmul(gsplat_matmul(gsplat_transpose(v_cov2d), J), cov3d));
...
)
{
    ...
    float xy_len = gsplat_length(glm::vec2({x, y})) + eps;
    float theta  = atan2f(xy_len, z + eps);
    ...
    float b  = atan2f(xy_len, z) / xy_len / x2y2;
...
)
{
    ...
    float b             = atan2f(len_xy, z) / len_xy / x2y2;
    float a             = z * x2y2z2_inv / (x2y2);
    v_mean3d           += vec3(
    ...
    const float theta = atan2f(len_xy, z);
    ...
    float dL_dtx_raw  = dJ_dx00 * v_J[0][0]
                      + dJ_dx01 * v_J[1][0]
                      + dJ_dx02 * v_J[2][0]
                      + dJ_dx10 * v_J[0][1]
                      + dJ_dx11 * v_J[1][1]
                      + dJ_dx12 * v_J[2][1];
    float dL_dty_raw  = dJ_dy00 * v_J[0][0]
                      + dJ_dy01 * v_J[1][0]
                      + dJ_dy02 * v_J[2][0]
                      + dJ_dy10 * v_J[0][1]
                      + dJ_dy11 * v_J[1][1]
                      + dJ_dy12 * v_J[2][1];
    float dL_dtz_raw  = dJ_dz00 * v_J[0][0]
                      + dJ_dz01 * v_J[1][0]
                      + dJ_dz02 * v_J[2][0]
                      + dJ_dz10 * v_J[0][1]
                      + dJ_dz11 * v_J[1][1]
...
// three multiplies and two adds open to hipcc's statement-scope FMA
...
inline __device__ vec3 safe_normalize_bw(const vec3 &v, const vec3 &d_out)
{
    ...
    if(l > 0.0f)
    {
        ...
        return il * d_out - il3 * gsplat_dot(d_out, v) * v;
    }
    return d_out;
}
} // namespace gsplat
