...
if not hasattr(jit, "_get_build_directory"):
    raise RuntimeError(
        ...
        "get_gsplat_build_directory in gsplat/hip/build.py."
    )
...
# hipcc requires escaped commas in -D values.
def _hip_num_channels_define(num_channels):
    return "-DGSPLAT_NUM_CHANNELS=" + num_channels


def format_jit_hip_cflags(cuda_cflags):
...
def _detect_rocm_target_archs():
    """Return the list of gfx offload archs this build is targeting.

    Precedence mirrors what PyTorch's own ROCm cpp_extension path uses to
    decide which ``--offload-arch=`` flags hipcc gets:
      1. ``PYTORCH_ROCM_ARCH`` (the documented, current env var).
      2. ``HCC_AMDGPU_TARGET`` (older/legacy alias some build scripts still
         set; this repo's own canonical Instinct build command sets both).
      3. If neither is set, fall back to the gcnArchName of every ROCm GPU
         actually visible to this process (``torch.cuda.get_device_properties``)
         so a plain ``pip install -e .`` / JIT build on a single dev machine
         still picks the right flag set without requiring the env var.
    Returns an empty list if none of the above yields anything (e.g. no ROCm
    GPU visible at build time) — callers must treat that as "unknown" and
    keep today's behavior (blanket FAST_MATH) rather than guessing.
    """
    archs_env = os.getenv("PYTORCH_ROCM_ARCH") or os.getenv("HCC_AMDGPU_TARGET")
    if archs_env:
        # PYTORCH_ROCM_ARCH is documented as ';'-separated; HCC_AMDGPU_TARGET
        # has historically used ','. Accept both plus stray whitespace, and
        # strip any ':xnack+/-' feature suffix before matching the arch name.
        raw = re.split(r"[;,\s]+", archs_env.strip())
        archs = [a.split(":")[0] for a in raw if a]
        if archs:
            return archs
...
def _is_instinct_arch(arch):
    """True for CDNA/Instinct archs (gfx900/906/908/90a, gfx94x, gfx950, ...).

    RDNA/Radeon archs (gfx10xx, gfx11xx, gfx12xx) do NOT match this pattern.
    """
    return bool(re.match(r"^gfx9\d\d", arch))
...
def get_build_parameters():
    ...
    # Source files ------------------------------------
    sources = (
        list(glob.glob(os.path.join(PATH, "csrc/*.hip")))
    ...
    if DEBUG:
        ...
        if sys.platform != "win32":  # MSVC equivalent (/W4 /WX) is untested
            extra_cflags += ["-Wall"]
            extra_hip_cflags += [
                # hipcc intercepts bare -Werror as its own --Werror flag, so
        ...
        if sys.platform != "win32":
            ...
            extra_hip_cflags += ["-O2", "-DNDEBUG"]

    if FAST_MATH:
        _target_archs = _detect_rocm_target_archs()
        _instinct_build = any(_is_instinct_arch(a) for a in _target_archs)
        if _instinct_build:
            # Instinct/CDNA: blanket -ffast-math's unsafe reciprocal +
            # reassociation reordering can silently break exact floor/ceil
            # boundary invariants and reorder atomic-summed gradient
            # accumulation on AMD/clang — we already root-caused and hand-
            # patched one confirmed instance of exactly this bug class with a
            # scoped `#pragma clang fp reciprocal(off) reassociate(off)
            # contract(off)` in IntersectTileLidar.hip (kept unchanged,
            # belt-and-suspenders, regardless of this flag). A separate,
            # independent CUDA->HIP translation of this same project
            # (gsplat_push) hit the identical class of issue on real Instinct
            # hardware and documents using this exact conservative flag set
            # instead of blanket -ffast-math for CDNA targets, while keeping
            # full -ffast-math for Radeon. Mirror that here: this is a
            # proactive, evidence-motivated safety measure for Instinct, not
            # a fix verified on real Instinct hardware this session (no
            # Instinct host was accessible) — see optimizer_report.md.
            extra_hip_cflags += [
                "-fno-math-errno",
                "-ffp-contract=fast",
            ...
            # Radeon/RDNA (and the "unknown target arch" fallback, which
            # preserves this flag's pre-existing blanket behavior exactly).
            extra_hip_cflags += ["-ffast-math"]
    ...
    #       torch::python::module member is valid code, but hipcc emits
    if not os.name == "nt":
    ...
    if BUILD_2DGS is not None:
        ...
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_2DGS={BUILD_2DGS}"]
    if BUILD_3DGS is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_3DGS={BUILD_3DGS}"]
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_3DGS={BUILD_3DGS}"]
    if BUILD_3DGUT is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_3DGUT={BUILD_3DGUT}"]
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_3DGUT={BUILD_3DGUT}"]
    if BUILD_ADAM is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_ADAM={BUILD_ADAM}"]
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_ADAM={BUILD_ADAM}"]
    if BUILD_RELOC is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_RELOC={BUILD_RELOC}"]
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_RELOC={BUILD_RELOC}"]
    if BUILD_LOSSES is not None:
        extra_cflags += [f"-DGSPLAT_BUILD_LOSSES={BUILD_LOSSES}"]
        if sys.platform == "win32":
            extra_hip_cflags += [f"-DGSPLAT_BUILD_LOSSES={BUILD_LOSSES}"]
    if BUILD_CAMERA_WRAPPERS:
        extra_cflags += ["-DGSPLAT_BUILD_CAMERA_WRAPPERS=1"]
        if sys.platform == "win32":
            extra_hip_cflags += ["-DGSPLAT_BUILD_CAMERA_WRAPPERS=1"]
    else:
        # Remove 'csrc/CameraWrappers.hip' from the sources list if it exists
        sources = [s for s in sources if not s.endswith("csrc/CameraWrappers.hip")]

    extra_ldflags += [] if WITH_SYMBOLS or sys.platform == "win32" else ["-s"]

    if WITH_SYMBOLS:
        extra_hip_cflags += ["-lineinfo"]
    ...
    if (
        ...
        if sys.platform == "win32":
            extra_cflags += ["/openmp"]
            extra_hip_cflags += ["-Xcompiler", "/openmp"]
        else:
            extra_cflags += ["-fopenmp"]
        if sys.platform == "win32":
            extra_hip_cflags += ["-DAT_PARALLEL_OPENMP"]
    else:
        print("Compiling without OpenMP...")

    if sys.platform != "win32":
        extra_hip_cflags += extra_cflags

    # Add -Werror after the copy so it reaches gcc but not hipcc (see DEBUG block above).
    if DEBUG and sys.platform != "win32":
        extra_cflags += ["-Werror"]

    if NUM_CHANNELS is not None:
        # hipcc treats an unescaped comma in a -D value as a second macro identifier,
        # so escape it as \, here. The AOT path passes this straight to hipcc; the
        # JIT path additionally shell-quotes it (see format_jit_hip_cflags).
        extra_hip_cflags += [_hip_num_channels_define(NUM_CHANNELS)]
        # gcc would not grok the backslash, so here we just pass NUM_CHANNELS as is.
        extra_cflags += [f"-DGSPLAT_NUM_CHANNELS={NUM_CHANNELS}"]

    extra_hip_cflags += [] if HIPCC_FLAGS == "" else HIPCC_FLAGS.split(" ")
...
def build_and_load_gsplat():
    ...
    def status_context():
        tic = time.time()
        msg = f"gsplat: Setting up HIP with MAX_JOBS={MAX_JOBS if MAX_JOBS else 'max'} (This may take a few minutes the first time)"
        if _console is not None:
        ...
        if _console is not None:
            _console.print(
                f"[green]gsplat: HIP extension has been set up successfully in {toc - tic:.2f} seconds.[/green]"
            )
        else:
            print(
                f"gsplat: HIP extension has been set up successfully in {toc - tic:.2f} seconds."
            )
    ...
    with (
        ...
        try:
            ...
            gsplat_module = jit.load(
                ...
                extra_cuda_cflags=format_jit_hip_cflags(build_params.extra_hip_cflags),
...
def _setup_py_core_objects_for_loaded_csrc(return_build_dir=False):
    ...
    ``<repo>/build/temp.<platform>-cpython-<tag>/gsplat/hip/{csrc/*.o,ext.o}``.
...
def _setup_py_extra_hip_cflags_for_loaded_csrc():
    """Reconstruct ``extra_hip_cflags`` from setup.py's build.ninja.

    Strips the prefix/suffix that torch's extension build path injects on
    Linux/macOS (see ``unix_cuda_flags`` and ``COMMON_HIPCC_FLAGS`` in
    ``torch.utils.cpp_extension``) so the result is comparable with the
    ``extra_hip_cflags`` produced by ``get_build_parameters()``.
    """
...
def _setup_py_build_parameters_mismatch_reason(build_params):
    ...
    recorded_cuda_cflags = _setup_py_extra_hip_cflags_for_loaded_csrc()
    # `_setup_py_extra_hip_cflags_for_loaded_csrc()` drops every torch-injected
    ...
    expected_cuda_cflags = [
        f for f in build_params.extra_hip_cflags if f != "--expt-relaxed-constexpr"
    ]
    ...
    return (
        ...
        f"setup.py extra_hip_cflags: {recorded_cuda_cflags}\n"
        f"pytest extra_hip_cflags: {expected_cuda_cflags}"
    )


_LINK_INPUTS_MISSING_MSG = (
    "gsplat.csrc is loaded but no setup.py-built .o files were found under "
    "<repo>/build/temp.*-cpython-<tag>/gsplat/hip/. Native C++ tests need "
...
def get_gsplat_link_inputs_skip_reason(build_params=None):
    ...
    missing ROCm toolkit) are not detected here — they surface at the call
    site.
    """
...
def get_gsplat_link_inputs():
    ...
    * setup.py-built ``gsplat.csrc`` (editable install). Reuse the ``.o``
      files at ``<repo>/build/temp.*-cpython-<tag>/gsplat/hip/`` so the
