 ...
 * `RasterizeToPixelsFromWorld3DGSParallelBatchBwd.hip` (a `.hip` translation unit, so that
 * hipcc can emit the kernel launch). This header only exposes the constants
 * + declarations so both `.hip` files and `Rasterization.cpp` (a pure C++
...
// `RasterizeToPixelsFromWorld3DGSParallelBatchBwd.hip` so the CUDA kernel launch +
// cumsum can be emitted by hipcc.
