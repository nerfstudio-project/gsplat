...
// Note: replacing the following with per-operation kernels might make compile
// faster.
...
namespace
{
    void check_quat_scale_to_covar_preci_inputs(const at::Tensor &quats, const at::Tensor &scales)
    {
        TORCH_CHECK(quats.dim() >= 1, "quats must have shape [..., 4], got ", quats.sizes());
        TORCH_CHECK(scales.dim() == quats.dim(), "scales must have shape [..., 3], got ", scales.sizes());

        const int64_t batch_ndim = quats.dim() - 1;
        TORCH_CHECK(quats.size(batch_ndim) == 4, "quats must have shape [..., 4], got ", quats.sizes());

        at::DimVector scales_shape(quats.sizes().slice(0, batch_ndim));
        scales_shape.append({3});
        TORCH_CHECK(scales.sizes() == scales_shape, "scales must have shape [..., 3], got ", scales.sizes());
        CHECK_INPUT(quats);
        CHECK_INPUT(scales);
    }
} // namespace
...
template<>
struct TorchArgDef<QuatScaleToCovarPreciFwdResult>
{
    static auto to(const QuatScaleToCovarPreciFwdResult &r)
    {
        return to_torch_args(r.covars, r.precis);
    }
};
...
template<>
struct TorchArgDef<QuatScaleToCovarPreciBwdResult>
{
...
template<>
struct TorchArgDef<QuatScaleToCovarPreciGrad>
{
    static auto to(const QuatScaleToCovarPreciGrad &g)
    {
        return to_torch_args(g.covars, g.precis);
    }
...
)
{
    ...
    {
        launch_quat_scale_to_covar_preci_bwd_kernel(quats, scales, triu, grad.covars, grad.precis, v_quats, v_scales);
    }
    else
    {
