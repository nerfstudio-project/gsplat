...
GSPLAT_HOST_DEVICE constexpr uint32_t encode_from_fp16_bits(uint16_t T_fp16_bits, int32_t K, bool saturated)
{
    ...
    const uint32_t T_bits  = static_cast<uint32_t>(T_fp16_bits) & detail::T_MAGNITUDE_MASK;
    const uint32_t neg_K   = static_cast<uint32_t>(static_cast<uint16_t>(-K));
    const uint32_t sat_inv = saturated ? 0u : detail::SAT_BIT_INV;
    return sat_inv | (T_bits << detail::T_SHIFT) | neg_K;
}

constexpr uint32_t INIT_PACKED = encode_from_fp16_bits(detail::fp16_power_of_two_bits(0), 0, false);

static_assert((INIT_PACKED & detail::SAT_BIT_INV) != 0u, "init must have !sat=1");
static_assert((INIT_PACKED & detail::K_MASK) == 0u, "init K must be 0");
static_assert(
    ((INIT_PACKED & detail::T_MASK) >> detail::T_SHIFT) == detail::fp16_power_of_two_bits(0),
...
constexpr uint32_t INVALID_RAY_PACKED = encode_from_fp16_bits(0u, 0, true);

static_assert(INVALID_RAY_PACKED == 0u, "invalid ray sentinel must remain the atomicMin floor");

__device__ __forceinline__ uint16_t __half_as_ushort_rocm(__half value)
{
    return *reinterpret_cast<uint16_t *>(&value);
}

__device__ __forceinline__ __half __ushort_as_half_rocm(uint16_t value)
{
    return *reinterpret_cast<__half *>(&value);
}
...
__device__ __forceinline__ uint32_t encode(float T, int32_t K, bool saturated)
{
    ...
    const uint32_t T_bits = static_cast<uint32_t>(__half_as_ushort_rocm(__float2half_ru(T))) & detail::T_MAGNITUDE_MASK;
    return encode_from_fp16_bits(static_cast<uint16_t>(T_bits), K, saturated);
}

__device__ __forceinline__ void decode(uint32_t packed, float &T, int32_t &K, bool &saturated)
{
    saturated = (packed & detail::SAT_BIT_INV) == 0u;

    const uint16_t T_bits = static_cast<uint16_t>((packed & detail::T_MASK) >> detail::T_SHIFT);
    T                     = __half2float(__ushort_as_half_rocm(T_bits));

    const uint32_t neg_K = packed & detail::K_MASK;
    K                    = (neg_K == 0u) ? 0 : static_cast<int32_t>(detail::K_VALUE_COUNT - neg_K);
...
__device__ __forceinline__ DecodeForBatchResult decode_for_batch(uint32_t packed, int32_t batch_id)
{
    float stored_T;
    int32_t stored_K;
    bool stored_sat;
...
{
    const uint32_t neg_K = packed & detail::K_MASK;
    const int32_t K      = (neg_K == 0u) ? 0 : static_cast<int32_t>(detail::K_VALUE_COUNT - neg_K);
    assert(K >= 0 && K < detail::K_LIMIT);
    return K;
}
...
{
    const uint16_t T_bits = static_cast<uint16_t>((packed & detail::T_MASK) >> detail::T_SHIFT);
    const float T         = __half2float(__ushort_as_half_rocm(T_bits));
    assert(T >= 0.0f && T <= 1.0f);
    return T;
}
