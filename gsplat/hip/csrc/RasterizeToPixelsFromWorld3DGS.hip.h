...
#include "Common.h"
#include "Cameras.hip.h"
#include "ExternalDistortion.hip.h"
#include "Lidars.hip.h"
...
__device__ __forceinline__ PixelCoords compute_pixel_coords(
    ...
    const std::optional<RowOffsetStructuredSpinningLidarModelParametersExtDevice> &lidar_device_coeffs
)
{
    PixelCoords out;
...
__device__ __forceinline__ WorldRay compute_world_ray(
    ...
    const std::optional<RowOffsetStructuredSpinningLidarModelParametersExtDevice> &lidar_device_coeffs,
    const std::optional<extdist::BivariateWindshieldModelDeviceParams> &external_distortion_device_params
)
{
    WorldRay ray;
    ...
    else
    {
        ...
        {
            assert(rays != nullptr);
            // Note: use at least 3x64b loads instead of 6x32b
...
)
{
    ...
    // is statically true; the `if constexpr` lets hipcc elide it.
    // Without this fold, hipcc emits a redundant ISETP that
    ...
    {
        // Note: only support 1 camera for now so it is ok to abuse the index.
...
// follow is defined in `RasterizeCSR.hip.h`.
