...
// TRANSLATOR NOTE (ROCm limitation, accepted): the HIP shim for
// cooperative_groups::labeled_partition (gsplat/hip/include/cooperative_groups/reduce.h)
// is an identity stub -- thread_rank() always returns 0 and reduce() returns the
// value unreduced. On upstream CUDA this function groups threads by `elem_id`
// within a coalesced-threads set and combines their v_dir contributions in one
// shuffle-reduction before a single representative thread does the atomic add,
// cutting atomic contention. With the stub, every thread satisfies
// thread_rank() == 0 and reduce() is a no-op, so every thread instead does its
// own atomic add of its own (unreduced) contribution. The total accumulated
// sum is identical (this is just the pre-optimization behavior: one atomicAdd
// per thread instead of one per elem_id group), so this stays numerically
// correct -- it simply does not get the intended reduced-atomic-contention
// speedup on this backend. A real coalesced-group partition/reduction (e.g.
// via __match_any / ballot-based grouping) would be needed to recover it.
