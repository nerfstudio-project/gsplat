...
#include "Common.h"
#include "RasterizeContributingCommon.hip.h"
#include "RasterizeContributingCommonSparse.hip.h"
...
)
{
    ...
    auto launch_variant = [&]<uint32_t TILE_SIZE, uint32_t CTA_SIZE>()
    {
        ...
        rasterize_contributing_common_kernel<TILE_SIZE, CTA_SIZE, Accumulator>
            <<<grid, threads, shmem_size, at::hip::getCurrentHIPStream()>>>(
                n_isects,
    ...
    auto launch_variant = [&]<uint32_t TILE_SIZE, uint32_t CTA_SIZE>()
    {
        ...
        rasterize_contributing_common_sparse_kernel<TILE_SIZE, CTA_SIZE, Accumulator>
            <<<grid, threads, shmem_size, at::hip::getCurrentHIPStream()>>>(
                ...
                tile_offsets.data_ptr<int32_t>(),
                flatten_ids.data_ptr<int32_t>(),
                tile_pixel_mask.data_ptr<uint64_t>(),
                tile_pixel_cumsum.data_ptr<int64_t>(),
                pixel_map.data_ptr<int64_t>(),
                words,
                accum
            );
    };
