...
"""JIT build for the gsplat_scene_hip extension (Gaussian scene packing ops)."""
...
def _status_context(msg: str):
    ...
    try:
        ...
        print(
            f"gsplat_scene: HIP extension ready in {time.time() - tic:.2f}s",
            flush=True,
        )


def build_and_load_scene_hip():
    ...
    ctx = (
        _status_context(
            "gsplat_scene: compiling HIP extension "
    ...
    try:
        ...
        if module_exists and not build_params_changed:
            try:
                ...
                raise OSError(
                    "Failed to load cached gsplat_scene HIP extension after "
...
__all__ = ["build_and_load_scene_hip", "get_build_parameters"]
