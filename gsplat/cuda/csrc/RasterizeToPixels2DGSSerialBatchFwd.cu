...
)
{
    ...
    tile_offsets  += image_id * tile_height * tile_width;          // get the global offset of the tile w.r.t the image
    render_colors += image_id * image_height * image_width * CDIM; // get the global offset of the pixel w.r.t the image
    render_alphas += image_id * image_height * image_width;        // get the global offset of the pixel w.r.t the image
    last_ids      += image_id * image_height * image_width;        // get the global offset of the pixel w.r.t the image
...
)
{
    ...
    int64_t shmem_size
        = tile_size * tile_size * (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) + sizeof(vec3) + sizeof(vec3));
