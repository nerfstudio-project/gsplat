...
__global__ void rasterize_to_pixels_2dgs_bwd_kernel(
    ...
    const scalar_t *__restrict__ ray_transforms,  // transformation matrices that transforms xy-planes in pixel spaces into splat coordinates. [C, N, 3, 3] if packed is False, [nnz, channels] if packed is True.
                                           // This is (KWH)^{-1} in the paper (takes screen [x,y] and map to [u,v])
    const scalar_t *__restrict__ colors,      // [C, N, CDIM] or [nnz, CDIM]  // Gaussian colors or ND features.
    const scalar_t *__restrict__ normals,     // [C, N, 3] or [nnz, 3]                  // The normals in camera space.
    const scalar_t *__restrict__ opacities,   // [C, N] or [nnz]                        // Gaussian opacities that support per-view values.
    const scalar_t *__restrict__ backgrounds, // [C, CDIM]                         // Background colors on camera basis
    ...
    // fwd outputs
    const scalar_t *__restrict__ render_colors,    // [C, image_height, image_width,
                                            // CDIM]
    ...
    // grad outputs
    const scalar_t *__restrict__ v_render_colors,  // [C, image_height, image_width,     // RGB
                                            // CDIM]
    const scalar_t *__restrict__ v_render_alphas,  // [C, image_height, image_width, 1]  // total opacities.
    const scalar_t *__restrict__ v_render_normals, // [C, image_height, image_width, 3]  // camera space normals
    const scalar_t *__restrict__ v_render_distort, // [C, image_height, image_width, 1]  // mip-nerf 360 distorts
    const scalar_t *__restrict__ v_render_median,  // [C, image_height, image_width, 1]  // the median depth
