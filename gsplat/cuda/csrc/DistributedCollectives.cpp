...
namespace
{
    ...
    at::Tensor repeat_rank_offsets(
        const std::vector<int64_t> &offsets, const std::vector<int64_t> &counts, const at::TensorOptions &options
    )
    {
        TORCH_CHECK(offsets.size() == counts.size(), "repeat_rank_offsets requires offsets and counts of equal length");
    ...
    std::vector<int64_t> dense_scatter_input_splits(const std::vector<int64_t> &C_world, int64_t local_N)
    {
    ...
    std::vector<int64_t> dense_scatter_output_splits(int64_t local_C, const std::vector<int64_t> &N_world)
    {
        std::vector<int64_t> splits;
    ...
    at::Tensor reshape_dense_distributed_view(
        int64_t local_C, const at::Tensor &world_view, const std::vector<int64_t> &N_world
    )
    {
    ...
    std::vector<at::Tensor> gather_batched(
        const std::vector<at::Tensor> &tensors, int64_t world_size, const std::string &pg
    )
    {
        ...
        at::Tensor gathered            = all_gather_into_tensor(at::cat(flat, -1).contiguous(), world_size, pg);
    ...
    )
    {
        ...
        at::Tensor gathered              = all_to_all_single(
            at::cat(flat, -1).contiguous(), at::SymIntArrayRef(out_sym), at::SymIntArrayRef(in_sym), pg
        );
        std::vector<at::Tensor> chunks = gathered.split(feature_sizes, -1);
        std::vector<at::Tensor> out;
        out.reserve(tensors.size());
    ...
    std::vector<int64_t> all_gather_int(
        int64_t value, const at::Tensor &like, int64_t world_size, const std::string &pg
    )
    {
    ...
    std::vector<int64_t> all_to_all_int(
        const std::vector<int64_t> &values, const at::Tensor &like, int64_t world_size, const std::string &pg
    )
    {
        ...
        at::Tensor output = all_to_all_single(input, at::SymIntArrayRef(unit_sym), at::SymIntArrayRef(unit_sym), pg);
...
{
    std::vector<int64_t> N_world = all_gather_int(local_N, viewmats, world_size, process_group_name);
    std::vector<int64_t> C_world = all_gather_int(local_C, viewmats, world_size, process_group_name);
    for(int64_t C_i: C_world)
    {
        TORCH_CHECK(
            C_i == local_C,
    ...
    std::vector<at::Tensor> cameras = gather_batched({viewmats, Ks}, world_size, process_group_name);
...
)
{
    ...
    if(packed)
    {
        ...
        at::Tensor per_camera_counts                = at::bincount(projection.camera_ids, c10::nullopt, global_C);
        std::vector<int64_t> per_camera_counts_host = to_int64_vector(per_camera_counts);
        ...
        std::vector<int64_t> output_splits
            = all_to_all_int(send_splits, projection.camera_ids, world_size, process_group_name);

        out.radii
            = all_to_all_batched({projection.radii}, send_splits, output_splits, world_size, process_group_name)[0];
        std::vector<at::Tensor> payload
            = has_features ? all_to_all_batched(
                                 {projection.means2d,
                                  projection.depths,
                                  projection.conics,
                           ...
                           : all_to_all_batched(
                                 {projection.means2d, projection.depths, projection.conics, projection.opacities},
                                 send_splits,
                                 output_splits,
                                 world_size,
                                 process_group_name
        ...
        at::Tensor camera_offsets
            = repeat_rank_offsets(cumulative_offsets(C_world), send_splits, projection.camera_ids.options());
        at::Tensor gaussian_offsets
            = repeat_rank_offsets(cumulative_offsets(N_world), send_splits, projection.gaussian_ids.options());
        std::vector<at::Tensor> ids = all_to_all_batched(
            {projection.camera_ids - camera_offsets, projection.gaussian_ids + gaussian_offsets},
            send_splits,
            output_splits,
            world_size,
            process_group_name
    ...
    {
        std::vector<int64_t> input_splits  = dense_scatter_input_splits(C_world, local_N);
        std::vector<int64_t> output_splits = dense_scatter_output_splits(local_C, N_world);

        out.radii = reshape_dense_distributed_view(
            local_C,
            all_to_all_batched(
                {projection.radii.flatten(0, 1)}, input_splits, output_splits, world_size, process_group_name
            )[0],
            N_world
        );
        std::vector<at::Tensor> payload = has_features ? all_to_all_batched(
