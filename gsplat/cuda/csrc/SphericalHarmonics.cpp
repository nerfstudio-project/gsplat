...
at::Tensor spherical_harmonics_fwd_3dcs(
    ...
    launch_spherical_harmonics_fwd_kernel_3dcs(
        degrees_to_use, convex_points, dirs, coeffs, masks, colors
    );
    return colors; // [..., 3]
}
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> spherical_harmonics_bwd_3dcs(
    ...
    at::Tensor v_coeffs = at::zeros_like(coeffs);
    at::Tensor v_convex_points;
    at::Tensor v_dirs;
    if (compute_v_convex_points)
    {
        v_convex_points = at::zeros_like(convex_points);
    }
    if (compute_v_dirs)
    {
        v_dirs = at::zeros_like(dirs);
    ...
    uint32_t n_elements = N;
    uint32_t shmem_size = 0;
    launch_spherical_harmonics_bwd_kernel_3dcs(
        degrees_to_use,
        convex_points,
        dirs,
        coeffs,
    ...
    return std::make_tuple(v_convex_points, v_coeffs, v_dirs); // [..., K, 3], [..., 3]
}
