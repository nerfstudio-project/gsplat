...
__device__ __forceinline__ void mark_partials_meta_needs_batch_replay(
    ushort2 *__restrict__ partials_meta, int32_t slot, int32_t pixels_per_tile
)
{
    ...
    atomicOr(reinterpret_cast<unsigned int *>(partials_meta + slot * pixels_per_tile), BATCH_REPLAY_FLAG_WORD);
}
...
)
{
    ...
    const uint32_t tile_element_id = thread_y * TILE_SIZE + thread_x + p * CTA_SIZE;
...
__device__ __forceinline__ FwdShmemBatch fwd_unpack_shmem(int32_t *raw, uint32_t cta_size)
{
    FwdShmemBatch b;
    b.id_batch          = raw;
    b.xyz_opacity_batch = reinterpret_cast<vec4 *>(&b.id_batch[cta_size]);
    b.iscl_rot_batch    = reinterpret_cast<mat3 *>(&b.xyz_opacity_batch[cta_size]);
    b.scale_batch       = reinterpret_cast<vec3 *>(&b.iscl_rot_batch[cta_size]);
    b.normal_batch      = reinterpret_cast<vec3 *>(&b.scale_batch[cta_size]);
    return b;
}
...
)
{
    ...
    if(cta_sync_count<CTA_SIZE>(done_mask == ALL_DONE) >= static_cast<int32_t>(CTA_SIZE))
    {
        return;
    }
    ...
    (void)process_logical_batch_gaussians<
        CDIM,
        LOGICAL_BATCH,
        FETCH_SIZE,
        CTA_SIZE,
...
>
__global__ void
    __launch_bounds__(CTA_SIZE, min_blocks_for_cdim<CDIM, CTA_SIZE>()) rasterize_to_pixels_from_world_3dgs_fwd_partials_kernel(
        const uint32_t B,
        const uint32_t C,
        const uint32_t N,
    ...
    )
{
    ...
    assert(bid >= 0);
    const int32_t tile_linear
        = static_cast<int32_t>(find_tile_for_block(batch_offsets_csr, num_tiles_total, static_cast<uint32_t>(bid)));
    ...
    if(masks != nullptr && !masks[tile_id])
    {
        if(tid == 0u)
        {
            ...
            FwdPartialsMetaView<ushort2> summary_view(partials_meta, bid, pixels_per_tile, 0);
            summary_view.reset();
        }
        return;
    }
    ...
    const RollingShutterParameters rs_params(
        viewmats0 + image_index * 16, viewmats1 == nullptr ? nullptr : viewmats1 + image_index * 16
    );
...
#    pragma unroll
    ...
    FwdShmemBatch smem = fwd_unpack_shmem(reinterpret_cast<int32_t *>(s), CTA_SIZE);
    ...
    for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
    {
        ...
        if(pcs[p].inside && b == 0u)
        {
            if(!valid_pixel[p])
            {
                ...
                priming_state[pcs[p].pix_id] = static_cast<int32_t>(gsplat::priming::INVALID_RAY_PACKED);
            }
            if constexpr(ForBackward)
            {
                const uint32_t pix_y_in_tile   = thread_y + p * ROW_STRIDE;
                const int32_t pix_rank_in_tile = pix_y_in_tile * TILE_SIZE + thread_x;
                assert(tile_linear <= (INT32_MAX - pix_rank_in_tile) / pixels_per_tile);
                const int32_t compose_idx   = tile_linear * pixels_per_tile + pix_rank_in_tile;
                compose_c_stop[compose_idx] = valid_pixel[p] ? COMPOSE_C_STOP_NONE : COMPOSE_C_STOP_INVALID_RAY;
            }
        }
    }
...
#    pragma unroll
    for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
    {
        ...
        const unsigned int packed
            = cuda::atomic_ref<unsigned int, cuda::thread_scope_device>{*reinterpret_cast<unsigned int *>(
                                                                            &priming_state[pcs[p].pix_id]
                                                                        )}
                  .load(cuda::memory_order_relaxed);
        const gsplat::priming::DecodeForBatchResult priming_decision
            = gsplat::priming::decode_for_batch(packed, static_cast<int32_t>(b));
        ...
        else
        {
            ...
            T_init_used[p]             = priming_decision.T_init_used;
            transmittance_threshold[p] = TRANSMITTANCE_THRESHOLD / priming_decision.T_init_used;
        }
    ...
    process_batch_gaussians_fwd<
        ...
        ReturnNormals,
        UseHitDistance,
        ForBackward ? SaturationTPolicy::StorePostSaturationT : SaturationTPolicy::KeepPreAndCapturePostSaturationT,
        scalar_t
    >(b,
      range_start,
      range_end,
...
#    pragma unroll
    for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
    {
        ...
        const bool this_batch_saturated = valid_pixel[p] && (chain_saturated[p] || ((done_mask & (1u << p)) != 0u));
        const bool fwd_terminal_batch   = !ForBackward && this_batch_saturated && !chain_saturated[p];
        if(valid_pixel[p])
        {
...
#    pragma unroll
        ...
        if(valid_pixel[p])
        {
            ...
            {
                chain_walk_prod = fwd_terminal_batch ? saturating_T[p] : walk_prod;
            }
            const float T_after_for_chain = T_init_used[p] * chain_walk_prod;
            assert(T_after_for_chain >= 0.0f && T_after_for_chain <= 1.0f + 1e-5f);
            const float T_clamped = fminf(T_after_for_chain, 1.0f);
            assert(static_cast<int32_t>(b + 1u) < gsplat::priming::detail::K_LIMIT);
            const unsigned int packed_out
                = gsplat::priming::encode(T_clamped, static_cast<int32_t>(b + 1u), this_batch_saturated);
            atomicMin(reinterpret_cast<unsigned int *>(&priming_state[pcs[p].pix_id]), packed_out);
        }
    }
}
...
>
__global__ void
    __launch_bounds__(CTA_SIZE, min_blocks_for_cdim<CDIM, CTA_SIZE>()) rasterize_to_pixels_from_world_3dgs_fwd_batch_scan_kernel(
        const uint32_t B,
        const uint32_t C,
        const uint32_t N,
        const int64_t n_isects,
...
#    pragma unroll
    ...
    const uint32_t num_batches_this_tile
...
#    pragma unroll
    for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
    {
        ...
        if constexpr(!ForBackward)
        {
            const uint32_t packed = static_cast<uint32_t>(priming_state[pcs[p].pix_id]);
            initial_c_stop = gsplat::priming::is_invalid_ray(packed) ? decode_compose_c_stop(COMPOSE_C_STOP_INVALID_RAY)
        ...
        else
        {
            ...
            assert(tile_linear <= (INT32_MAX - pix_rank_in_tile) / pixels_per_tile);
            const int32_t compose_idx = tile_linear * pixels_per_tile + pix_rank_in_tile;
            initial_c_stop            = decode_compose_c_stop(compose_c_stop[compose_idx]);
            assert(
...
#    pragma unroll
        for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
        {
            ...
            const FwdPartialsMetaView<const ushort2> meta_view(partials_meta, slot, pixels_per_tile, pix_rank_in_tile);
            const float walk_prod     = inactive ? 1.0f : partial_slot.T();
            const bool terminal_batch = !ForBackward && !inactive && meta_view.isTerminalBatch();
            const bool fold_this = !inactive && (((T_cum[p] * walk_prod) > TRANSMITTANCE_THRESHOLD) || terminal_batch);
...
#    pragma unroll
                for(uint32_t k = 0; k < CDIM; ++k)
                {
                    ...
                    pix_cum[p][k]       = pix_cum[p][k] + T_cum[p] * pix_p_c;
                }
                if constexpr(ReturnNormals)
                {
...
#    pragma unroll
                ...
                else
                {
                    ...
                    FwdBatchReplayPreambleView<int2> preamble_view(
                        batch_replay_preamble, tile_linear, pixels_per_tile, pix_rank_in_tile
                    );
                    preamble_view.set(last_idx_global[p], n_acc_global[p]);
                    mark_partials_meta_needs_batch_replay(partials_meta, slot, pixels_per_tile);
                }
            }
        }
...
>
__global__ void
    __launch_bounds__(CTA_SIZE, min_blocks_for_cdim<CDIM, CTA_SIZE>()) rasterize_to_pixels_from_world_3dgs_fwd_batch_replay_kernel(
        const uint32_t B,
        const uint32_t C,
        const uint32_t N,
        const int64_t n_isects,
    ...
    )
{
    ...
    const int32_t bid              = bid_to_slot[block.group_index().x];
    assert(bid >= 0);
    const FwdPartialsMetaView<const ushort2> summary_view(partials_meta, bid, pixels_per_tile, 0);
    if(!summary_view.needsBatchReplay())
    {
        return;
    }
    const int32_t tile_linear
        = static_cast<int32_t>(find_tile_for_block(batch_offsets, num_tiles_total, static_cast<uint32_t>(bid)));
...
#    pragma unroll
    for(uint32_t p = 0; p < PIXELS_PER_THREAD; ++p)
    {
        ...
        assert(tile_linear <= (INT32_MAX - pix_rank_in_tile) / pixels_per_tile);
        const int32_t compose_idx = tile_linear * pixels_per_tile + pix_rank_in_tile;
        c_stop[p]                 = decode_compose_c_stop(compose_c_stop[compose_idx]);
        const bool engage         = c_stop[p] == batch_id && ((valid_mask & (1u << p)) != 0u);
        if(engage)
        {
...
)
{
    ...
    DeviceExternalDistortionParamsOpt external_distortion_device_params = cuda::std::nullopt;
    ...
    {
        TORCH_CHECK(
            camera_model == CameraModelType::LIDAR, "If lidar coeffs are given, the camera model must be lidar"
        );
                             ...
                             typename FwdOnlyT>()
    {
        ...
        TORCH_CHECK(fwd_batch_state.dim() == 3, "fwd_batch_state must be 3-D");
        TORCH_CHECK(
            fwd_batch_state.size(0) == total_batches
        ...
        TORCH_CHECK(partials_meta.scalar_type() == at::kUInt16, "partials_meta must be uint16");
        TORCH_CHECK(
        ...
        if(!fwd_only)
        {
            ...
            TORCH_CHECK(batch_replay_preamble.scalar_type() == at::kInt, "batch_replay_preamble must be int32");
            TORCH_CHECK(
        ...
        if constexpr(ForBackward)
        {
            const cudaError_t batch_replay_attr_status = cudaFuncSetAttribute(
        ...
        if(total_batches > 0)
        {
            const dim3 grid_partials = {static_cast<uint32_t>(total_batches), 1, 1};
            rasterize_to_pixels_from_world_3dgs_fwd_partials_kernel<
                CDIM,
                TILE_SIZE,
                CTA_SIZE,
        ...
        {
            // Re-derived locally (not captured) — MSVC rejects implicit capture of
            // an enclosing lambda's constexpr local as a "simple capture" (no
            // automatic storage duration). The enclosing lambda's own type template
            // parameters (ChannelsT, ReturnNormalsT, TileSizeT, FwdOnlyT) are always
            // visible here without capture, so re-deriving from them sidesteps it.
            constexpr uint32_t CDIM      = ChannelsT::value;
            constexpr bool ReturnNormals = static_cast<bool>(ReturnNormalsT::value);
            constexpr uint32_t TILE_SIZE = TileSizeT::value;
            constexpr uint32_t CTA_SIZE  = CtaSizeForTile<TILE_SIZE>::value;
        ...
        if constexpr(ForBackward)
        {
            if(total_batches > 0)
            {
                const dim3 grid_batch_replay = {static_cast<uint32_t>(total_batches), 1, 1};
                rasterize_to_pixels_from_world_3dgs_fwd_batch_replay_kernel<
                    CDIM,
                    TILE_SIZE,
                    CTA_SIZE,
