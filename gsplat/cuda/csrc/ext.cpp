...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    // jagged version
    m.def("persp_proj_jagged_fwd", &gsplat::persp_proj_jagged_fwd_tensor);
    m.def("persp_proj_jagged_bwd", &gsplat::persp_proj_jagged_bwd_tensor);
    m.def(
        "fully_fused_projection_jagged_fwd",
        &gsplat::fully_fused_projection_jagged_fwd_tensor
    );
    m.def(
        "fully_fused_projection_jagged_bwd",
        &gsplat::fully_fused_projection_jagged_bwd_tensor
    );
}