...
namespace seg_sort
{
    ...
    __device__ void MergePartition(
        int4 desc, int32_t *keys_a, int32_t *vals_a, int32_t *keys_b, int32_t *vals_b, int2 *smem_pairs
    )
    {
...
#pragma unroll
    ...
    __device__ void UpdateMergeQueue(
        const PersistentMergeArgs &args, const MergeWorkItem &work_item, const MergeTreeNode &node
    )
    {
        const int lane         = threadIdx.x & 31;
        ...
        if(parent >= 0)
        {
            ...
            parent_n_partitions
                = (parent_node.end - parent_node.begin + MERGE_PARTITION_SIZE - 1) / MERGE_PARTITION_SIZE;
        }
...
#pragma unroll 1
    ...
    __global__ void __launch_bounds__(MERGE_CTA_SIZE, MERGE_MIN_BLOCKS) MergePersistentKernel(
        const PersistentMergeArgs args
    )
    {
    ...
    template<int CTA_SIZE>
    __device__ void WriteOverflowBlocks(
        const BlockSortArgs &args, int seg_start, int seg_end, int seg_size, int n_merge_passes, int32_t *smem
    )
    {
...
#pragma unroll 1
    ...
    // CTA-cooperative: must be called by all CTA_SIZE threads.
    template<int CTA_SIZE>
    __device__ void WriteMergeTree(
        const BlockSortArgs &args, int seg_start, int seg_end, int seg_size, int n_merge_passes, int32_t *smem
    )
    {
        constexpr int CAPACITY             = CTA_SIZE * BLOCK_SORT_ITEMS_PER_THREAD;
...
#pragma unroll 1
        for(int i = threadIdx.x; i < leaf_partition_count; i += CTA_SIZE)
        {
            ...
            args.work_items[*smem_queue_base + i]
                = {.node_idx = tree_base_idx + leaf_node_idx, .partition_idx = partition_idx};
        }
...
#pragma unroll 1
    ...
    template<int CTA_SIZE>
    __device__ void BlockRadixSortSegment(
        BlockSortSmem<CTA_SIZE> &smem,
    ...
    __global__ void __launch_bounds__(CTA_SIZE, 512 / CTA_SIZE) BlockSortKernel(const BlockSortArgs args)
    {
        ...
        if constexpr(IS_PERSISTENT)
        {
            ...
            while(true)
            {
                ...
                BlockRadixSortSegment<CTA_SIZE>(
                    smem, args.keys_in, args.values_in, out_keys, out_vals, block_begin, block_size
                );
        ...
        else
        {
            ...
            BlockRadixSortSegment<CTA_SIZE>(
                smem, args.keys_in, args.values_in, out_keys, out_vals, seg_start, seg_size
            );
        }
    }
} // namespace seg_sort
...
size_t SegmentedSortSetup(SegmentedSortState &s, int32_t n_segments, int32_t n_items, void *scratch)
{
    ...
    auto alignUp = [](size_t x, size_t a) -> size_t
    {
        return (x + a - 1) & ~(a - 1);
    };
...
int SegmentedSortAsync(
    const SegmentedSortState &st,
    const int32_t *d_offsets,
    int32_t *const *d_keys,
...
)
{
    ...
    const BlockSortArgs cascade_args
    ...
    const BlockSortArgs cascade_epilog_args
        = {.n_segments        = st.n_segments,
           .n_items           = st.n_items,
           .offsets           = d_offsets,
           .keys_in           = d_keys[0],
    ...
    const BlockSortArgs overflow_args
        = {.n_items         = st.n_items,
           .keys_in         = d_keys[0],
           .values_in       = d_values[0],
           .values_out      = d_values[0],
    ...
    launchPdl(
        BlockSortKernel<BLOCK_SORT_T3_CTA_SIZE, BLOCK_SORT_T3_MIN_SIZE, true>,
        BLOCK_SORT_T3_CTA_SIZE,
        cascade_epilog_args
    );
