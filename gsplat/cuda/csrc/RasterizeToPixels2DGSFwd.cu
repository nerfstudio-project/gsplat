...
__global__ void rasterize_to_pixels_2dgs_fwd_kernel(
    ...
    scalar_t *__restrict__ render_distort, // [C, image_height, image_width, 1]  // Stores the per-pixel distortion error proposed in Mip-NeRF 360.
    scalar_t *__restrict__ render_median,  // [C, image_height, image_width, 1]  // Stores the median depth contribution for each pixel "set to the depth of the Gaussian that brings the accumulated opacity over 0.5."
    ...
    render_colors += camera_id * image_height * image_width * CDIM;  // get the global offset of the pixel w.r.t the camera
