...
__global__ void
__launch_bounds__(CTA_SIZE) rasterize_contributing_common_kernel(
    ...
    static_assert(
        ...
        "TILE_SIZE must be a power of 2 (TILE_MASK/TILE_SHIFT rely on this)"
    );
    static_assert(
        PIXELS_PER_THREAD > 0,
...
#pragma unroll
    ...
    const int32_t range_end =
        (image_id == static_cast<int32_t>(gridDim.x) - 1) &&
...
#pragma unroll 1
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        if (idx < range_end) {
            const int32_t g =
                flatten_ids[idx]; // flatten index in [I * N] or [nnz]
            id_batch[tid] = g;
        ...
        const uint32_t batch_size =
            min(BATCH_SIZE, static_cast<uint32_t>(range_end - batch_start));
