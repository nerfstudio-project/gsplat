...
)
{
    ...
    // Note: Lidar camera model is no longer supported through the generic create() method.
    // Use PyLidarCameraModel constructor directly with all required parameters.
    else
    {
...
/**
 * @brief Constructor kernel for Lidar cameras
 * @param cameras Output tensor view for camera models
 * @param width Image width (n_columns)
 * @param height Image height (n_rows)
 * @param fov_elevation_min Minimum elevation angle (radians)
 * @param fov_elevation_max Maximum elevation angle (radians)
 * @param fov_azimuth_min Minimum azimuth angle (radians)
 * @param fov_azimuth_max Maximum azimuth angle (radians)
 * @param spin_direction Spinning direction of the lidar
 * @param rs_type Rolling shutter type
 * @param angle_to_column_map_ptr Device pointer to angle-to-column lookup table (optional)
 * @param angle_to_column_map_resolution_factor Upsampling factor for lookup grid
 * @param map_resolution_az Grid cell size in azimuth (radians)
 * @param map_resolution_el Grid cell size in elevation (radians)
 */
__global__ void construct_lidar_cameras_kernel(
    TensorView<LidarCameraModel, CAMERA> cameras,
    int width, int height,
    float fov_elevation_min, float fov_elevation_max,
    float fov_azimuth_min, float fov_azimuth_max,
    SpinningDirection spin_direction,
    ShutterType rs_type,
    const int* angle_to_column_map_ptr,
    int angle_to_column_map_resolution_factor,
    float map_resolution_az, float map_resolution_el
)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= cameras.shape(0))
    ...
    LidarCameraModel::Parameters params;
    params.resolution = {static_cast<uint32_t>(width), static_cast<uint32_t>(height)};
    params.shutter_type = rs_type;

    // Set lidar-specific parameters
    params.lidar.n_rows = height;
    params.lidar.n_columns = width;

    // Set FOV ranges as tuples (min, max)
    params.lidar.fov_elevation_range = std::make_tuple(fov_elevation_min, fov_elevation_max);
    params.lidar.fov_azimuth_range = std::make_tuple(fov_azimuth_min, fov_azimuth_max);
    params.lidar.spin_direction = spin_direction;

    // Set optional VREN-style lookup tables (not used yet, set to nullptr)
    params.lidar.row_elevations = nullptr;
    params.lidar.column_azimuths = nullptr;
    params.lidar.row_azimuth_offsets = nullptr;

    // Set rolling shutter timing parameters (angleToColumnMap)
    params.lidar.angle_to_column_map = angle_to_column_map_ptr;
    params.lidar.angle_to_column_map_resolution_factor = angle_to_column_map_resolution_factor;
    params.lidar.map_resolution = {map_resolution_az, map_resolution_el};

    // Create the camera with initialized parameters
    new (&cameras(idx)) LidarCameraModel(params);
}

PyLidarCameraModel::PyLidarCameraModel(
    int width, int height,
...
) : PyBaseCameraModel(1, width, height, rs_type,
                      // Lidar doesn't use traditional focal lengths, but provide a dummy value
                      // Determine device from input tensors (prefer angle_to_column_map, fallback to CUDA)
                      torch::tensor({{1.0f, 1.0f}}, torch::dtype(torch::kFloat32).device(
                          (angle_to_column_map.defined() && angle_to_column_map.numel() > 0)
                              ? angle_to_column_map.device()
                              : torch::kCUDA)),
                      torch::tensor({{width / 2.0f, height / 2.0f}}, torch::dtype(torch::kFloat32).device(
                          (angle_to_column_map.defined() && angle_to_column_map.numel() > 0)
                              ? angle_to_column_map.device()
                              : torch::kCUDA))),
    // Store tensors as members to keep data alive
    angle_to_column_map_(angle_to_column_map),
    row_elevations_(row_elevations),
    column_azimuths_(column_azimuths),
    row_azimuth_offsets_(row_azimuth_offsets)
{
    // This API supports both VREN preprocessing parameters (row_elevations, row_azimuth_offsets,
    // column_azimuths) and precomputed acceleration structures (angle_to_column_map).
    // All tensor parameters are optional (can be empty tensors).

    // Handle angle_to_column_map tensor (used for rolling shutter timing)
    const int* angle_to_column_map_ptr = nullptr;
    if (angle_to_column_map_.defined() && angle_to_column_map_.numel() > 0) {
        TORCH_CHECK(angle_to_column_map_.is_cuda(), "angle_to_column_map must be a CUDA tensor");
        TORCH_CHECK(angle_to_column_map_.dtype() == torch::kInt32, "angle_to_column_map must be int32");
        TORCH_CHECK(angle_to_column_map_.dim() == 1, "angle_to_column_map must be 1D tensor (row-major order)");

        int expected_n_pts_vert = height * angle_to_column_map_resolution_factor;
        int expected_n_pts_horiz = width * angle_to_column_map_resolution_factor;
        int expected_size = expected_n_pts_vert * expected_n_pts_horiz;

        TORCH_CHECK(angle_to_column_map_.size(0) == expected_size,
                    "angle_to_column_map size must be (n_rows * n_columns * resolution_factor^2) = ",
                    expected_size, " but got ", angle_to_column_map_.size(0));

        // Use data pointer from stored member tensor (lifetime guaranteed)
        angle_to_column_map_ptr = angle_to_column_map_.data_ptr<int>();
    }

    // TODO: Handle row_elevations, column_azimuths, row_azimuth_offsets for future VREN-style preprocessing
    // For now, these are accepted but not used (can compute angle_to_column_map from them in the future)
    if (row_elevations_.defined() && row_elevations_.numel() > 0) {
        TORCH_CHECK(row_elevations_.is_cuda(), "row_elevations must be a CUDA tensor");
        TORCH_CHECK(row_elevations_.dtype() == torch::kFloat32, "row_elevations must be float32");
        TORCH_CHECK(row_elevations_.dim() == 1 && row_elevations_.size(0) == height,
                    "row_elevations must be 1D tensor of size [n_rows]");
    }
    if (column_azimuths_.defined() && column_azimuths_.numel() > 0) {
        TORCH_CHECK(column_azimuths_.is_cuda(), "column_azimuths must be a CUDA tensor");
        TORCH_CHECK(column_azimuths_.dtype() == torch::kFloat32, "column_azimuths must be float32");
        TORCH_CHECK(column_azimuths_.dim() == 1 && column_azimuths_.size(0) == width,
                    "column_azimuths must be 1D tensor of size [n_columns]");
    }
    if (row_azimuth_offsets_.defined() && row_azimuth_offsets_.numel() > 0) {
        TORCH_CHECK(row_azimuth_offsets_.is_cuda(), "row_azimuth_offsets must be a CUDA tensor");
        TORCH_CHECK(row_azimuth_offsets_.dtype() == torch::kFloat32, "row_azimuth_offsets must be float32");
        TORCH_CHECK(row_azimuth_offsets_.dim() == 1 && row_azimuth_offsets_.size(0) == height,
                    "row_azimuth_offsets must be 1D tensor of size [n_rows]");
    }

    // Convert spin direction
    SpinningDirection spin_direction = spin_clockwise ? SpinningDirection::CLOCKWISE : SpinningDirection::COUNTER_CLOCKWISE;

    // Extract FOV range values
    float fov_elevation_min = std::get<0>(fov_elevation_range);
    float fov_elevation_max = std::get<1>(fov_elevation_range);
    float fov_azimuth_min = std::get<0>(fov_azimuth_range);
    float fov_azimuth_max = std::get<1>(fov_azimuth_range);

    // Get the CUDA stream from the device that owns the camera buffers
    // Prefer angle_to_column_map device if available, otherwise default to device 0
    int device_index = (angle_to_column_map.defined() && angle_to_column_map.numel() > 0)
                        ? angle_to_column_map.device().index()
                        : 0;
    cudaStream_t stream = at::cuda::getStreamFromPool(false, device_index);
    ...
    construct_lidar_cameras_kernel<<<blocks, threads, 0, stream>>>(
        make_tensor_view<CAMERA>(this->dev_cameras(), {m_num_cameras}, {1}, "cameras"),
        m_width, m_height,
        fov_elevation_min, fov_elevation_max,
        fov_azimuth_min, fov_azimuth_max,
        spin_direction, m_rs_type,
        angle_to_column_map_ptr, angle_to_column_map_resolution_factor,
        map_resolution[0], map_resolution[1]
    );

    CUDA_CHECK(cudaGetLastError());
}

// ========== Lidar-specific overrides for flexible batch handling ==========

std::tuple<torch::Tensor, torch::Tensor> PyLidarCameraModel::camera_ray_to_image_point(
    const torch::Tensor& camera_ray,
    float margin_factor) const
{
    torch::TensorOptions opt = torch::TensorOptions()
    ...
    // Save original shape for output reshaping
    std::vector<int64_t> original_shape = camera_ray.sizes().vec();

    // Reshape input to [batch_cameras, n_rays, 3] by flattening leading dimensions
    // This allows users to pass arbitrary batch shapes like [10, 10, 3]
    torch::Tensor camera_ray_reshaped = camera_ray.reshape({m_num_cameras, -1, 3});
    int rays_per_camera = camera_ray_reshaped.size(1);

    // Create output tensors in flattened shape
    torch::Tensor image_points_flat = torch::empty({m_num_cameras, rays_per_camera, 2}, opt);
    torch::Tensor valid_result_flat = torch::empty(
        {m_num_cameras, rays_per_camera},
        opt.dtype(torch::kBool).device(camera_ray.device())
    );
    ...
    camera_ray_to_image_point_kernel<<<blocks, threads, 0, stream>>>(
        make_tensor_view<CAMERA>(this->const_dev_cameras(), {m_num_cameras}, {1}, "cameras"),
        make_tensor_view<const float, CAMERA, RAY, 3>(camera_ray_reshaped, m_num_cameras, "camera_ray"),
        make_tensor_view<float, CAMERA, RAY, 2>(image_points_flat, m_num_cameras, "image_points"),
        make_tensor_view<bool, CAMERA, RAY>(valid_result_flat, m_num_cameras, "valid_result"),
        margin_factor
    );
    CUDA_CHECK(cudaGetLastError());

    // Reshape outputs back to original batch shape
    original_shape.back() = 2;
    torch::Tensor image_points = image_points_flat.reshape(original_shape);

    original_shape.pop_back();
    torch::Tensor valid_result = valid_result_flat.reshape(original_shape);

    return std::make_tuple(image_points, valid_result);
}

std::tuple<torch::Tensor, torch::Tensor> PyLidarCameraModel::image_point_to_camera_ray(
    const torch::Tensor& image_points) const
{
    torch::TensorOptions opt = torch::TensorOptions()
        .dtype(torch::kFloat32).device(image_points.device());
    ...
    // Save original shape for output reshaping
    std::vector<int64_t> original_shape = image_points.sizes().vec();

    // Reshape input to [batch_cameras, n_points, 2] by flattening leading dimensions
    // This allows users to pass arbitrary batch shapes like [10, 10, 2]
    torch::Tensor image_points_reshaped = image_points.reshape({m_num_cameras, -1, 2});
    int points_per_camera = image_points_reshaped.size(1);

    // Create output tensors in flattened shape
    torch::Tensor camera_rays_flat = torch::empty({m_num_cameras, points_per_camera, 3}, opt);
    torch::Tensor valid_result_flat = torch::empty({m_num_cameras, points_per_camera}, opt.dtype(torch::kBool).device(image_points.device()));
    ...
    image_point_to_camera_ray_kernel<<<blocks, threads, 0, stream>>>(
        make_tensor_view<CAMERA>(this->const_dev_cameras(), {m_num_cameras}, {1}, "cameras"),
        make_tensor_view<const float, CAMERA, POINT, 2>(image_points_reshaped, m_num_cameras, "image_points"),
        make_tensor_view<float, CAMERA, POINT, 3>(camera_rays_flat, m_num_cameras, "camera_rays"),
        make_tensor_view<bool, CAMERA, POINT>(valid_result_flat, m_num_cameras, "valid_result")
    );
    CUDA_CHECK(cudaGetLastError());

    // Reshape outputs back to original batch shape
    original_shape.back() = 3;
    torch::Tensor camera_rays = camera_rays_flat.reshape(original_shape);
