...
#    pragma unroll
    ...
    const uint32_t num_logical_batches = (range_end - range_start + LOGICAL_BATCH - 1) / LOGICAL_BATCH;
    ...
    const uint32_t tile_linear = iid * grid_height * grid_width + tile_id;
    ...
    batch_base_slot = persist_batches ? static_cast<int64_t>(batch_offsets_csr[tile_linear]) : 0;
    ...
    {
        assert(
            static_cast<uint32_t>(batch_offsets_csr[tile_linear + 1] - batch_offsets_csr[tile_linear])
            == num_logical_batches
        );
    }
#    ifndef NDEBUG
...
#    endif
    ...
    vec3 *normal_batch      = reinterpret_cast<vec3 *>(&scale_batch[FETCH_SIZE]);       // [FETCH_SIZE]
...
#    pragma unroll 1
    for(uint32_t lb = 0; lb < num_logical_batches; ++lb)
    {
        ...
        if(persist_batches)
        {
            persist_batch_state<CDIM, PIXELS_PER_THREAD, CTA_SIZE, ReturnNormals>(
                lb, batch_base_slot, pixels_per_tile, tid, T, pix_out, normal_out, fwd_batch_state
            );
        }
        ...
        if(all_done)
        {
            ...
            if(persist_batches)
            {
                ...
                {
                    persist_batch_state<CDIM, PIXELS_PER_THREAD, CTA_SIZE, ReturnNormals>(
                        lbb, batch_base_slot, pixels_per_tile, tid, T, pix_out, normal_out, fwd_batch_state
                    );
                }
            }
            break;
