...
/****************************************************************************
 * Fused Projection Forward Pass
 ****************************************************************************/

__global__ void proj_fused_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
...
void proj_fused_fwd_launcher(
    ...
    proj_fused_fwd_kernel<<<n_blocks_linear(n_elements), N_THREADS, shmem_size, stream>>>(
        C,
        N,
        means,
        covars,
...
/****************************************************************************
 * Fused Projection Backward Pass
 ****************************************************************************/


__global__ void proj_fused_bwd_kernel(
    // fwd inputs
    const uint32_t C,
    const uint32_t N,
    const float *__restrict__ means,    // [N, 3]
...
void proj_fused_bwd_launcher(
    ...
    proj_fused_bwd_kernel<<<n_blocks_linear(n_elements), N_THREADS, shmem_size, stream>>>(
        C,
        N,
        means,
        covars,
