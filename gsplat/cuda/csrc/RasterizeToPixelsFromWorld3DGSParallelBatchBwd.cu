...
)
{
    ...
    auto batches_per_tile_t = at::empty({static_cast<int64_t>(num_tiles)}, int_opts);
    if(num_tiles > 0)
    {
    ...
    auto batch_offsets_t    = at::cat({at::zeros({1}, int_opts), batch_offsets_tail});
    const int64_t total_batches
        = num_tiles == 0 ? int64_t{0}
                         : static_cast<int64_t>(batch_offsets_t[static_cast<int64_t>(num_tiles)].item<int32_t>());
...
)
{
    ...
    for(int32_t batch = 0; batch < count; ++batch)
    {
        ...
        const uint64_t key
            = (static_cast<uint64_t>(static_cast<uint32_t>(batch)) << bit_width_tile) | static_cast<uint64_t>(tile);
...
{
    auto int_opts          = dummy_options.dtype(at::kInt);
    at::Tensor bid_to_slot = at::empty({static_cast<int64_t>(total_batches)}, int_opts);

    const uint32_t num_tiles = static_cast<uint32_t>(batches_per_tile.size(0));
    if(total_batches == 0 || num_tiles == 0)
    {
        return bid_to_slot;
    }

    const uint32_t bit_width_tile  = ceil_log2_u64(static_cast<uint64_t>(num_tiles) + 1u);
    const uint32_t bit_width_round = ceil_log2_u64(static_cast<uint64_t>(total_batches) + 1u);
    const uint32_t total_bits      = bit_width_tile + bit_width_round;

    auto long_opts         = dummy_options.dtype(at::kLong);
    at::Tensor sort_keys   = at::empty({static_cast<int64_t>(total_batches)}, long_opts);
    at::Tensor sorted_keys = at::empty({static_cast<int64_t>(total_batches)}, long_opts);
    at::Tensor sort_values = at::empty({static_cast<int64_t>(total_batches)}, int_opts);
...
__global__ void rasterize_gradient_bwd_kernel(
    ...
    const scalar_t *__restrict__ v_render_colors,  // [B, C, image_height,
                                                   // image_width, CDIM]
    const scalar_t *__restrict__ v_render_alphas,  // [B, C, image_height, image_width, 1]
    const scalar_t *__restrict__ v_render_normals, // [B, C, image_height, image_width, 3] optional
    // grad inputs
...
)
{
    ...
    assert(tile_linear <= (INT32_MAX - thread_rank) / pixels_per_tile);
    const int32_t compose_idx = tile_linear * pixels_per_tile + thread_rank;
    const int32_t c_stop      = (has_compose_stop && inside) ? decode_compose_c_stop(compose_c_stop[compose_idx])
                                                             : decode_compose_c_stop(COMPOSE_C_STOP_NONE);
    assert(
        !has_compose_stop || !inside || ray.valid_flag || c_stop == decode_compose_c_stop(COMPOSE_C_STOP_INVALID_RAY)
    );
    const bool sat_flag     = c_stop >= 0;
    const bool pixel_valid  = pix_valid_raw && !(sat_flag && batch_id > c_stop);
...
#pragma unroll
    ...
    const FwdBatchSlotView<CDIM, ReturnNormals, const scalar_t> boundary_slot_view(
        fwd_batch_state, bid, pixels_per_tile, thread_rank
    );

    const int32_t final_slot = (sat_flag && pixel_valid) ? batch_offsets[tile_linear] + c_stop : terminal_slot;
    const FwdBatchSlotView<CDIM, ReturnNormals, const scalar_t> final_slot_view(
        fwd_batch_state, final_slot, pixels_per_tile, thread_rank
    );
    const bool final_matches_boundary = pixel_valid && (final_slot == bid);
...
#pragma unroll
    ...
    if constexpr(ReturnNormals)
    {
        if(pixel_valid && !final_matches_boundary)
        {
            ...
            normal_accum_dot
                = normal_delta.x * v_render_n.x + normal_delta.y * v_render_n.y + normal_delta.z * v_render_n.z;
        }
    }
...
#pragma unroll
    ...
    vec3 *scale_batch       = reinterpret_cast<vec3 *>(&xyz_opacity_batch[block_size]);
    vec4 *quat_batch        = reinterpret_cast<vec4 *>(&scale_batch[block_size]);
...
#pragma unroll
        ...
        for(uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t)
        {
            ...
            if(valid)
            {
                ...
                S          = mat3(1.0f / scale[0], 0.f, 0.f, 0.f, 1.0f / scale[1], 0.f, 0.f, 0.f, 1.0f / scale[2]);
...
#pragma unroll
                ...
                float v_alpha = rgb_render_dot * T - render_accum_dot * ra + T_final * ra * v_alpha_ind_coeff;
...
)
{
    ...
    uint32_t B           = static_cast<uint32_t>(c10::multiply_integers(means.sizes().slice(0, means.dim() - 2)));
    ...
    // every bwd.
    TORCH_CHECK(batches_per_tile.numel() == static_cast<int64_t>(num_tiles), "batches_per_tile has wrong size");
    TORCH_CHECK(batch_offsets.numel() == static_cast<int64_t>(num_tiles) + 1, "batch_offsets has wrong size");

    const bool return_normals = v_render_normals.has_value();
    auto launch_kernel        = [&]<typename ChannelsT, typename ReturnNormalsT, typename UseHitDistanceT>()
    {
        ...
        TORCH_CHECK(fwd_batch_state.dim() == 3, "fwd_batch_state must be 3-D (got ", fwd_batch_state.dim(), "-D)");
        TORCH_CHECK(
            ...
            ", ",
            fwd_batch_state.size(1),
            ", ",
            fwd_batch_state.size(2),
            "], expected [",
            total_batches,
            ", ",
            state_dim,
        ...
        if(use_compose_stop)
        {
            TORCH_CHECK(compose_c_stop.dim() == 2, "compose_c_stop must be 2-D");
            TORCH_CHECK(compose_c_stop.scalar_type() == at::kUInt16, "compose_c_stop must be uint16");
            TORCH_CHECK(
                compose_c_stop.size(0) == static_cast<int64_t>(num_tiles) && compose_c_stop.size(1) == pixels_per_tile,
        ...
        if(cudaFuncSetAttribute(
           ...
           != cudaSuccess)
        {
            AT_ERROR("Failed to set shmem for gradient kernel (", shmem_size, " bytes).");
        }
        rasterize_gradient_bwd_kernel<CDIM, ReturnNormals, UseHitDistance, float>
            <<<grad_grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
                B,
                C,
                N,
                n_isects,
