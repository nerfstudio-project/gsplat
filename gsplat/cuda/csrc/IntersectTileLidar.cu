...
__device__ int cdf_region_sum(
    const int32_t *raycdf, int raycdf_stride, int range_min_el, int range_max_el, int range_min_az, int range_max_az
)
{
...
__device__ bool has_any_rays_in_tile(
...
namespace
{
    ...
    inline __device__ int sample_tile_el(int dense_el, const int *cdf_elevation, int elevation_cdf_resolution)
    {
        assert(0 <= dense_el);
...
)
{
    ...
    const int tile_max_el
        = min(sample_tile_el(max_dense_el - 1, lidar.cdf_elevation, raycdf_size_el) + 1, (int)lidar.n_bins_elevation);
    ...
    const bool has_raysA = has_any_rays_in_tile(
        lidar, raycdf_size_el, raycdf_size_az, min_dense_el, max_dense_el, begA_dense, endA_dense
    );
    ...
    if(underflows || overflows)
    {
        ...
        const float endB_pix_az
            = min(max(underflows ? full_circle_pix : (end_az - full_circle_pix), 0.f), fov_span_pix_az);
        ...
        has_raysB            = has_any_rays_in_tile(
            lidar, raycdf_size_el, raycdf_size_az, min_dense_el, max_dense_el, begB_dense, endB_dense
        );
        if(has_raysB)
        {
    ...
    else
    {
        ...
        if(has_raysA && has_raysB && az_ranges[1][0] < az_ranges[0][1] && az_ranges[0][0] < az_ranges[1][1])
        {
            az_ranges[0][0] = 0;
    ...
    if(first_pass)
    {
        const int az_span = max(az_ranges[0][1] - az_ranges[0][0], 0) + max(az_ranges[1][1] - az_ranges[1][0], 0);
    ...
    assert(depth_f >= 0.f);
    const int64_t depth_id_enc = static_cast<int64_t>(__float_as_uint(depth_f));
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]()
        {
            intersect_tile_lidar_kernel<scalar_t><<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
                packed,
                I,
                N,
                nnz,
