...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    // if render distort maps, the last channel of render_colors should be depth
    float *__restrict__ render_distloss, // [C, image_height, image_width, 1] optional
    ...
    if (render_distloss != nullptr) {
        render_distloss += camera_id * image_height * image_width;
    }
...
rasterize_to_pixels_fwd_tensor(
    ...
    case 1:
        ...
        rasterize_to_pixels_fwd_kernel<1><<<blocks, threads, shared_mem, stream>>>(
            ...
            distloss ? distortions.data_ptr<float>() : nullptr,
...
__global__ void rasterize_to_pixels_bwd_kernel(
    ...
    const float
        *__restrict__ v_render_distloss, // [C, image_height, image_width, 1] optional
    // grad inputs
    ...
    if (v_render_distloss != nullptr) {
        v_render_distloss += camera_id * image_height * image_width;
    }
...
rasterize_to_pixels_bwd_tensor(
    ...
    if (v_render_distloss.has_value()) {
        CHECK_INPUT(v_render_distloss.value());
    }
    ...
    if (n_isects) {
        ...
        case 1:
            ...
            rasterize_to_pixels_bwd_kernel<1><<<blocks, threads, shared_mem, stream>>>(
                ...
                v_render_distloss.has_value()
                    ? v_render_distloss.value().data_ptr<float>()
                    : nullptr,
