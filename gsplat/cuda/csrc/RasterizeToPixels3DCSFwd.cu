...
__global__ void rasterize_to_pixels_3dcs_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    const scalar_t *__restrict__ offsets,                          // [C, total_nb_points, 1]
    ...
    const uint32_t num_points_per_convex,                          // [C]
    const int32_t *__restrict__ cumsum_of_points_per_convex,       // [C]
...
#pragma unroll
    ...
    scalar_t *normals_batch = (scalar_t *)&conic_batch[block_size];
...
#pragma unrol
            ...
            float phi_x = depths_batch[t] * delta_batch[t]*max_val + __logf(sum_exp);
            float Cx = 1.0f / (1.0f + __expf(depths_batch[t] * sigma_batch[t] * phi_x));

            float alpha = min(0.999f, opac * Cx);
...
void launch_rasterize_to_pixels_3dcs_fwd_kernel(
    ...
    int64_t shmem_size =
        tile_size * tile_size *
        (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) + sizeof(float)*MAX_NB_POINTS*2
        + sizeof(float)*MAX_NB_POINTS + sizeof(int32_t) + sizeof(float) + sizeof(float) + sizeof(float));

    // TODO: an optimization can be done by passing the actual number of
    // channels into the kernel functions and avoid necessary global memory
    // writes. This requires moving the channel padding from python to C side.
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_3dcs_fwd_kernel<CDIM, float>,
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_3dcs_fwd_kernel<CDIM>(            \
