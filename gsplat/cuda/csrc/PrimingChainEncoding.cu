...
namespace
{
    ...
    )
    {
        ...
        const gsplat::priming::DecodeForBatchResult decoded
            = gsplat::priming::decode_for_batch(static_cast<uint32_t>(packed[idx]), batch_ids[idx]);
...
)
{
    ...
    TORCH_CHECK(packed.device() == batch_ids.device(), "packed and batch_ids must be on the same CUDA device");
    TORCH_CHECK(packed.scalar_type() == at::kInt, "packed must have dtype int32");
    TORCH_CHECK(batch_ids.scalar_type() == at::kInt, "batch_ids must have dtype int32");
    TORCH_CHECK(packed.sizes() == batch_ids.sizes(), "packed and batch_ids must have identical shapes");
    ...
    at::Tensor T_init_used      = at::empty(packed.sizes(), packed.options().dtype(at::kFloat));
    at::Tensor stored_K         = at::empty_like(batch_ids);
    at::Tensor stored_sat       = at::empty(packed.sizes(), packed.options().dtype(at::kBool));
    at::Tensor chain_saturated  = at::empty(packed.sizes(), packed.options().dtype(at::kBool));
    at::Tensor use_stored_state = at::empty(packed.sizes(), packed.options().dtype(at::kBool));
