...
__global__ void rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    const bool packed,
    // fwd inputs
    const vec2 *__restrict__ means2d,         // [B, C, N, 2] or [nnz, 2]
    const vec3 *__restrict__ conics,          // [B, C, N, 3] or [nnz, 3]
    ...
    vec3 *__restrict__ v_conics,       // [B, C, N, 3] or [nnz, 3]
