...
)
{
    ...
    int64_t shmem_size = tile_size * tile_size * (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3));

    // TODO: an optimization can be done by passing the actual number of
    // channels into the kernel functions and avoid necessary global memory
    // writes. This requires moving the channel padding from python to C side.
    if(cudaFuncSetAttribute(
           rasterize_to_indices_3dgs_kernel<float>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size
       )
       != cudaSuccess)
    {
        AT_ERROR(
    ...
    rasterize_to_indices_3dgs_kernel<float><<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
        range_start,
        range_end,
        I,
        N,
