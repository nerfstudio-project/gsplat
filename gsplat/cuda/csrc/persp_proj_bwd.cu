...
template <typename T>
__global__ void persp_proj_bwd_kernel(const uint32_t C, const uint32_t N,
                                      const T *__restrict__ means,  // [C, N, 3]
                                      const T *__restrict__ covars, // [C, N, 3, 3]
                                      const T *__restrict__ Ks,     // [C, 3, 3]
                                      const uint32_t width, const uint32_t height,
                                      const T *__restrict__ v_means2d,  // [C, N, 3]
                                      const T *__restrict__ v_covars2d, // [C, N, 3, 3]
    ...
    const vec3<OpT> v_mean2d = glm::make_vec3(v_means2d);
    const mat3<OpT> v_covar2d = glm::make_mat3(v_covars2d);
    persp_proj_vjp<OpT>(mean, covar, fx, fy, cx, cy, width, height,
                        glm::transpose(v_covar2d), v_mean2d, v_mean, v_covar);
...
persp_proj_bwd_tensor(const torch::Tensor &means,  // [C, N, 3]
                      ...
                      const torch::Tensor &v_covars2d // [C, N, 3, 3]
) {
    DEVICE_GUARD(means);
    CHECK_INPUT(means);
    CHECK_INPUT(covars);
    ...
    torch::Tensor v_means = torch::empty_like(means);
    torch::Tensor v_covars = torch::empty_like(covars);

    if (C && N) {
        at::cuda::CUDAStream stream = at::cuda::getCurrentCUDAStream();
        AT_DISPATCH_FLOATING_TYPES_AND2(
            at::ScalarType::Half, at::ScalarType::BFloat16, v_means.scalar_type(),
            "persp_proj_bwd", [&]() {
                persp_proj_bwd_kernel<scalar_t>
                    <<<(C * N + N_THREADS - 1) / N_THREADS, N_THREADS, 0, stream>>>(
                        ...
                        v_means2d.data_ptr<scalar_t>(), v_covars2d.data_ptr<scalar_t>(),
                        v_means.data_ptr<scalar_t>(), v_covars.data_ptr<scalar_t>());
            });
    }
