...
)
{
    ...
    for(uint32_t b = range_start; b < min(range_end, num_batches); ++b)
    {
        ...
        for(uint32_t t = 0; (t < batch_size) && !done; ++t)
        {
            ...
            const vec2 s                = {ray_cross.x / ray_cross.z, ray_cross.y / ray_cross.z};
            ...
            else
            {
                ...
                pixel_ids[base + cnt]     = pix_id + image_id * image_height * image_width;
                cnt                      += 1;
            }
...
)
{
    ...
    if(cudaFuncSetAttribute(
           rasterize_to_indices_2dgs_kernel<float>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size
       )
       != cudaSuccess)
    {
        AT_ERROR(
    ...
    rasterize_to_indices_2dgs_kernel<float><<<grid, threads, shmem_size, at::cuda::getCurrentCUDAStream()>>>(
        range_start,
        range_end,
        I,
        N,
