...
rasterize_to_pixels_fwd_2dgs_tensor(
    ...
    const torch::Tensor &normals,   // [C, K, 3] or [nnz, 3]
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_projection_packed_fwd_tensor_3dcs(
    const torch::Tensor &convex_points,        // [N, 6, 3]
    const torch::Tensor &viewmats,             // [C, 4, 4]
    const torch::Tensor &Ks,                   // [C, 3, 3]
    const uint32_t image_width,
    const uint32_t image_height,
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_projection_packed_bwd_tensor_3dcs(
    // fwd inputs
    const torch::Tensor &convex_points,        // [N, 6, 3]
    const torch::Tensor &viewmats,             // [C, 4, 4]
    const torch::Tensor &Ks,                   // [C, 3, 3]
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_projection_fwd_tensor_3dcs(
    const torch::Tensor &convex_points,               // [N, K, 3]
    const torch::Tensor &cumsum_of_points_per_convex, // [N]
    const torch::Tensor &delta,                       // [N]
    const torch::Tensor &sigma,                       // [N]
    torch::Tensor &scaling,                     // [N]
    const torch::Tensor &viewmats,                    // [C, 4, 4]
    const torch::Tensor &Ks,                          // [C, 3, 3]
    const uint32_t image_width,
    const uint32_t image_height,
...
fully_fused_projection_bwd_tensor_3dcs(
    ...
    // fwd outputs
    const torch::Tensor &radii,                       // [C, N]
    const torch::Tensor &hull,                        // [C, 2*total_nb_points]
    const torch::Tensor &num_points_per_convex_view,  // [C, N]
    //const torch::Tensor &normals,                     // [C, total_nb_points, 2]
    const torch::Tensor &offsets,                     // [C, total_nb_points]
    const torch::Tensor &p_image,                     // [C, total_nb_points, 2]
    const torch::Tensor &indices,                     // [C, total_nb_points]
    const torch::Tensor &conics,                      // [C, N, 3]
    const at::optional<torch::Tensor> &compensations, // [C, N] optional
    // grad inputs
    const torch::Tensor &v_normals,                     // [C, total_nb_points, 2]
    const torch::Tensor &v_offsets,                     // [C, total_nb_points]
    // grad outputs
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
rasterize_to_pixels_fwd_tensor_3dcs(
    // Gaussian parameters
    const torch::Tensor &means2d,   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &normals,   // [C, total_nb_points, 2] or [nnz, 2]
    const torch::Tensor &offsets,   // [C, total_nb_points] or [nnz]
    const torch::Tensor &num_points_per_convex_view, // [C, N]
    const torch::Tensor &delta,     // [C, N]
    const torch::Tensor &sigma,     // [C, N]
    const uint32_t num_points_per_convex, // 6
    const torch::Tensor &cumsum_of_points_per_convex, // [C]
...
std::tuple<
    ...
    const torch::Tensor &sigma,                     // [C, N]
    const torch::Tensor &num_points_per_convex_view, // [C, N]
    const torch::Tensor &cumsum_of_points_per_convex, // [C, N]
    const torch::Tensor &depths,                    // [C, N]
    const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &opacities,                 // [C, N] or [nnz]
...
torch::Tensor compute_sh_fwd_tensor_3dcs(
    const uint32_t degrees_to_use,
    const torch::Tensor &convex_points,     // [N, 6, 3]
    const torch::Tensor &dirs,              // [..., 3]
    const torch::Tensor &coeffs,            // [..., K, 3]
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> compute_sh_bwd_tensor_3dcs(
    const uint32_t K,
    const uint32_t degrees_to_use,
    const torch::Tensor &convex_points,      // [N, 6, 3]
    const torch::Tensor &dirs,               // [..., 3]
