...
struct UnscentedTransformParameters {
    ...
    float in_image_margin_factor        = 0.1f;  // 10% out of bounds margin is acceptable for "valid" projection state
    bool require_all_sigma_points_valid = false; // true: all sigma points must be valid to mark a projection as "valid"
...
struct MonteCarloTransformParameters {
    ...
    float in_image_margin_factor         = 0.1f;  // 10% out of bounds margin is acceptable for "valid" projection state
    bool require_all_sample_points_valid = false; // true: all sample points must be valid to mark a projection as "valid"
                                                  // false: a single valid sample point is sufficient to mark a projection as "valid"
};
