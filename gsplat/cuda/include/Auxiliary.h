...
__forceinline__ __device__ glm::vec3 pix2world(
    const glm::vec2 pix,
    const int W,
    const int H,
    glm::vec4 inverse_vp0,
    glm::vec4 inverse_vp1,
    glm::vec4 inverse_vp3
) {
    const glm::vec2 pix_ndc = pix * glm::vec2(2.0f / W, 2.0f / H) - 1.0f;
    glm::vec4 p_world =
        inverse_vp0 * pix_ndc.x + inverse_vp1 * pix_ndc.y + inverse_vp3;
    float rcp_w = __frcp_rn(p_world.w);
    return glm::vec3(p_world) * rcp_w;
}

__forceinline__ __device__ glm::vec3 pix2world(
    const glm::vec2 pix, const int W, const int H, const glm::mat4 inverse_vp
) {
    return pix2world(pix, W, H, inverse_vp[0], inverse_vp[1], inverse_vp[3]);
}

__forceinline__ __device__ glm::vec3
world2ndc(const glm::vec3 p_world, const glm::mat4 viewproj_matrix) {
...
__forceinline__ __device__ void getRect(
    const glm::vec2 c,
    const glm::vec2 rect_extent,
    uint2 &rect_min,
    uint2 &rect_max,
    dim3 grid
) {
    // tile bounds covering pixel centers computed from rect centers (c ~ image
    // points) and rect extents
    rect_min = {
        min(grid.x,
            max((int)0, (int)floorf((c.x - 0.5f - rect_extent.x) / BLOCK_X))),
        min(grid.y,
            max((int)0, (int)floorf((c.y - 0.5f - rect_extent.y) / BLOCK_Y)))
    };
    rect_max = {
        min(grid.x,
            max((int)0, (int)ceilf((c.x - 0.5f + rect_extent.x) / BLOCK_X))),
        min(grid.y,
            max((int)0, (int)ceilf((c.y - 0.5f + rect_extent.y) / BLOCK_Y)))
    };
}
...
__forceinline__ __device__ bool in_frustum(
    ...
    {
        if (prefiltered) {
            printf("Point is filtered although prefiltered is set. This "
                   "shouldn't happen!");
            __trap();
        }
        return false;
...
constructSortKey(uint32_t tile_id, float depth) {
...
__forceinline__ __device__ glm::mat3 computeCov2D(
    ...
    bool is_perfect_equidistant_fisheye
) {
    ...
    if (!is_perfect_equidistant_fisheye) {
        ...
        // From FisheyGS
        // (https://github.com/zmliao/Fisheye-GS/blob/master/submodules/fisheye_gs_rasterizer/cuda_rasterizer/forward.cu)
        ...
        J = glm::mat3(
            focal_x * (x2 * a + y2 * b),
            focal_x * xy * (a - b),
            -focal_x * t.x * x2y2z2_inv,
            focal_y * xy * (a - b),
            focal_y * (y2 * a + x2 * b),
            -focal_y * t.y * x2y2z2_inv,
            0,
            0,
            0
        );
...
__device__ inline glm::vec3 dilateCov2D(
    const glm::mat3 cov,
    const bool proper_ewa_scaling,
    float &det_dilated,
    float &convolution_scaling_factor
) {
    // Apply low-pass filter: every Gaussian should be at least one pixel
    // wide/high. Discard 3rd row and column.
    ...
    if (proper_ewa_scaling) {
        // As employed by Yu et al. in "Mip-Splatting: Alias-free 3D Gaussian
        // Splatting" https://github.com/autonomousvision/mip-splatting
        const float det_orig = cov[0][0] * cov[1][1] - cov[0][1] * cov[0][1];
        // convolution_scaling_factor = sqrt(max(0.000025f, det_orig /
        // det_dilated)); // max for numerical stability
...
__forceinline__ __device__ float4 computeConicOpacity(
    glm::vec3 cov2D, float opacity, float det, float convolution_scaling_factor
) {
...
__forceinline__ __device__ void computeCov3D(
    const glm::vec3 scale, /*float mod,*/ const glm::vec4 rot, float *cov3D
) {
...
bwd_safe_normalize(const glm::vec3 v, glm::vec3 &d_v, glm::vec3 d_out) {
    float l = sqrtf(v.x * v.x + v.y * v.y + v.z * v.z);
    if (l > 0.0f) {
        float fac = 1.0 / powf(v.x * v.x + v.y * v.y + v.z * v.z, 1.5f);
        d_v.x += (d_out.x * (v.y * v.y + v.z * v.z) - d_out.y * (v.x * v.y) -
                  d_out.z * (v.x * v.z)) *
                 fac;
        d_v.y += (d_out.y * (v.x * v.x + v.z * v.z) - d_out.x * (v.y * v.x) -
                  d_out.z * (v.y * v.z)) *
                 fac;
        d_v.z += (d_out.z * (v.x * v.x + v.y * v.y) - d_out.x * (v.z * v.x) -
                  d_out.y * (v.z * v.y)) *
                 fac;
    }
}
...
__forceinline__ __device__ glm::vec3
matmul_bw_vec(const glm::mat3 &m, const glm::vec3 &gdt) {
    return glm::vec3(
...
__forceinline__ __device__ glm::vec4
matmul_bw_quat(const glm::vec3 &p, const glm::vec3 &g, const glm::vec4 &q) {
    glm::vec3 dmat[3];
    dmat[0] = g.x * p;
    dmat[1] = g.y * p;
    dmat[2] = g.z * p;
    ...
    // m[0] = make_float3((1.f - 2.f * (y * y + z * z)), 2.f * (x * y + r *
    // z), 2.f * (x * z - r * y));
    ...
    // m[1] = make_float3(2.f * (x * y - r * z), (1.f - 2.f * (x * x + z *
    // z)), 2.f * (y * z + r * x));
    ...
    // m[2] = make_float3(2.f * (x * z + r * y), 2.f * (y * z - r * x), (1.f
    // - 2.f * (x * x + y * y)));
...
__forceinline__ __device__ glm::vec3
safe_normalize_bw(const glm::vec3 &v, const glm::vec3 &d_out) {
    const float l = v.x * v.x + v.y * v.y + v.z * v.z;
    if (l > 0.0f) {
        const float il = rsqrtf(l);
        const float il3 = (il * il * il);
        return il * d_out - il3 * (d_out * v) * v;
    }
    return d_out;
}
...
#if GENERALIZED_GAUSSIAN_KERNEL ==                                             \
    8 // just because I love the Zenzizenzizenzic kernel
...
evaluate_opacity_factor3D_and_depth_geometric_bwd(
    ...
    const glm::vec3 grdsRayHitGrd =
        gdist > 0.0f
            ? ((/*2.0f **/ grds) / (/*2.0f **/ gdist)) * rayHitGrdWeight
            : glm::vec3(0.0f);
    const glm::vec3 gsclRayHitGrd = grdd * grdsRayHitGrd;
    const glm::vec3 grdRayHitGrd =
        -gscl *
        glm::vec3(
...
/// d_e^{-a*|x|^b}/d_x^2 = -a*(0.5*b)*x^{b-2}*e^{-a*|x|^b}
    ...
    dL_dscales.x += gsclRayHitGrd.x + gsclGrdGro.x +
                    (-rayDirR.x / (gscl.x * gscl.x)) * grduGrd.x;
    dL_dscales.y += gsclRayHitGrd.y + gsclGrdGro.y +
                    (-rayDirR.y / (gscl.y * gscl.y)) * grduGrd.y;
    dL_dscales.z += gsclRayHitGrd.z + gsclGrdGro.z +
                    (-rayDirR.z / (gscl.z * gscl.z)) * grduGrd.z;
...
__forceinline__ __device__ float depth_along_ray_geometric(
    const glm::vec3 &grd, const glm::vec3 &gro, const glm::vec3 &gscl
) {
...
__forceinline__ __device__ glm::vec3 computeViewRay(
    ...
    const glm::vec3 p_world =
        pix2world(glm::vec2(pix.x, pix.y), W, H, inverse_vp);
    const glm::vec3 viewdir =
        glm::normalize(p_world - glm::vec3(campos.x, campos.y, campos.z));
    ...
    glm::vec3 res =
        (c_magma[0] +
         x * (c_magma[1] +
              x * (c_magma[2] +
                   x * (c_magma[3] +
                        x * (c_magma[4] + x * (c_magma[5] + c_magma[6] * x)))))
        );
    return glm::vec3(
...
// supporting the TURBO depth colormap of google
...
__forceinline__ __device__ glm::vec3 colormapTurbo(float x) {
    ...
    return glm::vec3(
        glm::clamp(
            turbo_srgb_floats[floor][0] +
                (turbo_srgb_floats[ceil][0] - turbo_srgb_floats[floor][0]) *
                    diff,
            0.f,
            1.f
        ),
        glm::clamp(
            turbo_srgb_floats[floor][1] +
                (turbo_srgb_floats[ceil][1] - turbo_srgb_floats[floor][1]) *
                    diff,
            0.f,
            1.f
        ),
        glm::clamp(
            turbo_srgb_floats[floor][2] +
                (turbo_srgb_floats[ceil][2] - turbo_srgb_floats[floor][2]) *
                    diff,
            0.f,
            1.f
        )
