...
)
{
    ...
    float Rt     = fmaxf(sqrtf(fmaxf(0.25f * (at - ct) * (at - ct) + bt * bt, 0.f)), 1e-12f);
    float alpha  = Rb / Rt;
...
)
{
    ...
    v_cov3d     += glm::transpose(J_unclamped) * G_true * J_unclamped + glm::transpose(J) * G_box * J;
    mat3x2 v_Jt  = G_true * J_unclamped * glm::transpose(cov3d) + glm::transpose(G_true) * J_unclamped * cov3d;
    mat3x2 v_J   = G_box * J * glm::transpose(cov3d) + glm::transpose(G_box) * J * cov3d;
    v_mean3d.x  += -fx * rz2 * v_Jt[2][0];
    v_mean3d.y  += -fy * rz2 * v_Jt[2][1];
    v_mean3d.z  += -fx * rz2 * v_Jt[0][0]
                 - fy * rz2 * v_Jt[1][1]
