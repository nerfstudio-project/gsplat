...
from gsplat.cuda._lidar import SpinningDirection, LidarCameraParameters
...
class _BaseCameraModel(ABC):
    ...
    def create(
        ...
        if camera_model == "lidar":
            assert lidar_coeffs is not None, "lidar_coeffs is required for lidar camera model"
            return _LidarCameraModel(lidar_coeffs)
        ...
        assert principal_points is not None, "principal_points is required for non-lidar camera models"
...
class _LidarCameraModel(_BaseCameraModel):
    ...
    def __init__(
        self,
        params: LidarCameraParameters,
    ):
        """Initialize lidar camera model.

        Args:
            params: Lidar camera parameters dataclass containing sensor configuration.
        """
        ...
        super().__init__(width=params.row_elevations_rad.shape[0], height=params.column_azimuths_rad.shape[0], shutter_type=RollingShutterType.GLOBAL)

        self._params = params

        # Check if lidar parameters are consistent with what we expect
        assert params.spinning_frequency_hz > 0.0

        assert params.row_elevations_rad.dtype == torch.float32
        assert params.row_elevations_rad.shape == (self.n_rows,)
        assert params.row_azimuth_offsets_rad.dtype == torch.float32
        assert params.row_azimuth_offsets_rad.shape == (self.n_rows,)
        assert params.column_azimuths_rad.dtype == torch.float32
        assert params.column_azimuths_rad.shape == (self.n_columns,)

        # Check elevation angles are sorted consistently
        relative_row_elevations_rad = self._relative_angle(params.row_elevations_rad[0], params.row_elevations_rad, SpinningDirection.CLOCKWISE)
        assert torch.all(torch.diff(relative_row_elevations_rad) > 0), (
            "Row elevation angles must be sorted in descending order (cw)"
        )
        assert torch.all(~self._angle_range_wrap_around(params.row_elevations_rad[0], params.row_elevations_rad)), (
        ...
        relative_column_azimuths_rad = self._relative_angle(
            params.column_azimuths_rad[0], params.column_azimuths_rad, params.spinning_direction
        )
        ...
        assert torch.all(~self._angle_range_wrap_around(params.column_azimuths_rad[0], params.column_azimuths_rad)), (
            "Column azimuth angles (without offsets) must not wrap around the start element"
        )

        assert params.angles_to_columns_map.shape[0] % params.row_elevations_rad.shape[0] == 0
        assert params.angles_to_columns_map.shape[1] % params.column_azimuths_rad.shape[0] == 0
        a2cmap_resfactor = params.angles_to_columns_map.shape[0]/params.row_elevations_rad.shape[0]
        assert a2cmap_resfactor == params.angles_to_columns_map.shape[1]/params.column_azimuths_rad.shape[0]
    ...
    @property
    def n_rows(self) -> int:
        # TODO: Transposing to conform with nrend, this must be undone later
        return self.width
    ...
    @property
    def fov_vert_rad(self):
        start = self.row_elevations_rad[0].item()
        span = start-self.row_elevations_rad[-1].item()
        return self.FOV(start=start, span=span, direction=SpinningDirection.CLOCKWISE)

    @property
    def fov_horiz_rad(self):
        # Reconstruct first and last (wrapped) element azimuths per elevation once to obtain FoV bounds
        azimuth_extremes = self.column_azimuths_rad[None, [0, self.n_columns-1]] + self.row_azimuth_offsets_rad[:, None]
        ...
        return self.FOV(start=start, span=min(span,2*math.pi), direction=self.spinning_direction)

    @property
    def principal_points(self) -> Tensor:
        # Just return a dummy tensor, as Lidar doesn't have principal point.
        # TODO: fix the class hierarchy to properly accomidate lidar sensors
        return torch.tensor([self.width / 2, self.height / 2], dtype=torch.float32, device=self.row_elevations_rad.device)

    def _relative_clock_rotation(self, angle_ref, angle, spinning_direction: SpinningDirection):
    ...
    def _relative_angle(self, angle_ref, angle, spinning_direction: SpinningDirection) -> float:
        """ Normalize angle to the interval (-pi, pi]"""
        rel_angle = self._relative_clock_rotation(angle_ref, angle, spinning_direction)
        return torch.remainder(rel_angle, 2*math.pi)

    def _angle_range_wrap_around(self, start_angle, end_angle) -> bool:
        return torch.abs(end_angle-start_angle) >= 2*math.pi

    def _relative_sensor_angles(self, *, elevation: Tensor, azimuth: Tensor) -> tuple[Tensor,Tensor]:
        ...
        rel_elevation = self._relative_clock_rotation(fov_vert_start_adj, elevation, SpinningDirection.CLOCKWISE)
        rel_azimuth = self._relative_angle(fov_horiz_start_adj, azimuth, self.spinning_direction)

        return rel_elevation, rel_azimuth

    def _valid_relative_sensor_angles(self, *, rel_elevation: Tensor, rel_azimuth: Tensor) -> Tensor:
        ...
        return (rel_elevation <= self.fov_vert_rad.span + self.fov_eps_rad*2) & \
               (rel_azimuth <= self.fov_horiz_rad.span + self.fov_eps_rad*2)

    def _valid_sensor_angles(self, *, elevation: Tensor, azimuth: Tensor) -> Tensor:
        rel_elevation, rel_azimuth = self._relative_sensor_angles(elevation=elevation, azimuth=azimuth)
        return self._valid_relative_sensor_angles(rel_elevation=rel_elevation, rel_azimuth=rel_azimuth)
    ...
    def camera_ray_to_image_point(
        ...
        valid = self._valid_relative_sensor_angles(rel_elevation=rel_elevation, rel_azimuth=rel_azimuth) & \
                (rel_elevation < self.fov_vert_rad.span + margin_elevation) & \
                (rel_azimuth < self.fov_horiz_rad.span + margin_azimuth) & \
                (rel_elevation >= -margin_elevation) & \
                (rel_azimuth >= -margin_azimuth)
    ...
    def image_point_to_camera_ray(
        ...
        return camera_ray, self._valid_sensor_angles(elevation=elevation, azimuth=azimuth)
    ...
    def shutter_relative_frame_time(
        ...
        map_resolution_horiz_rad: float = self.fov_horiz_rad.span/(self.angles_to_columns_map.shape[1]-1)
        map_resolution_vert_rad: float = self.fov_vert_rad.span/(self.angles_to_columns_map.shape[0]-1)

        # 1.compute relative angles wrt. FOV start
        rel_elevation, rel_azimuth = self._relative_sensor_angles(elevation=elevation, azimuth=azimuth)

        assert torch.all(self._valid_relative_sensor_angles(rel_elevation=rel_elevation, rel_azimuth=rel_azimuth))

        # 2. Compute the indices into the lookup map
        ivert = torch.clamp(rel_elevation/map_resolution_vert_rad + 0.5, 0, self.angles_to_columns_map.shape[0]-1).to(torch.int)
        ihoriz = torch.clamp(rel_azimuth/map_resolution_horiz_rad + 0.5, 0, self.angles_to_columns_map.shape[1]-1).to(torch.int)
