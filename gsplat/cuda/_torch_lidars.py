...
class _SpinningLidarModel(_LidarModel):
    ...
    def relative_angle(self, angle_ref, angle, spinning_direction: SpinningDirection, *, scale: float = 1) -> float:
        rel_angle = self.relative_clock_rotation(angle_ref, angle, spinning_direction)
        period = scale*2*math.pi
        return rel_angle % period

    def angle_range_wrap_around(self, start_angle, end_angle, *, scale: float = 1) -> bool:
        return torch.abs(end_angle-start_angle) >= scale*2*math.pi

    def relative_sensor_angles(self, angles: SphericalUnitCoord, *, scale: float = 1) -> SphericalUnitCoord:
        rel_elevation = self.relative_clock_rotation(self.fov_vert_rad.start*scale, angles.elevation, SpinningDirection.CLOCKWISE)
        rel_azimuth = self.relative_angle(self.fov_horiz_rad.start*scale, angles.azimuth, self.spinning_direction, scale=scale)

        return SphericalUnitCoord(elevation=rel_elevation, azimuth=rel_azimuth)

    def valid_sensor_angles(self, angles: SphericalUnitCoord, *, scale: float=1) -> Tensor:
        ...
        rel_elevation = self.relative_clock_rotation(fov_vert_start_adj*scale, angles.elevation, SpinningDirection.CLOCKWISE)
        rel_azimuth = self.relative_angle(fov_horiz_start_adj*scale, angles.azimuth, self.spinning_direction, scale=scale)
        ...
        return (rel_elevation <= scale*(self.fov_vert_rad.span + self.fov_eps_rad*2)) & \
...
class _RowOffsetStructuredSpinningLidarModel(_StructuredSpinningLidarModel):
    ...
    def principal_points(self) -> Tensor:
        ...
        # TODO: fix the class hierarchy to properly accomodate lidar sensors
        return torch.tensor([self.width / 2, self.height / 2], dtype=torch.float32, device=self.device)

    def relative_clock_rotation(self, angle_ref, angle, spinning_direction: SpinningDirection):
    ...
    @staticmethod
    def _compute_fov_vert_rad(params: RowOffsetStructuredSpinningLidarModelParameters) -> FOV:
    ...
    @staticmethod
    def _compute_fov_horiz_rad(params: RowOffsetStructuredSpinningLidarModelParameters) -> FOV:
        # Reconstruct first and last (wrapped) element azimuths per elevation once to obtain FoV bounds
        azimuth_extremes = params.column_azimuths_rad[None, [0, params.n_columns-1]] + params.row_azimuth_offsets_rad[:, None]
        ...
        return FOV(start=start, span=min(span,2*math.pi), direction=params.spinning_direction)
    ...
    def shutter_relative_frame_time(
        ...
        ivert = torch.clamp(rel_angles.elevation/map_resolution_vert_rad + 0.5, 0, self.angles_to_columns_map.shape[0]-1).to(torch.int)
        ihoriz = torch.clamp(rel_angles.azimuth/map_resolution_horiz_rad + 0.5, 0, self.angles_to_columns_map.shape[1]-1).to(torch.int)
