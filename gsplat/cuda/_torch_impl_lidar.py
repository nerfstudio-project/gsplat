...
def lidar_sample_tileid(lidar: RowOffsetStructuredSpinningLidarModelParametersExt,
                        rel_pix_el: Tensor,
                        rel_pix_az: Tensor) -> LidarSampleTileIdReturn:
    ...
    idxdense_az = torch.floor(norm_az*lidar.tiling.cdf_resolution_azimuth).to(dtype=torch.int32)
    idxdense_el = torch.floor(norm_el*lidar.tiling.cdf_resolution_elevation).to(dtype=torch.int32)
    # Clamp to valid range
    idxdense_el = torch.clamp(idxdense_el, min=0, max=lidar.tiling.cdf_resolution_elevation-1)
...
def has_any_rays_in_tile(lidar: RowOffsetStructuredSpinningLidarModelParametersExt, min_el: Tensor, max_el: Tensor, min_az: Tensor, max_az: Tensor) -> Tensor:
    ...
    has_rays = torch.where((has_rays<0) & ((min_az >= 0) & (max_az <= raycdf_size_az)),
                           region_sum_fully_inside > 0, has_rays)

    zero_az = torch.zeros_like(min_az)
    full_az = torch.full_like(min_az, raycdf_size_az)

    # Wraps at left edge
    region_sum_wraps_left = (
        cdf_region_sum(min_az=zero_az, max_az=max_az)
...
def _isect_tiles_lidar(
    ...
    *,
    ...
    rel_angles_pix = sensor.relative_sensor_angles(angles_pix, scale=ANGLE_TO_PIXEL_SCALING_FACTOR)
    ...
    max_dense_el = torch.min(max_sample_tileid.idxdense_el+1, min_dense_el+lidar.tiling.cdf_resolution_elevation)
    max_dense_az = torch.min(max_sample_tileid.idxdense_az+1, min_dense_az+lidar.tiling.cdf_resolution_azimuth)
    ...
    has_rays = ((extent_x!=0) & (extent_y!=0)) & \
               has_any_rays_in_tile(lidar, min_el=min_dense_el, max_el=max_dense_el,
                                            min_az=min_dense_az, max_az=max_dense_az)
    ...
    tile_max_el = torch.where(has_rays, torch.min(max_sample_tileid.idx_el+1, tile_min_el+lidar.tiling.n_bins_elevation), 0)
    tile_max_az = torch.where(has_rays, torch.min(max_sample_tileid.idx_az+1, tile_min_az+lidar.tiling.n_bins_azimuth), 0)
