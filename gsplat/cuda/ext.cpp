...
TORCH_LIBRARY(gsplat, m) {
    ...
    m.def(
        "intersect_tile_sparse(Tensor means2d, Tensor radii, Tensor depths, "
        "Tensor? image_ids, Tensor tile_mask, Tensor active_tiles, int I, int "
        "tile_size, int tile_width, int tile_height) -> (Tensor, Tensor)"
    );
