...
class LidarTiling:
    ...
    def __post_init__(self):
        ...
        assert self.cdf_dense_ray_mask.shape[-1] == self.cdf_elevation.shape[0], (self.cdf_dense_ray_mask.shape, self.cdf_elevation.shape)
...
def compute_histogram_equalization(parameters: RowOffsetStructuredSpinningLidarModelParameters,
    ...
    assert torch.all(
        torch.logical_and(
            ...
            angles.elevation <= parameters.fov_vert_rad.span + 4 * torch.finfo(torch.float32).eps,
            angles.azimuth <= parameters.fov_horiz_rad.span + 4 * torch.finfo(torch.float32).eps,
        )
    ), (
