...
def relative_clock_rotation(angle_ref: Tensor, angle: Tensor, direction: SpinningDirection) -> Tensor:
...
def relative_angle(angle_ref: Tensor, angle: Tensor, direction: SpinningDirection, scale: float = 1) -> float:
...
def normalize_elevation(angle: Tensor, scale: float = 1) -> Tensor:
    return torch.clamp(normalize_angle(angle, start=-scale*math.pi, scale=scale), -scale*math.pi/2, scale*math.pi/2)
...
class SphericalUnitCoord:
    ...
    def __post_init__(self):
        assert self.elevation.dtype == self.azimuth.dtype, (self.elevation.dtype, self.azimuth.dtype)
        assert self.elevation.device == self.azimuth.device, (self.elevation.device, self.azimuth.device)
        assert self.elevation.shape == self.azimuth.shape, (self.elevation.shape, self.azimuth.shape)

    def to(self, *, dtype: torch.dtype, device: torch.device) -> "SphericalUnitCoord":
        return SphericalUnitCoord(elevation=self.elevation.to(dtype=dtype, device=device),
    ...
    def reshape(self, *args) -> "SphericalUnitCoord":
        return SphericalUnitCoord(elevation=self.elevation.reshape(*args),
                                  azimuth=self.azimuth.reshape(*args))

    def __getitem__(self, *args) -> "SphericalUnitCoord":
        return SphericalUnitCoord(elevation=self.elevation[*args],
                                  azimuth=self.azimuth[*args])

    def _binop(self, other, op) -> "SphericalUnitCoord":
        if isinstance(other, SphericalUnitCoord):
            return SphericalUnitCoord(elevation=op(self.elevation, other.elevation),
                                      azimuth=op(self.azimuth, other.azimuth))
        return SphericalUnitCoord(elevation=op(self.elevation, other),
                                  azimuth=op(self.azimuth, other))

    def _rbinop(self, other, op) -> "SphericalUnitCoord":
        return SphericalUnitCoord(elevation=op(other, self.elevation),
                                  azimuth=op(other, self.azimuth))
...
@dataclass(frozen=True, kw_only=True)
class StructuredSpinningLidarModelParameters(StructuredLidarModelParameters, SpinningLidarModelParameters):
    pass
...
class RowOffsetStructuredSpinningLidarModelParameters(StructuredSpinningLidarModelParameters):
    ...
    def __init__(self, *,
        ...
        params.fov_horiz_rad = self._compute_fov_horiz_rad(spinning_direction=spinning_direction)
    ...
    def __post_init__(self, fov_eps_factor: int):
        ...
        relative_row_elevations_rad = relative_angle(self.row_elevations_rad[0], self.row_elevations_rad, SpinningDirection.CLOCKWISE)
        assert torch.all(torch.diff(relative_row_elevations_rad) > 0), (
            "Row elevation angles must be sorted in descending order (cw)"
        )
        assert torch.all(~angle_range_wrap_around(self.row_elevations_rad[0], self.row_elevations_rad)), (
        ...
        relative_column_azimuths_rad = relative_angle(self.column_azimuths_rad[0], self.column_azimuths_rad, self.spinning_direction)
        ...
        assert torch.all(~angle_range_wrap_around(self.column_azimuths_rad[0], self.column_azimuths_rad)), (
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, RowOffsetStructuredSpinningLidarModelParameters):
            ...
            return (
                ...
                torch.equal(self.row_elevations_rad, other.row_elevations_rad) and
                torch.equal(self.column_azimuths_rad, other.column_azimuths_rad) and
                torch.equal(self.row_azimuth_offsets_rad, other.row_azimuth_offsets_rad)
            )
    ...
    def _compute_fov_vert_rad(self) -> FOV:
        ...
        span_rad = relative_angle(start_rad, self.row_elevations_rad[-1], SpinningDirection.CLOCKWISE).item()
        assert span_rad >= 0

        return FOV(start=float(start_rad), span=float(span_rad), direction=SpinningDirection.CLOCKWISE)
    ...
    def _compute_fov_horiz_rad(self,spinning_direction: SpinningDirection) -> FOV:
        ...
        azimuth_extremes_rad = self.column_azimuths_rad[None, [0, self.column_azimuths_rad.shape[0]-1]] + self.row_azimuth_offsets_rad[:, None]
        ...
        if torch.any(angle_range_wrap_around(start_rad, end_rad)):
            ...
            span_rad = relative_angle(start_rad, end_rad, spinning_direction).max().item()

        return FOV(start=float(start_rad), span=float(span_rad), direction=spinning_direction)

    def elements_to_sensor_angles(self, elements: SphericalUnitCoord) -> SphericalUnitCoord:
        return SphericalUnitCoord(
            elevation = normalize_elevation(self.row_elevations_rad[elements.elevation]),
            azimuth = normalize_azimuth(self.column_azimuths_rad[elements.azimuth] + self.row_azimuth_offsets_rad[elements.elevation])
        )
...
class LidarTiling:
    ...
    def __post_init__(self):
        ...
        assert self.cdf_dense_ray_mask.shape[-1] == self.cdf_elevation.shape[0], f"{self.cdf_dense_ray_mask.shape=} {self.cdf_elevation.shape=}"

        assert self.tiles_pack_info.ndim == 2, self.tiles_pack_info.ndim
        assert self.tiles_pack_info.shape == (self.n_bins_azimuth*self.n_bins_elevation,2), self.tiles_pack_info.shape
        ...
        assert self.tiles_to_elements_map.dtype == torch.int32, self.tiles_to_elements_map.dtype
...
class RowOffsetStructuredSpinningLidarModelParametersExt(RowOffsetStructuredSpinningLidarModelParameters):
    """Lidar camera parameters extended with acceleration structures"""

    angles_to_columns_map: Tensor
    tiling: LidarTiling

    def __init__(self, angles_to_columns_map: Tensor, tiling: LidarTiling, **kwargs) -> None:
        ...
        assert self.angles_to_columns_map.shape[0] % self.row_elevations_rad.shape[0] == 0
        assert self.angles_to_columns_map.shape[1] % self.column_azimuths_rad.shape[0] == 0
        a2cmap_resfactor = self.angles_to_columns_map.shape[0]/self.row_elevations_rad.shape[0]
        assert a2cmap_resfactor == self.angles_to_columns_map.shape[1]/self.column_azimuths_rad.shape[0]


# --------------------- Computation of angles_to_columns_map ---------------------------

def relative_sensor_angles(lidar: SpinningLidarModelParameters, coord: SphericalUnitCoord, scale: float = 1) -> SphericalUnitCoord:
    return SphericalUnitCoord(elevation = relative_clock_rotation(lidar.fov_vert_rad.start*scale, coord.elevation, SpinningDirection.CLOCKWISE),
                              azimuth = relative_angle(lidar.fov_horiz_rad.start*scale, coord.azimuth, lidar.spinning_direction, scale))

def valid_sensor_angles(lidar: SpinningLidarModelParameters, coord: SphericalUnitCoord, scale: float = 1) -> Tensor:
    ...
    rel_elevation = relative_clock_rotation(fov_vert_start_adj*scale, coord.elevation, SpinningDirection.CLOCKWISE)
    rel_azimuth = relative_angle(fov_horiz_start_adj*scale, coord.azimuth, lidar.spinning_direction, scale)
    ...
    return (rel_elevation <= (lidar.fov_vert_rad.span + lidar.fov_eps_rad*2)*scale) & \
...
def sensor_angles_to_rays(lidar: SpinningLidarModelParameters, sensor_angles: SphericalUnitCoord) -> SensorRayReturn:
    ...
    x = torch.cos(sensor_angles.azimuth) * (cos_elevations := torch.cos(sensor_angles.elevation))
...
def compute_angles_to_columns_map(lidar: RowOffsetStructuredSpinningLidarModelParameters, resolution_factor: float=4, dtype: torch.dtype=torch.int32):
    ...
    grid_angles = SphericalUnitCoord(elevation=grid_elevations_rad, azimuth=grid_azimuths_rad)
...
def angles_to_dense_ray_mask_cdf(
    ...
    *,
    ...
    normalized_angles = SphericalUnitCoord(elevation=relative_angles.elevation/parameters.fov_vert_rad.span,
                                           azimuth=relative_angles.azimuth/parameters.fov_horiz_rad.span)

    elevations_indices = uniform_quantization(normalized_angles.elevation, resolution_elevation)
    azimuths_indices = uniform_quantization(normalized_angles.azimuth, resolution_azimuth)
...
def angles_to_tile_indices(
    ...
    *,
    ...
    normalized_angles = SphericalUnitCoord(elevation = relative_angles.elevation/parameters.fov_vert_rad.span*resolution,
    ...
    elevations_indices_cdf = torch.clamp(normalized_angles.elevation, 0, resolution - 1).int()
...
def compute_tiles_to_elements_map(
    ...
    *,
    ...
    tile_indices = angles_to_tile_indices(parameters, angles,
    ...
    tiles_pack_info = torch.stack([tile_starts, tile_counts], dim=-1).int().to(device=elements.device).contiguous()
    ...
    sorted_elements = torch.stack([sorted_elements.elevation, sorted_elements.azimuth], dim=-1)
...
def compute_histogram_equalization(parameters: RowOffsetStructuredSpinningLidarModelParameters,
                                   *,
                                   n_bins_elevation: int,
    ...
    elements = parameters.create_elements()
    angles = parameters.elements_to_sensor_angles(elements).reshape(parameters.n_rows, parameters.n_columns)
    ...
    hist, _ = compute_hist1d(angles.elevation, bins=resolution_elevation, range=ranges_elevation)
    ...
    hist_elevations, _ = compute_hist1d(angles.elevation, bins=edges_elevation, range=ranges_elevation)
...
def compute_tiling(lidar_params: RowOffsetStructuredSpinningLidarModelParameters,
    ...
    ) = compute_histogram_equalization(lidar_params,
                                       n_bins_elevation = n_bins_elevation,
    ...
    ) = compute_tiles_to_elements_map(lidar_params,
