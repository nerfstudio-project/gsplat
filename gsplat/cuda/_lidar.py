...
def relative_angle(angle_ref: Tensor, angle: Tensor, direction: SpinningDirection, period: float = 2*math.pi) -> float:
    rel_angle = relative_clock_rotation(angle_ref, angle, direction)

    # Normalize it between [0, period)
    return rel_angle % period

def angle_range_wrap_around(start_angle, end_angle, period: float = 2*math.pi) -> bool:
...
class LidarCameraParameters:
    ...
    def __hash__(self) -> int:
        return hash((int(torch.hash_tensor(self.row_elevations_rad).item()),
    ...
    def __eq__(self, other) -> bool:
        if not isinstance(other, LidarCameraParameters):
            ...
            return (
                self.spinning_direction == other.spinning_direction and
                self.spinning_frequency_hz == other.spinning_frequency_hz and
    ...
    def fov_vert_rad(self) -> FOV:
        ...
        if self._cache_fov_vert is None:
            ...
            object.__setattr__(self,"_cache_fov_vert",FOV(start=start_rad, span=span_rad, direction=SpinningDirection.CLOCKWISE))
    ...
    def fov_horiz_rad(self) -> FOV:
        ...
        if self._cache_fov_horiz is None:
            ...
            azimuth_extremes_rad = self.column_azimuths_rad[None, [0, self.n_columns-1]] + self.row_azimuth_offsets_rad[:, None]
            ...
            if torch.any(angle_range_wrap_around(start_rad, end_rad)):
                ...
                span_rad = relative_angle(start_rad, end_rad, self.spinning_direction).max().item()

            object.__setattr__(self, "_cache_fov_horiz", FOV(start=start_rad, span=span_rad, direction=self.spinning_direction))

        return self._cache_fov_horiz

def relative_sensor_angles(lidar: LidarCameraParameters, coord: SphericalUnitCoord) -> SphericalUnitCoord:
    ...
    return SphericalUnitCoord(elevation = relative_clock_rotation(fov_vert_start_adj, coord.elevation, SpinningDirection.CLOCKWISE),
                              azimuth = relative_angle(fov_horiz_start_adj, coord.azimuth, lidar.spinning_direction))

def valid_relative_sensor_angles(lidar: LidarCameraParameters, relcoord: SphericalUnitCoord) -> Tensor:
...
def valid_sensor_angles(lidar: LidarCameraParameters, coord: SphericalUnitCoord) -> Tensor:
...
def sensor_angles_to_rays(lidar: LidarCameraParameters, sensor_angles: SphericalUnitCoord) -> SensorRayReturn:
...
def compute_angles_to_columns_map(lidar: LidarCameraParameters, resolution_factor: float=4, dtype: torch.dtype=torch.int32):
    ...
    element_azimuths_rad = normalize_angle_mpi_pi(
        lidar.column_azimuths_rad[elements[:, :, 1]] + lidar.row_azimuth_offsets_rad[elements[:, :, 0]]
    )

    # Compute a column-major list of all sensor coordinates
    sensor_angles = SphericalUnitCoord(elevation=torch.tile(lidar.row_elevations_rad, [lidar.n_columns, 1]).reshape(-1),
