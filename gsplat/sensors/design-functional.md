...
`gsplat/sensors/functional/` is the public stateless API for camera and
spinning-LiDAR projection. It is the import path downstream code should prefer
(`gsplat.sensors.functional` or `from gsplat.sensors import functional as F`).
...
pose unpacking — lives under `gsplat/sensors/kernels/`. The model layer
(`gsplat/sensors/models/`) builds on top of the functional API for trainable /
...
`kernels/common/utils.poses_to_matrix`, which delegates to `gsplat/geometry`. The
...
`kernels/common/utils.poses_to_matrix`, which delegates to `gsplat/geometry`.
