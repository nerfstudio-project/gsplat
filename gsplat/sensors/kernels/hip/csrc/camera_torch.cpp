...
// the CUDA _launch helpers (camera_params.h / camera_kernel.hip).
//
// Responsibilities:
...
OpenCVPinholeProjection_KernelParameters OpenCVPinholeProjection::to_kernel_params() const
{
    ...
    params.focal_length      = focal_length.data_ptr<float>();
    params.principal_point   = principal_point.data_ptr<float>();
    params.radial_coeffs     = radial_coeffs.data_ptr<float>();
    params.tangential_coeffs = tangential_coeffs.data_ptr<float>();
    params.thin_prism_coeffs = thin_prism_coeffs.data_ptr<float>();
...
at::Tensor generate_image_points(int64_t width, int64_t height, c10::Device device)
{
    ...
    auto guard        = c10::hip::HIPGuard(device);
    ...
    generate_image_points_launch(
        width, height, image_points.data_ptr<float>(), at::hip::getCurrentHIPStream(device.index()).stream()
    );
    return image_points;
}
...
)
{
    ...
    auto guard = c10::hip::HIPGuard(rays.device());
...
)
{
    ...
    auto guard = c10::hip::HIPGuard(pts.device());
...
FThetaProjection_KernelParameters FThetaProjection::to_kernel_params() const
{
    ...
    params.principal_point      = principal_point.data_ptr<float>();
    params.fw_poly              = fw_poly.data_ptr<float>();
    params.bw_poly              = bw_poly.data_ptr<float>();
    params.A                    = A.data_ptr<float>();
    params.Ainv                 = Ainv_cache.data_ptr<float>();
    params.width                = resolution[0];
    params.height               = resolution[1];
...
OpenCVFisheyeProjection_KernelParameters OpenCVFisheyeProjection::to_kernel_params() const
{
    ...
    params.principal_point        = principal_point.data_ptr<float>();
    params.focal_length           = focal_length.data_ptr<float>();
    params.forward_poly           = forward_poly.data_ptr<float>();
    params.approx_backward_factor = approx_backward_factor.data_ptr<float>();
    params.width                  = resolution[0];
    params.height                 = resolution[1];
    params.newton_iterations      = newton_iterations;
    params.max_angle              = static_cast<float>(max_angle);
    ...
    )
{
    ...
    auto guard = c10::hip::HIPGuard(grad.device());
