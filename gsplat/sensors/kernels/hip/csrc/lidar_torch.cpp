...
// LiDAR CUDA _launch helpers (lidar_params.h / lidar_kernel.hip).
...
namespace
{
    ...
    void check_same_device(const at::Tensor &tensor, const at::Tensor &ref, const char *name, const char *ref_name)
    {
    ...
    )
    {
        ...
        TORCH_CHECK(
            max_iterations <= kMaxSpinningLidarIterations, "max_iterations must be <= ", kMaxSpinningLidarIterations
        );
        TORCH_CHECK(
        ...
        TORCH_CHECK(
            std::isfinite(initial_relative_time) && initial_relative_time >= 0.0 && initial_relative_time <= 1.0,
            "initial_relative_time must be finite and in [0, 1]"
        );
    }

    hipStream_t current_stream(const at::Tensor &tensor)
    {
        return at::hip::getCurrentHIPStream(tensor.get_device()).stream();
    }
    ...
    void check_angle_table(
        const at::Tensor &tensor, int64_t expected, const at::Tensor &ref, const char *name, const char *ref_name
    )
    {
        TORCH_CHECK(tensor.dim() == 1 && tensor.size(0) == expected, name, " must be (", expected, ",)");
    ...
    RowOffsetStructuredSpinningLidarProjection::to_kernel_params() const
{
    ...
    params.row_elevations_rad      = row_elevations_rad.data_ptr<float>();
    params.column_azimuths_rad     = column_azimuths_rad.data_ptr<float>();
    params.row_azimuth_offsets_rad = has_row_offsets ? row_azimuth_offsets_rad.data_ptr<float>() : nullptr;
...
void check_lidar_projection(const c10::intrusive_ptr<RowOffsetStructuredSpinningLidarProjection> &projection)
{
    ...
    const int64_t n_rows    = projection->row_elevations_rad.dim() == 1 ? projection->row_elevations_rad.size(0) : -1;
    const int64_t n_columns = projection->column_azimuths_rad.dim() == 1 ? projection->column_azimuths_rad.size(0) : -1;
...
)
{
    ...
    TORCH_CHECK(projection->row_elevations_rad.device() == device, "row_elevations_rad device must match inputs");
    TORCH_CHECK(projection->column_azimuths_rad.device() == device, "column_azimuths_rad device must match inputs");
    if(projection->has_row_offsets)
    {
        TORCH_CHECK(projection->row_azimuth_offsets_rad.is_cuda(), "row_azimuth_offsets_rad must be a CUDA tensor");
        TORCH_CHECK(
            projection->row_azimuth_offsets_rad.device() == device, "row_azimuth_offsets_rad device must match inputs"
        );
    }
}
...
at::Tensor sensor_rays_to_sensor_angles_backward(const at::Tensor &sensor_rays, const at::Tensor &grad_sensor_angles)
{
    ...
    TORCH_CHECK(grad_sensor_angles.size(0) == sensor_rays.size(0), "grad_sensor_angles batch must match sensor_rays");
    TORCH_CHECK(
        grad_sensor_angles.scalar_type() == sensor_rays.scalar_type(), "grad_sensor_angles dtype must match sensor_rays"
    );
...
at::Tensor sensor_angles_to_sensor_rays(const at::Tensor &sensor_angles)
{
    ...
    auto guard       = c10::hip::HIPGuard(sensor_angles.device());
    auto sensor_rays = at::empty({sensor_angles.size(0), 3}, sensor_angles.options());
    AT_DISPATCH_FLOATING_TYPES(
        sensor_angles.scalar_type(),
...
at::Tensor sensor_angles_to_sensor_rays_backward(const at::Tensor &sensor_angles, const at::Tensor &grad_sensor_rays)
{
    ...
    TORCH_CHECK(grad_sensor_rays.size(0) == sensor_angles.size(0), "grad_sensor_rays batch must match sensor_angles");
    TORCH_CHECK(
        grad_sensor_rays.scalar_type() == sensor_angles.scalar_type(), "grad_sensor_rays dtype must match sensor_angles"
    );
...
)
{
    ...
    check_angle_table(row_elevations_rad, n_rows, row_elevations_rad, "row_elevations_rad", "row_elevations_rad");
    check_angle_table(column_azimuths_rad, n_columns, row_elevations_rad, "column_azimuths_rad", "row_elevations_rad");
    if(projection->has_row_offsets)
    {
        check_angle_table(
            row_azimuth_offsets_rad, n_rows, row_elevations_rad, "row_azimuth_offsets_rad", "row_elevations_rad"
        );
    }
    ...
    auto guard = c10::hip::HIPGuard(elements.device());
    ...
    AT_DISPATCH_FLOATING_TYPES(
        row_elevations_rad.scalar_type(),
        ...
        [&]
        {
            elements_to_sensor_angles_launch<scalar_t>(
                ...
                n_columns,
                has_row_offsets,
                elements.data_ptr<int32_t>(),
                row_elevations_rad.data_ptr<scalar_t>(),
                column_azimuths_rad.data_ptr<scalar_t>(),
                has_row_offsets ? row_azimuth_offsets_rad.data_ptr<scalar_t>() : nullptr,
                sensor_angles.data_ptr<scalar_t>(),
...
)
{
    ...
    check_int_matrix(elements, 2, "elements");
    check_float_matrix(grad_sensor_angles, 2, "grad_sensor_angles");
    TORCH_CHECK(grad_sensor_angles.size(0) == elements.size(0), "grad_sensor_angles batch must match elements");
    const int64_t n_rows    = row_elevations_rad.size(0);
    const int64_t n_columns = column_azimuths_rad.size(0);
    check_angle_table(row_elevations_rad, n_rows, row_elevations_rad, "row_elevations_rad", "row_elevations_rad");
    check_angle_table(column_azimuths_rad, n_columns, row_elevations_rad, "column_azimuths_rad", "row_elevations_rad");
    ...
    const int has_row_offsets    = projection->has_row_offsets ? 1 : 0;
    auto grad_row_elevations     = need_row_elevations_grad ? at::zeros({n_rows}, row_elevations_rad.options())
                                                            : at::empty({0}, row_elevations_rad.options());
    auto grad_column_azimuths    = need_column_azimuths_grad ? at::zeros({n_columns}, row_elevations_rad.options())
                                                             : at::empty({0}, row_elevations_rad.options());
    const bool emit_offsets_grad = has_row_offsets != 0 && need_row_offsets_grad;
    auto grad_row_offsets        = emit_offsets_grad ? at::zeros({n_rows}, row_elevations_rad.options())
                                                     : at::empty({0}, row_elevations_rad.options());
...
namespace
{
    ...
    {
        TORCH_CHECK(
            control_translations.dim() == 2 && control_translations.size(0) == 2 && control_translations.size(1) == 3,
            "control_translations must be (2, 3)"
        );
        TORCH_CHECK(
            control_rotations.dim() == 2 && control_rotations.size(0) == 2 && control_rotations.size(1) == 4,
        ...
        TORCH_CHECK(
            control_translations.is_cuda() && control_rotations.is_cuda(), "control poses must be CUDA tensors"
        );
...
)
{
    ...
    else
    {
        ...
        elements_ptr = elements.data_ptr<int32_t>();
    }

    auto guard             = c10::hip::HIPGuard(row_elevations_rad.device());
    auto world_rays        = at::empty({count, 6}, row_elevations_rad.options());
    auto timestamps_us     = at::empty({count}, row_elevations_rad.options().dtype(at::kLong));
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]
        {
            generate_spinning_lidar_rays_launch<scalar_t>(
                ...
                column_azimuths_rad.data_ptr<scalar_t>(),
                has_row_offsets ? row_azimuth_offsets_rad.data_ptr<scalar_t>() : nullptr,
                control_translations.data_ptr<scalar_t>(),
                control_rotations.data_ptr<scalar_t>(),
...
)
{
    ...
    auto grad_row_offsets          = emit_offsets_grad ? at::zeros({n_rows}, row_elevations_rad.options())
                                                       : at::empty({0}, row_elevations_rad.options());
    auto grad_control_translations = need_control_translations_grad ? at::zeros({2, 3}, row_elevations_rad.options())
                                                                    : at::empty({0}, row_elevations_rad.options());
    auto grad_control_rotations    = need_control_rotations_grad ? at::zeros({2, 4}, row_elevations_rad.options())
                                                                 : at::empty({0}, row_elevations_rad.options());
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]
        {
            generate_spinning_lidar_rays_backward_launch<scalar_t>(
                ...
                control_translations.data_ptr<scalar_t>(),
                control_rotations.data_ptr<scalar_t>(),
                grad_world_rays.data_ptr<scalar_t>(),
                need_row_elevations_grad ? grad_row_elevations.data_ptr<scalar_t>() : nullptr,
                need_column_azimuths_grad ? grad_column_azimuths.data_ptr<scalar_t>() : nullptr,
                emit_offsets_grad ? grad_row_offsets.data_ptr<scalar_t>() : nullptr,
    ...
    return {
        grad_row_elevations, grad_column_azimuths, grad_row_offsets, grad_control_translations, grad_control_rotations
    };
}
...
)
{
    ...
    const int64_t n_columns = column_azimuths_rad.size(0);
    TORCH_CHECK(n_rows > 0 && n_columns > 0, "angle tables must be non-empty");
    check_angle_table(row_elevations_rad, n_rows, world_points, "row_elevations_rad", "world_points");
    check_angle_table(column_azimuths_rad, n_columns, world_points, "column_azimuths_rad", "world_points");
    if(projection->has_row_offsets)
    {
        check_angle_table(row_azimuth_offsets_rad, n_rows, world_points, "row_azimuth_offsets_rad", "world_points");
    }
    check_control_poses(control_translations, control_rotations, world_points, "world_points");
    check_inverse_solver_scalars(
        max_iterations, stop_mean_relative_time_error, stop_delta_mean_relative_time_error, initial_relative_time
    );
    ...
    auto guard             = c10::hip::HIPGuard(world_points.device());
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]
        {
            inverse_project_spinning_lidar_launch<scalar_t>(
                ...
                start_timestamp_us,
                end_timestamp_us,
                world_points.data_ptr<scalar_t>(),
                row_elevations_rad.data_ptr<scalar_t>(),
                column_azimuths_rad.data_ptr<scalar_t>(),
                has_row_offsets ? row_azimuth_offsets_rad.data_ptr<scalar_t>() : nullptr,
                control_translations.data_ptr<scalar_t>(),
...
)
{
    ...
    TORCH_CHECK(scratch.scalar_type() == world_points.scalar_type(), "scratch dtype must match world_points");
    TORCH_CHECK(scratch.is_contiguous(), "scratch must be contiguous");

    auto guard = c10::hip::HIPGuard(world_points.device());
    auto grad_world_points
        = need_world_points_grad ? at::empty_like(world_points) : at::empty({0}, world_points.options());
    auto grad_control_translations = need_control_translations_grad ? at::zeros({2, 3}, world_points.options())
                                                                    : at::empty({0}, world_points.options());
    auto grad_control_rotations    = need_control_rotations_grad ? at::zeros({2, 4}, world_points.options())
                                                                 : at::empty({0}, world_points.options());
