...
// (Jacobian of y = v / |v| w.r.t. v). Kept separate from camera_kernel.hip.h so
// math.hip.h can be included in translation units that do not pull in the full
...
__device__ __forceinline__ float3 normalize3_bwd(float3 v, float3 d_y)
{
    ...
    return make_float3((d_y.x - dot * y.x) * inv_len, (d_y.y - dot * y.y) * inv_len, (d_y.z - dot * y.z) * inv_len);
}
...
{
    static_assert(BlockThreads % kWarpSize == 0, "BlockThreads must be a multiple of warp size");
