...
// kernel (lidar_kernel.hip / lidar_kernel_backward.hip), and is the Tier-1 bridge
// that crosses the Torch/CUDA boundary.  No torch/ATen includes here; only
// <cstdint> + a forward-declared hipStream_t.  All pointers carry __restrict__
...
// in lidar_kernel.hip / lidar_kernel_backward.hip.  The element->angle op runs
...
template<typename T>
void sensor_rays_to_sensor_angles_launch(int64_t count, const T *sensor_rays, T *sensor_angles, hipStream_t stream);

template<typename T>
void sensor_angles_to_sensor_rays_launch(int64_t count, const T *sensor_angles, T *sensor_rays, hipStream_t stream);
...
void sensor_rays_to_sensor_angles_backward_launch(
    int64_t count, const T *sensor_rays, const T *grad_sensor_angles, T *grad_sensor_rays, hipStream_t stream
);

template<typename T>
void sensor_angles_to_sensor_rays_backward_launch(
    int64_t count, const T *sensor_angles, const T *grad_sensor_rays, T *grad_sensor_angles, hipStream_t stream
);
