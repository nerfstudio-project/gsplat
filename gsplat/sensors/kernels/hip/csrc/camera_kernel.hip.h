...
// (quaternion.hip.h, pose.hip.h), OpenCV pinhole projection/distortion helpers, and
...
#include "external_distortion_kernel.hip.h"
#include "math.hip.h"
#include "pose.hip.h"
#include "quaternion.hip.h"
...
__device__ __forceinline__ CameraParamsValue load_camera_params(OpenCVPinholeProjection_KernelParameters projection)
{
    return projection;
}
...
// actual quaternion math to libs/geometry/kernels/hip/csrc/quaternion.hip.h.
...
__device__ __forceinline__ float2 iterative_undistort(float2 xy0, const OpenCVPinholeParams &p)
{
    float2 xy = xy0;
    for(int i = 0; i < 10; ++i)
    {
...
__device__ __forceinline__ ProjectionEval
    project_camera_ray_opencv(const OpenCVPinholeParams &p, float3 ray, float *scratch, int64_t scratch_offset)
{
    float x            = 0.0f;
    float y            = 0.0f;
    float inv_z        = 0.0f;
...
__device__ __forceinline__ float radial_factor(const CameraParamsValue &params, float r2, float r4, float r6)
{
    float num = 1.0f + params.radial_coeffs[0] * r2 + params.radial_coeffs[1] * r4 + params.radial_coeffs[2] * r6;
    float den = safe_nonzero(
        1.0f + params.radial_coeffs[3] * r2 + params.radial_coeffs[4] * r4 + params.radial_coeffs[5] * r6
    );
...
__device__ __forceinline__ ProjectionEval project_camera_point(const CameraParamsValue &params, float3 camera_point)
{
...
__device__ __forceinline__ float2 undistort_xy(const CameraParamsValue &params, float2 xy0)
{
    float2 xy                           = xy0;
...
__device__ __forceinline__ float3 camera_ray_from_image_point(const CameraParamsValue &params, float2 image_point)
{
...
__device__ __forceinline__ void compute_dF_dxy(
    float x, float y, float r2, float icD, float den, const OpenCVPinholeParams &params, float M[2][2]
)
{
    if(fabsf(den) < kDenominatorEpsilon)
    {
