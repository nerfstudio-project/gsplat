...
BivariateWindshieldDistortion::BivariateWindshieldDistortion(
    at::Tensor distortion_coeffs_, int64_t reference_polynomial_, int64_t h_poly_degree_, int64_t v_poly_degree_
)
...
{
    return {
        distortion_coeffs.data_ptr<float>(),
...
void check_bivariate_windshield_distortion(const c10::intrusive_ptr<BivariateWindshieldDistortion> &distortion)
{
    ...
    TORCH_CHECK(distortion->h_poly_degree >= 0 && distortion->h_poly_degree <= 2, "h_poly_degree must be in [0, 2]");
    TORCH_CHECK(distortion->v_poly_degree >= 0 && distortion->v_poly_degree <= 4, "v_poly_degree must be in [0, 4]");
}
} // namespace gsplat_sensors
