...
//   D1  cr -> ip,  bivariate_windshield         : 8  floats / row
//   D2  ip -> cr,  bivariate_windshield         : 12 floats / row
//   D3  mean-pose, no_external                  : 10 floats / row
//   D3  mean-pose, bivariate_windshield         : 10 floats / row
//   D5  static-pose img -> world,  no_external  : 8  floats / row
//   D5  static-pose img -> world,  bivariate    : 12 floats / row
//   D4  shutter-pose, no_external               : 11 floats / row
//   D4  shutter-pose, bivariate_windshield      : 11 floats / row
//   D6  shutter-pose img -> world, no_external  : 9  floats / row
//   D6  shutter-pose img -> world, bivariate    : 12 floats / row
...
// Packs FThetaProjectState into the camera-ray projection's 8-slot scratch layout.
...
{
    using Scratch = FThetaForwardScratch<DistortionOpFamily::CameraRaysToImagePoints, typename DistortionPolicy::Tag>;
    ...
    bool valid                = false;
    const float2 img          = ftheta_project_ray(projected_ray, params, state, valid);
...
{
    using Scratch = FThetaForwardScratch<DistortionOpFamily::ImagePointsToCameraRays, typename DistortionPolicy::Tag>;
    int64_t idx   = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(idx >= count)
    {
        return;
    ...
    const float3 projected_ray = ftheta_backproject_image_point(image_point, params, state);
    ...
    if(scratch != nullptr)
    {
        ftheta_save_bp_state_8(scratch, off, state);
...
{
    using Scratch
        = FThetaForwardScratch<DistortionOpFamily::ProjectWorldPointsMeanPose, typename DistortionPolicy::Tag>;
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    ...
    if(cam_pt.z > 0.0f)
    {
        ...
        img                        = ftheta_project_ray(projected_ray, params, state, valid);
    }
    else
    {
...
{
    using Scratch
        = FThetaForwardScratch<DistortionOpFamily::ImagePointsToWorldRaysStaticPose, typename DistortionPolicy::Tag>;
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(idx >= count)
    {
        return;
...
{
    using Scratch
        = FThetaForwardScratch<DistortionOpFamily::ImagePointsToWorldRaysShutterPose, typename DistortionPolicy::Tag>;
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(idx >= count)
    {
        return;
    ...
    {
        ftheta_save_bp_state_8(scratch, off, state);
        scratch[off + Scratch::kScratchStride - 1] = relative_time;
    }
}
} // namespace
