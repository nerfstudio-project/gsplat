...
__device__ __forceinline__ void ftheta_save_proj_state_8(
    ...
    scratch[off + 6] = ftheta_pack_flags(
        state.behind_camera, state.angle_clamped, state.min2d_clamped);
    scratch[off + 7] = 0.0f;
}
...
__device__ __forceinline__ void ftheta_backproject_bivariate_camera_ray(
    float2 image_point,
    FThetaParams const& params,
    BivariateWindshieldParams const& bivariate_params,
    float3& camera_ray,
    float3& unnorm_out,
    FThetaBackprojectState& bp_state) {
    float3 const ftheta_ray =
        ftheta_backproject_image_point(image_point, params, bp_state);
...
__global__ void camera_rays_to_image_points_ftheta_no_external_forward_kernel(
    ...
    bool valid = false;
    float2 const img = ftheta_project_ray(ray, params, state, valid);
...
__global__ void image_points_to_camera_rays_ftheta_no_external_forward_kernel(
    ...
    float3 const ray = ftheta_backproject_image_point(image_point, params, state);
...
__global__ void camera_rays_to_image_points_ftheta_bivariate_windshield_forward_kernel(
    ...
    float3 ray = read_vec3(camera_rays, idx);
    float3 const distorted_ray = apply_bivariate_distortion(ray, bivariate_params);
    FThetaProjectState state;
    bool valid = false;
    float2 const img = ftheta_project_ray(distorted_ray, params, state, valid);
    image_points[idx * 2 + 0] = img.x;
    image_points[idx * 2 + 1] = img.y;
    valid_flags[idx] = valid && ftheta_image_point_in_frame(img, params);
    if (scratch != nullptr) {
...
__global__ void image_points_to_camera_rays_ftheta_bivariate_windshield_forward_kernel(
    ...
    float3 camera_ray;
    float3 unnorm_out;
    ftheta_backproject_bivariate_camera_ray(
        image_point, params, bivariate_params, camera_ray, unnorm_out, bp_state);
...
__global__ void project_world_points_mean_pose_ftheta_no_external_forward_kernel(
    ...
    if (scratch != nullptr) {
        ...
        scratch[off + 9] = ftheta_pack_flags(
            state.behind_camera, state.angle_clamped, state.min2d_clamped);
    }
}
...
__global__ void project_world_points_mean_pose_ftheta_bivariate_windshield_forward_kernel(
    ...
    float2 img = make_float2(0.0f, 0.0f);
    if (cam_pt.z > 0.0f) {
        float3 const camera_ray = normalize3(cam_pt);
        float3 const distorted_ray =
            apply_bivariate_distortion(camera_ray, bivariate_params);
...
__global__ void image_points_to_world_rays_static_pose_ftheta_no_external_forward_kernel(
    ...
    float3 const camera_ray =
        ftheta_backproject_image_point(image_point, params, state);
...
__global__ void project_world_points_shutter_pose_ftheta_no_external_forward_kernel(
    ...
    for (int64_t i = 0; i < max_iterations; ++i) {
        ...
        float2 const img =
            ftheta_project_ray(cam_pt, params, iter_state, fwd_valid);
        state = iter_state;
        image_point_out = img;
        valid_out = fwd_valid;
        if (static_cast<gsplat_sensors::ShutterType>(shutter_type)
                == gsplat_sensors::ShutterType::GLOBAL
            || !fwd_valid) {
            break;
        }
        float next_relative_time = compute_relative_frame_time_opencv(
            img, projection.width, projection.height, shutter_type);
        float2 delta_px = make_float2(
            img.x - previous_image_point.x, img.y - previous_image_point.y);
        float pixel_error =
            sqrtf(delta_px.x * delta_px.x + delta_px.y * delta_px.y);
        if (i > 0 && pixel_error < stop_delta_mean_error_px) {
            break;
        }
        float time_delta = fabsf(next_relative_time - relative_time);
        float approximate_pixel_error = time_delta
            * static_cast<float>(std::max(projection.width, projection.height));
    ...
    valid_flags[idx] = valid_out
        && ftheta_image_point_in_frame(image_point_out, params);
    if (timestamps_us != nullptr) {
        timestamps_us[idx] = timestamp_from_relative_time(
            relative_time, start_timestamp_us, end_timestamp_us);
    }
    if (pose_translations != nullptr) {
        write_vec3(pose_translations, idx, pose_t);
    }
...
__global__ void project_world_points_shutter_pose_ftheta_bivariate_windshield_forward_kernel(
    ...
    for (int64_t i = 0; i < max_iterations; ++i) {
        ...
        FThetaProjectState iter_state;
        float2 const img =
            ftheta_project_ray(distorted_ray, params, iter_state, fwd_valid);
        state = iter_state;
        image_point_out = img;
        valid_out = fwd_valid;
        if (static_cast<gsplat_sensors::ShutterType>(shutter_type)
...
__global__ void image_points_to_world_rays_shutter_pose_ftheta_no_external_forward_kernel(
    ...
    float2 image_point = make_float2(
        image_points[idx * 2 + 0], image_points[idx * 2 + 1]);
    float relative_time = compute_relative_frame_time_opencv(
        image_point, projection.width, projection.height, shutter_type);
    FThetaBackprojectState bp_state;
    float3 const camera_ray =
        ftheta_backproject_image_point(image_point, params, bp_state);
    float3 pose_t = lerp3(
        read_vec3(start_translation, 0),
        read_vec3(end_translation, 0),
        relative_time);
    float4 rot0 = read_quat_xyzw_from_wxyz(start_rotation, 0);
    float4 rot1 = read_quat_xyzw_from_wxyz(end_rotation, 0);
    float qx, qy, qz, qw;
...
void camera_rays_to_image_points_ftheta_no_external_forward_launch(
    ...
    camera_rays_to_image_points_ftheta_no_external_forward_kernel
...
void image_points_to_camera_rays_ftheta_no_external_forward_launch(
    ...
    image_points_to_camera_rays_ftheta_no_external_forward_kernel
        <<<grid_for_count(count), kThreads, 0, stream>>>(
...
void project_world_points_mean_pose_ftheta_no_external_forward_launch(
    ...
    project_world_points_mean_pose_ftheta_no_external_forward_kernel
        <<<grid_for_count(count), kThreads, 0, stream>>>(
            count,
            projection,
            world_points,
...
void project_world_points_shutter_pose_ftheta_no_external_forward_launch(
    ...
    project_world_points_shutter_pose_ftheta_no_external_forward_kernel
        <<<grid_for_count(count), kThreads, 0, stream>>>(
            count,
            projection,
            world_points,
