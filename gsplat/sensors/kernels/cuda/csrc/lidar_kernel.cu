...
template<typename T>
__device__ __forceinline__ int find_nearest_row(T elevation, const T *__restrict__ row_elevations_rad, int64_t n_rows)
{
    T min_diff   = T(1e10);
    int best_row = 0;
...
)
{
    ...
    const T two_pi = T(2) * pi_const<T>();
    T rel_azimuth  = (spinning_dir == 1) ? azimuth - fov_horiz_start_rad : fov_horiz_start_rad - azimuth;
...
__global__ void sensor_rays_to_sensor_angles_kernel(
    int64_t count, const T *__restrict__ sensor_rays, T *__restrict__ sensor_angles
)
{
...
__global__ void sensor_angles_to_sensor_rays_kernel(
    int64_t count, const T *__restrict__ sensor_angles, T *__restrict__ sensor_rays
)
{
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(idx >= count)
...
)
{
    ...
    int64_t timestamp
        = valid
            ? start_timestamp_us
                  + static_cast<int64_t>(
                      static_cast<double>(relative_time) * static_cast<double>(end_timestamp_us - start_timestamp_us)
                  )
            : start_timestamp_us;
...
template<typename T>
__global__ void __launch_bounds__(kLidarThreads, 5) inverse_project_spinning_lidar_kernel(
    int64_t count,
    int64_t n_rows,
    int64_t n_columns,
    int has_row_offsets,
...
void sensor_rays_to_sensor_angles_launch(int64_t count, const T *sensor_rays, T *sensor_angles, cudaStream_t stream)
{
    ...
    sensor_rays_to_sensor_angles_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(count, sensor_rays, sensor_angles);
...
void sensor_angles_to_sensor_rays_launch(int64_t count, const T *sensor_angles, T *sensor_rays, cudaStream_t stream)
{
    ...
    sensor_angles_to_sensor_rays_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(count, sensor_angles, sensor_rays);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template void sensor_rays_to_sensor_angles_launch<float>(int64_t, const float *, float *, cudaStream_t);
template void sensor_rays_to_sensor_angles_launch<double>(int64_t, const double *, double *, cudaStream_t);
template void sensor_angles_to_sensor_rays_launch<float>(int64_t, const float *, float *, cudaStream_t);
template void sensor_angles_to_sensor_rays_launch<double>(int64_t, const double *, double *, cudaStream_t);
