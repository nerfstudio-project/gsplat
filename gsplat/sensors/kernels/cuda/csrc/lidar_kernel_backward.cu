...
template <typename T>
__device__ __forceinline__ constexpr T det_guard_eps();
template <>
__device__ __forceinline__ constexpr float det_guard_eps<float>() {
    return 1.0e-8f;
}
template <>
__device__ __forceinline__ constexpr double det_guard_eps<double>() {
    return 1.0e-12;
}
...
__device__ __forceinline__ void slerp_pair_bwd(
    ...
    T rx, T ry, T rz, T rw,
    T gx, T gy, T gz, T gw,
    T* gq1x, T* gq1y, T* gq1z, T* gq1w,
    T* gq2x, T* gq2y, T* gq2z, T* gq2w) {
    ...
    const T dw1_dtheta =
        ((T(1) - ti) * cos((T(1) - ti) * theta) * sin_theta - sin((T(1) - ti) * theta) * c) / den;
...
generate_spinning_lidar_rays_backward_kernel(
    int64_t count,
    int64_t n_rows,
    int64_t n_columns,
    int has_row_offsets,
    ...
    if (active) {
        ...
        if (valid) {
            ...
            quat_rotate_vector_bwd_impl<T>(
                qix, qiy, qiz, qiw, srx, sry, srz, g_dx, g_dy, g_dz,
                &gq_ix, &gq_iy, &gq_iz, &gq_iw, &g_srx, &g_sry, &g_srz);
            ...
            slerp_pair_bwd<T>(
                q0x, q0y, q0z, q0w, q1x, q1y, q1z, q1w, alpha,
                qix, qiy, qiz, qiw, gq_ix, gq_iy, gq_iz, gq_iw,
                &g_q0x, &g_q0y, &g_q0z, &g_q0w,
                &g_q1x, &g_q1y, &g_q1z, &g_q1w);
    ...
    const T pose_vals[14] = {
        g_t0x, g_t0y, g_t0z, g_t1x, g_t1y, g_t1z,
        g_q0x, g_q0y, g_q0z, g_q0w, g_q1x, g_q1y, g_q1z, g_q1w};
    T r[14];
...
template <typename T>
__global__ void __launch_bounds__(kLidarThreads, 5)
inverse_project_spinning_lidar_backward_kernel(
    int64_t count,
    ...
    if (active) {
        ...
        if (valid_flags[idx]) {
            ...
            quat_rotate_vector_bwd_impl<T>(
                -qix, -qiy, -qiz, qiw, vx, vy, vz, g_sx, g_sy, g_sz,
                &g_cqx, &g_cqy, &g_cqz, &g_cqw, &g_vx, &g_vy, &g_vz);
            ...
            slerp_pair_bwd<T>(
                q0x, q0y, q0z, q0w, q1x, q1y, q1z, q1w, alpha,
                qix, qiy, qiz, qiw, g_qix, g_qiy, g_qiz, g_qiw,
                &g_q0x, &g_q0y, &g_q0z, &g_q0w,
                &g_q1x, &g_q1y, &g_q1z, &g_q1w);
        }
...
void sensor_rays_to_sensor_angles_backward_launch(
    ...
    sensor_rays_to_sensor_angles_backward_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(
            count, sensor_rays, grad_sensor_angles, grad_sensor_rays);
...
void sensor_angles_to_sensor_rays_backward_launch(
    ...
    const T* grad_sensor_rays,
    T* grad_sensor_angles,
    cudaStream_t stream) {
    if (count <= 0) {
        return;
    }
    sensor_angles_to_sensor_rays_backward_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(
...
void elements_to_sensor_angles_backward_launch(
    ...
    elements_to_sensor_angles_backward_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(
            count,
            n_rows,
            n_columns,
...
template void elements_to_sensor_angles_backward_launch<float>(
    int64_t, int64_t, int64_t, int, const int32_t*, const float*, float*, float*,
    float*, cudaStream_t);
template void elements_to_sensor_angles_backward_launch<double>(
    int64_t, int64_t, int64_t, int, const int32_t*, const double*, double*, double*,
    double*, cudaStream_t);
...
void generate_spinning_lidar_rays_backward_launch(
    ...
    generate_spinning_lidar_rays_backward_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(
            count,
            n_rows,
            n_columns,
...
template void generate_spinning_lidar_rays_backward_launch<float>(
    int64_t, int64_t, int64_t, int, int, const int32_t*, const float*, const float*,
...
template void generate_spinning_lidar_rays_backward_launch<double>(
    int64_t, int64_t, int64_t, int, int, const int32_t*, const double*, const double*,
    const double*, const double*, const double*, const double*, double*, double*, double*,
    double*, double*, cudaStream_t);
...
void inverse_project_spinning_lidar_backward_launch(
    ...
    inverse_project_spinning_lidar_backward_kernel<T>
        <<<grid_for_count(count), kLidarThreads, 0, stream>>>(
            count,
            world_points,
            control_translations,
...
template void inverse_project_spinning_lidar_backward_launch<float>(
    int64_t, const float*, const float*, const float*, const bool*, const float*,
    const float*, float*, float*, float*, cudaStream_t);
template void inverse_project_spinning_lidar_backward_launch<double>(
    int64_t, const double*, const double*, const double*, const bool*, const double*,
    const double*, double*, double*, double*, cudaStream_t);
