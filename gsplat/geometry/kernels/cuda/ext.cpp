...
void quat_multiply_bwd_cuda(
    const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &grad_out, at::Tensor &grad_q1, at::Tensor &grad_q2
);
...
void quat_to_matrix_bwd_cuda(const at::Tensor &quat, const at::Tensor &grad_matrix_flat, at::Tensor &grad_quat);
...
void quat_angular_distance_bwd_cuda(
    const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &grad_dist, at::Tensor &grad_q1, at::Tensor &grad_q2
);
...
std::tuple<torch::Tensor, torch::Tensor> quat_multiply_bwd(
    const torch::Tensor &q1, const torch::Tensor &q2, const torch::Tensor &grad_out
)
{
    check_quat_multiply_inputs(q1, q2);
    CHECK_CUDA_TENSOR(grad_out);
...
std::tuple<torch::Tensor, torch::Tensor> quat_rotate_vector_bwd(
    const torch::Tensor &quat, const torch::Tensor &vec, const torch::Tensor &grad_out
)
{
    check_quat_rotate_vector_inputs(quat, vec);
    CHECK_CUDA_TENSOR(grad_out);
...
static void check_quat_slerp_batched_inputs(const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &t)
{
    check_quat_multiply_inputs(q1, q2);
    CHECK_CUDA_TENSOR(t);
    CHECK_CONTIGUOUS(t);
...
static void check_quat_from_axis_angle_inputs(const at::Tensor &axis, const at::Tensor &angle)
{
    ...
    TORCH_CHECK(
        angle.dim() == 2 && angle.size(0) == axis.size(0) && angle.size(1) == 1,
        "angle must be (N, 1) with same N as axis"
    );
}
...
std::tuple<torch::Tensor, torch::Tensor> quat_from_axis_angle_bwd(
    const torch::Tensor &axis, const torch::Tensor &angle, const torch::Tensor &grad_quat
)
{
    check_quat_from_axis_angle_inputs(axis, angle);
    ...
    TORCH_CHECK(
        grad_quat.dim() == 2 && grad_quat.size(0) == axis.size(0) && grad_quat.size(1) == 4,
        "grad_quat must be (N, 4) with same N as axis"
    );
...
std::tuple<torch::Tensor, torch::Tensor> quat_angular_distance_bwd(
    const torch::Tensor &q1, const torch::Tensor &q2, const torch::Tensor &grad_dist
)
{
    // Same (N, 4) constraints as quat_multiply.
    check_quat_multiply_inputs(q1, q2);
...
static void check_quat_manifold_interp_inputs(const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &t)
{
    check_quat_slerp_batched_inputs(q1, q2, t);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> quat_manifold_interp_bwd(
    const torch::Tensor &q1, const torch::Tensor &q2, const torch::Tensor &t, const torch::Tensor &grad_out
)
{
    check_quat_manifold_interp_inputs(q1, q2, t);
    CHECK_CUDA_TENSOR(grad_out);
...
static void check_se3_pose_vec3_inputs(const at::Tensor &translation, const at::Tensor &rotation, const at::Tensor &vec)
{
    ...
    TORCH_CHECK(translation.dim() == 2 && translation.size(1) == 3, "translation must be (N, 3)");
...
torch::Tensor se3pose_transform_direction(
    const torch::Tensor &translation, const torch::Tensor &rotation, const torch::Tensor &direction
)
{
...
static void check_se3_pose_pair_inputs(const at::Tensor &translation, const at::Tensor &rotation)
{
    ...
    CHECK_CONTIGUOUS(rotation);
    CHECK_FLOATING(translation);
    TORCH_CHECK(translation.device() == rotation.device(), "translation and rotation must be on the same CUDA device");
...
std::tuple<torch::Tensor, torch::Tensor> se3pose_to_matrix_bwd(
    const torch::Tensor &translation, const torch::Tensor &rotation, const torch::Tensor &grad_out
)
{
    check_se3_pose_pair_inputs(translation, rotation);
...
torch::Tensor se3pose_to_inverse_matrix(
    const torch::Tensor &translation, const torch::Tensor &rotation, bool wxyz_format
)
{
    check_se3_pose_pair_inputs(translation, rotation);
    const int64_t n = translation.size(0);
...
std::tuple<torch::Tensor, torch::Tensor> se3pose_to_inverse_matrix_bwd(
    const torch::Tensor &translation, const torch::Tensor &rotation, bool wxyz_format, const torch::Tensor &grad_out
)
{
    check_se3_pose_pair_inputs(translation, rotation);
    check_se3_pose_matrix_grad_out(translation, grad_out);
...
static void check_se3_pose_from_matrix_inputs(const at::Tensor &matrix)
{
    ...
    TORCH_CHECK(matrix.dim() == 2 && matrix.size(1) == 16, "matrix must be (N, 16) row-major flattened 4x4");
}

static void check_se3_pose_from_matrix_grad_outputs(
    const at::Tensor &matrix, const at::Tensor &grad_translation, const at::Tensor &grad_rotation
)
{
...
torch::Tensor se3pose_from_matrix_bwd(
    const torch::Tensor &matrix, const torch::Tensor &grad_translation, const torch::Tensor &grad_rotation
)
{
    check_se3_pose_from_matrix_inputs(matrix);
...
)
{
    ...
    TORCH_CHECK(
        rot0.dtype() == torch::kFloat32
            && trans1.dtype() == torch::kFloat32
            && rot1.dtype() == torch::kFloat32
    ...
    TORCH_CHECK(
        trans0.device() == rot0.device()
            && trans0.device() == trans1.device()
            && trans0.device() == rot1.device()
            && trans0.device() == point.device(),
...
)
{
    ...
    TORCH_CHECK(rot1.dim() == 2 && rot1.size(1) == 4 && rot1.size(0) == n, "rot1 must be (N, 4)");
    TORCH_CHECK(
        trans0.device() == rot0.device() && trans0.device() == trans1.device() && trans0.device() == rot1.device(),
        "trajectory pose tensors must share one CUDA device"
    );
    check_trajectory_2poses_time_query(trans0, time0, time1, query_time, n);
}
    ...
    )
{
    ...
    return std::make_tuple(
        grad_trans0, grad_rot0, grad_time0, grad_trans1, grad_rot1, grad_time1, grad_point, grad_query_time
    );
}
    ...
    )
{
    ...
    return std::make_tuple(grad_trans0, grad_rot0, grad_time0, grad_trans1, grad_rot1, grad_time1, grad_query_time);
}
...
)
{
    ...
    CHECK_CONTIGUOUS(point);
    TORCH_CHECK(
        trans.dtype() == torch::kFloat32 && rot.dtype() == torch::kFloat32 && point.dtype() == torch::kFloat32,
...
)
{
    ...
    if(n > 0)
    {
        trajectory_transform_point_1pose_cuda(trans, rot, time, point, query_time, result_point, result_oob);
    }
    return std::make_tuple(result_point, result_oob);
}
...
static void check_frame_transform_poses_tquat_inputs(const at::Tensor &input_poses)
{
    ...
    TORCH_CHECK(input_poses.dtype() == torch::kFloat32, "frame_transform_poses_tquat CUDA expects float32 input");
    TORCH_CHECK(input_poses.dim() == 2 && input_poses.size(1) == 7, "input_poses must be (N, 7) tquat rows");
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("quat_slerp_batched_bwd", &quat_slerp_batched_bwd, "Backward for quat_slerp_batched, CUDA");
    m.def(
    ...
    m.def("se3pose_to_matrix", &se3pose_to_matrix, "SE3 to 4x4 row-major (N,16): R(q) upper-left, t in column 3, CUDA");
    m.def(
