...
__global__ void quat_multiply_fwd_kernel(
    int64_t n, const scalar_t *__restrict__ q1, const scalar_t *__restrict__ q2, scalar_t *__restrict__ out
)
{
...
void launch_quat_multiply_fwd(const at::Tensor &q1, const at::Tensor &q2, at::Tensor &out)
{
    ...
    quat_multiply_fwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, q1.data_ptr<scalar_t>(), q2.data_ptr<scalar_t>(), out.data_ptr<scalar_t>());
...
__global__ void quat_rotate_vector_fwd_kernel(
    int64_t n, const scalar_t *__restrict__ quat, const scalar_t *__restrict__ vec, scalar_t *__restrict__ out
)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
...
void launch_quat_rotate_vector_fwd(const at::Tensor &quat, const at::Tensor &vec, at::Tensor &out)
{
    ...
    quat_rotate_vector_fwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, quat.data_ptr<scalar_t>(), vec.data_ptr<scalar_t>(), out.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template<typename scalar_t>
__global__ void quat_to_matrix_fwd_kernel(int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out_flat)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
    {
...
void launch_quat_to_matrix_fwd(const at::Tensor &quat, at::Tensor &out_flat)
{
    ...
    const int blocks          = static_cast<int>((n + threads - 1) / threads);
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(quat.device().index());
    quat_to_matrix_fwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, quat.data_ptr<scalar_t>(), out_flat.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template<typename scalar_t>
void launch_quat_to_matrix_bwd(const at::Tensor &quat, const at::Tensor &grad_flat, at::Tensor &grad_quat)
{
    const int64_t n = quat.size(0);
    if(n == 0)
    {
    ...
    quat_to_matrix_bwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, quat.data_ptr<scalar_t>(), grad_flat.data_ptr<scalar_t>(), grad_quat.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template<typename scalar_t>
__global__ void quat_normalize_safe_fwd_kernel(int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
    {
...
void launch_quat_normalize_safe_fwd(const at::Tensor &quat, at::Tensor &out)
{
    ...
    const int blocks          = static_cast<int>((n + threads - 1) / threads);
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(quat.device().index());
    quat_normalize_safe_fwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, quat.data_ptr<scalar_t>(), out.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
// Output: grad_quat (N,4).
template<typename scalar_t>
void launch_quat_normalize_safe_bwd(const at::Tensor &quat, const at::Tensor &grad_out, at::Tensor &grad_quat)
{
    const int64_t n = quat.size(0);
    if(n == 0)
    {
    ...
    quat_normalize_safe_bwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, quat.data_ptr<scalar_t>(), grad_out.data_ptr<scalar_t>(), grad_quat.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template<typename scalar_t>
__global__ void quat_conjugate_fwd_kernel(int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
    {
...
__global__ void quat_conjugate_bwd_kernel(
    int64_t n, const scalar_t *__restrict__ grad_out, scalar_t *__restrict__ grad_quat
)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
...
void launch_quat_conjugate_fwd(const at::Tensor &quat, at::Tensor &out)
{
    ...
    const int blocks          = static_cast<int>((n + threads - 1) / threads);
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(quat.device().index());
    quat_conjugate_fwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, quat.data_ptr<scalar_t>(), out.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void launch_quat_conjugate_bwd(const at::Tensor &grad_out, at::Tensor &grad_quat)
{
    ...
    quat_conjugate_bwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, grad_out.data_ptr<scalar_t>(), grad_quat.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
__global__ void quat_from_axis_angle_fwd_kernel(
    int64_t n, const scalar_t *__restrict__ axis, const scalar_t *__restrict__ angle, scalar_t *__restrict__ quat
)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
...
template<typename scalar_t>
void launch_quat_from_axis_angle_fwd(const at::Tensor &axis, const at::Tensor &angle, at::Tensor &quat)
{
    ...
    quat_from_axis_angle_fwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, axis.data_ptr<scalar_t>(), angle.data_ptr<scalar_t>(), quat.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
__global__ void quat_lerp_fwd_kernel(
    int64_t n, scalar_t t, const scalar_t *__restrict__ q1, const scalar_t *__restrict__ q2, scalar_t *__restrict__ out
)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
...
template<typename scalar_t>
void launch_quat_slerp_batched_fwd(const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &t, at::Tensor &out)
{
    const int64_t n = q1.size(0);
    if(n == 0)
    {
    ...
    quat_slerp_batched_fwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, q1.data_ptr<scalar_t>(), q2.data_ptr<scalar_t>(), t.data_ptr<scalar_t>(), out.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
__global__ void quat_angular_distance_fwd_kernel(
    int64_t n, const scalar_t *__restrict__ q1, const scalar_t *__restrict__ q2, scalar_t *__restrict__ dist_out
)
{
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if(i >= n)
...
template<typename scalar_t>
void launch_quat_angular_distance_fwd(const at::Tensor &q1, const at::Tensor &q2, at::Tensor &out)
{
    const int64_t n = q1.size(0);
    if(n == 0)
    {
    ...
    const int blocks          = static_cast<int>((n + threads - 1) / threads);
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(q1.device().index());
    quat_angular_distance_fwd_kernel<scalar_t>
        <<<blocks, threads, 0, stream>>>(n, q1.data_ptr<scalar_t>(), q2.data_ptr<scalar_t>(), out.data_ptr<scalar_t>());
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
template<typename scalar_t>
void launch_quat_manifold_interp_fwd(const at::Tensor &q1, const at::Tensor &q2, const at::Tensor &t, at::Tensor &out)
{
    const int64_t n = q1.size(0);
    if(n == 0)
    {
    ...
    quat_manifold_interp_fwd_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
        n, t.data_ptr<scalar_t>(), q1.data_ptr<scalar_t>(), q2.data_ptr<scalar_t>(), out.data_ptr<scalar_t>()
    );
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
