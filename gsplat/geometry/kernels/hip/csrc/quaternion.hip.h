...
// Device-side quaternion math for geometry CUDA. Paired with quaternion.hip:
// this header holds __device__ templates/helpers (scalar *_write / *_impl and
// per-row *_fwd_device / *_bwd_device delegating to them); the .hip file holds
...
__forceinline__ __device__ void quat_to_matrix_fwd_write(
    scalar_t qx, scalar_t qy, scalar_t qz, scalar_t qw, scalar_t *__restrict__ out9
)
{
...
static __device__ void so3_log_fwd_d(double qx, double qy, double qz, double qw, double *ox, double *oy, double *oz)
{
...
static __device__ void so3_exp_fwd_d(double wx, double wy, double wz, double *qx, double *qy, double *qz, double *qw)
{
    ...
    if(theta_sq < kManifoldSmallAngleSq)
    {
        ...
        const double wpart           = 1.0 - theta_sq_over_4 * (1.0 - theta_sq / 24.0 + theta_sq * theta_sq / 720.0);
        *qx                          = wx * c0;
        *qy                          = wy * c0;
        *qz                          = wz * c0;
        *qw                          = wpart;
...
)
{
    ...
    if(theta_sq < kManifoldSmallAngleSq)
    {
        ...
        const double dh_dr           = 0.25 * inner + theta_sq_over_4 * (-1.0 / 24.0 + r / 360.0);
...
template<typename scalar_t>
__device__ void quat_multiply_fwd_device(
    int64_t i, int64_t n, const scalar_t *__restrict__ q1, const scalar_t *__restrict__ q2, scalar_t *__restrict__ out
)
{
    const int64_t o   = i * 4;
...
template<typename scalar_t>
__device__ void quat_multiply_bwd_device(
    int64_t i,
    int64_t n,
...
template<typename scalar_t>
__device__ void quat_rotate_vector_fwd_device(
    int64_t i,
    int64_t n,
...
template<typename scalar_t>
__device__ void quat_rotate_vector_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ quat,
    const scalar_t *__restrict__ vec,
...
)
{
    ...
    quat_rotate_vector_bwd_impl(qx, qy, qz, qw, vx, vy, vz, gx, gy, gz, &gqx, &gqy, &gqz, &gqw, &gvx, &gvy, &gvz);
    grad_quat[oq + 0] = gqx;
    grad_quat[oq + 1] = gqy;
    grad_quat[oq + 2] = gqz;
    grad_quat[oq + 3] = gqw;
...
template<typename scalar_t>
__device__ void quat_to_matrix_fwd_device(
    int64_t i, int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out_flat
)
{
    const int64_t o = i * 4;
    quat_to_matrix_fwd_write(quat[o + 0], quat[o + 1], quat[o + 2], quat[o + 3], out_flat + i * 9);
}

// VJP: grad_flat row i = ∂L/∂R (9 row-major) → grad_quat row i (xyzw), chain through safe normalize.
template<typename scalar_t>
__device__ void quat_to_matrix_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ quat,
...
)
{
    ...
    const scalar_t gx       = g01 * scalar_t(2) * y
                            + g02 * scalar_t(2) * z
                            + g10 * scalar_t(2) * y
                            + g11 * scalar_t(-4) * x
                            + g12 * scalar_t(-2) * w
    ...
    const scalar_t gy       = g00 * scalar_t(-4) * y
                            + g01 * scalar_t(2) * x
                            + g02 * scalar_t(2) * w
                            + g10 * scalar_t(2) * x
                            + g12 * scalar_t(2) * z
                            ...
                            + g21 * scalar_t(2) * z
                            + g22 * scalar_t(-4) * y;
    const scalar_t gz       = g00 * scalar_t(-4) * z
                            + g01 * scalar_t(-2) * w
                            + g02 * scalar_t(2) * x
                            + g10 * scalar_t(2) * w
                            + g11 * scalar_t(-4) * z
                            ...
                            + g20 * scalar_t(2) * x
                            + g21 * scalar_t(2) * y;
    const scalar_t gw       = g01 * scalar_t(-2) * z
                            + g02 * scalar_t(2) * y
                            + g10 * scalar_t(2) * z
                            + g12 * scalar_t(-2) * x
                            + g20 * scalar_t(-2) * y
...
template<typename scalar_t>
__device__ void quat_normalize_safe_fwd_device(
    int64_t i, int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out
)
{
    const int64_t o = i * 4;
    quat_normalize_safe_fwd_write(
        quat[o + 0], quat[o + 1], quat[o + 2], quat[o + 3], out + o, out + o + 1, out + o + 2, out + o + 3
    );
}

// VJP through quat_normalize_safe_fwd_device: grad_out (xyzw) → grad_quat w.r.t. raw input quaternion.
template<typename scalar_t>
__device__ void quat_normalize_safe_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ quat,
    const scalar_t *__restrict__ grad_out,
...
template<typename scalar_t>
__device__ void quat_conjugate_fwd_device(
    int64_t i, int64_t n, const scalar_t *__restrict__ quat, scalar_t *__restrict__ out
)
{
    const int64_t o = i * 4;
...
template<typename scalar_t>
__device__ void quat_conjugate_bwd_device(
    int64_t i, int64_t n, const scalar_t *__restrict__ grad_out, scalar_t *__restrict__ grad_quat
)
{
    const int64_t o   = i * 4;
    const scalar_t gx = grad_out[o + 0], gy = grad_out[o + 1], gz = grad_out[o + 2], gw = grad_out[o + 3];
...
template<typename scalar_t>
__device__ void quat_from_axis_angle_fwd_device(
    int64_t i,
    int64_t n,
...
template<typename scalar_t>
__device__ void quat_from_axis_angle_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ axis,
    const scalar_t *__restrict__ angle,
...
template<typename scalar_t>
__device__ void quat_lerp_fwd_device(
    int64_t i,
    int64_t n,
    scalar_t t,
    const scalar_t *__restrict__ q1,
...
template<typename scalar_t>
__device__ void quat_lerp_bwd_device(
    int64_t i,
    int64_t n,
    scalar_t t,
    const scalar_t *__restrict__ q1,
...
template<typename scalar_t>
__device__ void quat_slerp_batched_fwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ q1,
    const scalar_t *__restrict__ q2,
...
template<typename scalar_t>
__device__ void quat_slerp_batched_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ q1,
    const scalar_t *__restrict__ q2,
...
template<typename scalar_t>
__device__ void quat_angular_distance_fwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ q1,
    const scalar_t *__restrict__ q2,
...
template<typename scalar_t>
__device__ void quat_angular_distance_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ q1,
    const scalar_t *__restrict__ q2,
...
template<typename scalar_t>
__device__ void quat_manifold_interp_fwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ t,
    const scalar_t *__restrict__ q1,
...
template<typename scalar_t>
__device__ void quat_manifold_interp_bwd_device(
    int64_t i,
    int64_t n,
    const scalar_t *__restrict__ t,
    const scalar_t *__restrict__ q1,
...
)
{
    ...
    quat_mul_bwd_d(
        x1, y1, z1, w1, ex, ey, ez, ew, gx, gy, gz, gw, &gq1x_m, &gq1y_m, &gq1z_m, &gq1w_m, &gex, &gey, &gez, &gew
    );
    ...
    quat_mul_bwd_d(
        c1x, c1y, c1z, c1w, x2, y2, z2, w2, gzx, gzy, gzz, gzw, &gcx, &gcy, &gcz, &gcw, &gq2x, &gq2y, &gq2z, &gq2w
    );
