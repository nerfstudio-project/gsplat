...
`gsplat/geometry` is organized around three package roles:
...
`gsplat/geometry` follows the repository-wide `gsplat/**` module model: it is internal to
...
The public geometry API is exposed through `gsplat.geometry.functional`.

The top-level package `gsplat.geometry` advertises only `functional` in
...
`gsplat.geometry.kernels.pose_ops` remain available for in-repo tests and
...
```python
from gsplat.geometry.functional import quat_multiply, se3pose_transform_point
```

The top-level package should make the functional surface easy to discover:

```python
import gsplat.geometry.functional as geometry
```
...
- public imports from `gsplat.geometry.functional` succeed,
