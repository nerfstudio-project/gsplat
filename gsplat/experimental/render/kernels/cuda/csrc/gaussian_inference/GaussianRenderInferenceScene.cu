...
namespace gaussian_render_inference_scene
{
    ...
    std::unique_ptr<InferenceRenderState> create_gaussian_render_inference_scene_state(
        const InferenceRenderScene &scene, int64_t sh_compression_mode
    )
    {
    ...
    )
    {
        ...
        if(out_rgbt.has_value())
        {
            ...
            TORCH_CHECK(
                buf.size(0) == 1 && buf.size(1) == height && buf.size(2) == width && buf.size(3) == 4,
                "out_rgbt must have shape [1, ",
                height,
                ", ",
                width,
                ", 4], got [",
        ...
        if(state.num_gaussians == 0)
        {
            ...
            {
                rgbt.narrow(3, 0, 3).copy_(background.value().reshape({1, 1, 1, 3}).to(at::kHalf));
            }
            return rgbt;
        }
        ...
        uint32_t tw
            = (static_cast<uint32_t>(width) + static_cast<uint32_t>(tile_size) - 1) / static_cast<uint32_t>(tile_size);
        uint32_t th
            = (static_cast<uint32_t>(height) + static_cast<uint32_t>(tile_size) - 1) / static_cast<uint32_t>(tile_size);

        // ---- Clamp SH degree ----
        TORCH_CHECK(
            sh_compression_mode == state.shCompressionMode,
            "render sh_compression_mode (",
            sh_compression_mode,
        ...
        if(background.has_value())
        {
            state.background.narrow(1, 0, 3).copy_(background.value().unsqueeze(0).to(at::kHalf));
        ...
        at::Tensor rgbt = out_rgbt.has_value() ? out_rgbt.value() : at::empty({1, height, width, 4}, opts_h);
    ...
    )
    {
        ...
        return render_gaussian_inference_scene(
            *state_,
            scene,
            viewmat,
            K,
    ...
    )
    {
        ...
        if(out_renders.has_value())
        {
            ...
            TORCH_CHECK(
                buf.dim() == 3 && buf.size(0) == height && buf.size(1) == width && buf.size(2) == 3,
                "out_renders must have shape [",
                height,
                ", ",
                width,
                ", 3], got [",
        ...
        if(out_alphas.has_value())
        {
            ...
            TORCH_CHECK(
                buf.dim() == 3 && buf.size(0) == height && buf.size(1) == width && buf.size(2) == 1,
                "out_alphas must have shape [",
                height,
                ", ",
                width,
                ", 1], got [",
