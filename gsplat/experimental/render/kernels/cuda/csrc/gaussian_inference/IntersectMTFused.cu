...
__global__ void __launch_bounds__(MT_CTA_THREADS, MT_MIN_BLOCKS) fused_mt_binning_kernel(
    const int32_t N,                                 // total gaussian count
...
) // number of macro-tile metadata tiles
{
...
static void launch_fused_mt_binning_impl(
    TileBinMode mode,
    int32_t N,
    int32_t tile_size,
    int32_t macro_tile_cols,
    int32_t macro_tile_rows,
    const float *means2d,
    ...
    int32_t *mt_chunk_meta_words,
    const int32_t *mt_chunk_tile_carry,
    int32_t n_mt_tiles
)
{
    if(N == 0)
    ...
    auto launch = [&](auto kernel_fn)
    {
        ...
        {
            AT_ERROR(
                "Failed to set shared memory size for fused macro-tile binning kernel (requested ",
                smem_bytes,
                " bytes, ",
                n_macro_tiles,
                " macro-tiles)"
    ...
    if(is_fill)
    {
        switch(tile_size)
        {
        case 16: launch(fused_mt_binning_kernel<TileBinMode::FILL, 16>); break;
        case 8:  launch(fused_mt_binning_kernel<TileBinMode::FILL, 8>); break;
        default: TORCH_CHECK(false, "tile_size must be 8 or 16, got ", tile_size); break;
        }
    }
    else
    {
        switch(tile_size)
        {
        case 16: launch(fused_mt_binning_kernel<TileBinMode::COUNT, 16>); break;
        case 8:  launch(fused_mt_binning_kernel<TileBinMode::COUNT, 8>); break;
        default: TORCH_CHECK(false, "tile_size must be 8 or 16, got ", tile_size); break;
        }
    }
}
...
)
{
    ...
    auto ceilDivInt = [](int32_t x, int32_t y) -> int32_t
    {
        return (x + y - 1) / y;
    };
    ...
    if(!m_mtGaussOffsets.defined() || m_mtGaussOffsets.size(0) != (int64_t)(m_nMacroTiles + 1))
    {
        ...
        m_mtIsectCapacity = 0;
        static_assert(
            (FUSED_GAUSS_BATCH_SIZE & (FUSED_GAUSS_BATCH_SIZE - 1)) == 0, "FUSED_GAUSS_BATCH_SIZE must be a power of 2"
        );
    ...
    launch_fused_mt_binning_impl(
        ...
        reinterpret_cast<const uint32_t *>(visible.data_ptr<int32_t>()),
        nullptr,
        reinterpret_cast<const __half *>(conics.data_ptr<at::Half>()),
        nullptr,
        nullptr,
        nullptr,
        m_mtChunkMetaWords.data_ptr<int32_t>(),
        nullptr,
        m_numMtTiles
    );

    // Stage 2: macro-tile chunk-base scan
    launch_mt_chunk_bases(
        m_mtChunkMetaWords,
        m_mtChunkTileSums,
        m_mtChunkTileCarry,
        m_mtGaussCounts,
        ...
        m_numChunks,
        &m_chunkBlockSums,
        &m_chunkBlockPrefixes,
        &m_chunkBlockScratchCapacity
    );
    ...
    if(m_nMacroIsects > 0)
    {
        ...
        launch_fused_mt_binning_impl(
            ...
            reinterpret_cast<const __half *>(conics.data_ptr<at::Half>()),
            m_mtGaussOffsets.data_ptr<int32_t>(),
            m_mtDepthKeys.data_ptr<int32_t>(),
            m_mtGaussIds.data_ptr<int32_t>(),
            m_mtChunkMetaWords.data_ptr<int32_t>(),
            m_mtChunkTileCarry.data_ptr<int32_t>(),
            m_numMtTiles
        );
        ...
        m_mtGaussIdsSorted = launch_mt_segmented_sort(
            m_nMacroIsects,
            m_nMacroTiles,
            m_mtGaussOffsets,
            m_mtDepthKeys,
...
void IntersectMTFused::rasterize(
    const at::Tensor &means2d,
    const at::Tensor &conics,
