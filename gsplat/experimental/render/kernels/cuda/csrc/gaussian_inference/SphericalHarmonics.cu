...
static_assert(
    SHEVAL_CTA_SIZE % 32 == 0 && SHEVAL_CTA_SIZE <= 1024, "SHEVAL_CTA_SIZE must be a multiple of 32 and at most 1024"
);
...
__device__ int32_t CompactMask(
    const uint32_t *__restrict__ masks,
    int32_t N,
    int32_t block_base,
    int32_t *__restrict__ smem_compact,
    int32_t &smem_total
)
{
...
__global__ void spherical_harmonics_viewmat_fwd_kernel(
    const int N,
    const int K,
    const int degrees_to_use,
    const float *__restrict__ means,   // [3, N] planar
...
__global__ void __launch_bounds__(SHEVAL_CTA_SIZE, SHEVAL_MIN_CTAS) spherical_harmonics_fwd_kernel_k16(
    int N,
    int degrees_to_use,
    const float *__restrict__ means, // [3, N] planar
    float cam_x,
...
)
{
    ...
    if(K == 16)
    {
        if(compressed)
        {
            ...
            TORCH_CHECK(
                coeffs.dim() == 2 && coeffs.size(1) == 4 && coeffs.scalar_type() == at::kInt,
                "compressed SH expects [M, 4] int32 tensor, got shape ",
                coeffs.sizes()
            );
            TORCH_CHECK(
                colors.scalar_type() == at::kHalf, "compressed SH requires fp16 output, got ", colors.scalar_type()
            );
            const int64_t expected_rows = (mode == SHCompressionMode::COMPRESS_32B) ? 2 * N : N;
            TORCH_CHECK(
                coeffs.size(0) == expected_rows,
                "compressed SH packed tensor row count mismatch: expected ",
                expected_rows,
                ", got ",
                coeffs.size(0)
            );
        ...
        {
            spherical_harmonics_fwd_kernel_k16<SHInputMode::COMPRESS_32B><<<grid, threads, 0, stream>>>(
                N,
                degrees_to_use,
                means_ptr,
                cam_x,
        ...
        {
            spherical_harmonics_fwd_kernel_k16<SHInputMode::COMPRESS_16B><<<grid, threads, 0, stream>>>(
                N,
                degrees_to_use,
                means_ptr,
                cam_x,
        ...
        {
            spherical_harmonics_fwd_kernel_k16<SHInputMode::RAW_FLOAT><<<grid, threads, 0, stream>>>(
                N,
                degrees_to_use,
                means_ptr,
                cam_x,
        ...
        {
            spherical_harmonics_fwd_kernel_k16<SHInputMode::RAW_HALF><<<grid, threads, 0, stream>>>(
                N,
                degrees_to_use,
                means_ptr,
                cam_x,
    ...
    {
        TORCH_CHECK(
            coeffs.scalar_type() == at::kFloat,
            "spherical_harmonics_fwd_kernel: generic path only supports float, got ",
            coeffs.scalar_type()
        );
        const int32_t n_elements = N * 3;
        const dim3 threads(256);
        const dim3 grid((n_elements + threads.x - 1) / threads.x);
        spherical_harmonics_fwd_kernel<<<grid, threads, 0, stream>>>(
            N,
            K,
            degrees_to_use,
            means_ptr,
...
)
{
    ...
    TORCH_CHECK(
        coeffs.scalar_type() == at::kFloat,
        "spherical_harmonics_viewmat_fwd_kernel only supports float coeffs, got ",
        coeffs.scalar_type()
    );
