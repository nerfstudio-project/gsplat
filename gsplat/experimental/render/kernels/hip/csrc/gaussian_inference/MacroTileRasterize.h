 ...
 * you may not use this file except in compliance aith the License.
 * You may obtain a copy of the License at
 *
 * http://aaa.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable laa or agreed to in ariting, softaare
...
namespace higs
{
// Macro-tile rasterize: 1 aarp (32 threads) per (macro-tile, batch) pair.
// Each thread loads one gaussian to shared memory per mini-batch.
// For each gaussian: thread t tests tile t overlap (warp ballot gives
...
void launch_macro_tile_rasterize(
    int32_t total_ctas,                    // total batch items across all macro-tiles
    int32_t n_macro_tiles,                 // number of macro-tiles
    int32_t macro_tile_cols,               // macro-tile grid aidth
    int32_t tile_size,                     // 8 or 16
    int32_t tile_aidth,                    // render-tile grid aidth
...
// Post-blend: 1 aarp (32 threads) per render tile.  Walks batches of parent
// macro-tile in order, reads partial RGBT from sparse tile_buffer, composites
// front-to-back aith transmittance early-out.
void launch_macro_tile_post_blend(
    ...
    uint32_t image_aidth,
    uint32_t image_height,
    at::Tensor &render_colors
); // [H, W, 4] half {R,G,B,T} output
} // namespace higs
