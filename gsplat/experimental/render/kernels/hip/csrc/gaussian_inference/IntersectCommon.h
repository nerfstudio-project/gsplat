...
// Used by both IntersectMT HIP standard and fused pipelines.
...
inline __device__ bool decode_gaussian_conic(
    const __half *__restrict__ conics, const float *__restrict__ means2d, int32_t idx, GaussianConic &gc
)
{
...
inline __device__ int32_t process_tiles_macro(
    float A,
    float B,
    float C,
    float disc,
...
{
    auto compute_ellipse_intersection
        = [](float A, float B, float C, float disc, float t, float2 p, bool isY, float coord) -> float2
    {
...
inline __device__ int32_t find_macro_tile(
    const int32_t *offsets, // [n+1] sorted offset array
...
) // value to look up
{
    ...
    while(lo < hi)
    {
        ...
        // Ballot32 (see WaveCompat.h): this function's sole caller gates it to
        // a single 32-lane macro-tile at a time, so a raw __ballot is already
        // correct there today (inactive lanes contribute 0 via the hardware
        // EXEC mask) -- Ballot32 is used anyway for uniformity/auditability
        // with every other ballot in this subsystem, and is a provable no-op
        // in that single-active-tile case.
        const uint32_t ballot = Ballot32(take_more);
...
// Hoisted into a __device__ function so each HIP translation unit can wrap it in a thin
...
__device__ void mt_binning_device_impl(
    const int32_t N,
    const float *__restrict__ means2d,
    const uint32_t *__restrict__ visible,
    const float *__restrict__ depths,
    const __half *__restrict__ conics,
    const int32_t macro_tile_cols,
    const int32_t macro_tile_rows,
    const int32_t *__restrict__ mt_gauss_offsets,
    int32_t *__restrict__ mt_depth_keys,
    int32_t *__restrict__ mt_gauss_ids,
    int32_t *__restrict__ mt_chunk_meta_words,
    const int32_t *__restrict__ mt_chunk_tile_carry,
    int32_t n_mt_tiles
)
{
...
#pragma unroll 1
    ...
    auto for_each_macro_tile_hit = [&](int32_t i, auto emit)
    {
        ...
        const int2 rect_min
            = {max(0, min(macro_tile_cols, (int)(gc.bbox_min.x * INV_MTS_X))),
               max(0, min(macro_tile_rows, (int)(gc.bbox_min.y * INV_MTS_Y)))};
        const int2 rect_max
            = {max(0, min(macro_tile_cols, (int)(gc.bbox_max.x * INV_MTS_X + 1.f))),
...
void launch_mt_chunk_bases(
    ...
    int64_t *inout_scratch_capacity  = nullptr
); // optional high-water-mark tracker
