...
def trainval(args):
    ...
    dataset = Dataset(data_dir=f"data/360_v2/{args.scene}/", factor=8, normalize=True)

    # preload
    images = np.stack(
        [imageio.imread(image_path)[..., :3] for image_path in dataset.image_paths]
    )
    images = torch.from_numpy(images).float().to(device) / 255.0
    c2ws = torch.from_numpy(dataset.camtoworlds).float().to(device)
    K = torch.from_numpy(dataset.K).float().to(device)
    points = torch.from_numpy(dataset.points).float()
    points_rgb = torch.from_numpy(dataset.points_rgb).float()
    ...
    for step in pbar:
        ...
        #     imageio.imwrite(f"results/image.png", canvas)
