...
def trainval(args):
    ...
    points_rgb = torch.randint(0, 255, (5000, 3),  requires_grad=True).float().to(device)
    ...
    for step in pbar:
        ...
        assert outputs["rgb"].requires_grad, "Output RGB does not require grad"
        assert outputs["depth"].requires_grad if outputs["depth"] is not None else True, "Output depth does not require grad"
        assert outputs["accumulation"].requires_grad, "Output accumulation does not require grad"
        ...
        print("optimizers.values() " , optimizers.values())
        for optimizer in optimizers.values():
