...
class Config:
    ...
    # Decode the training images once at the start of training and keep them in
    # host memory, instead of decoding one image per step (COLMAP data only).
    # Uses host memory for the decoded training set on every rank.
...
class Runner:
    ...
    def train(self):
        ...
        if cfg.cache_images:
            ...
            self.trainset.preload_images(num_workers=4)
        ...
        for step in pbar:
            ...
            pixels = (
                data["image"].to(device, non_blocking=True).float() / 255.0
            )  # [1, H, W, 3]
