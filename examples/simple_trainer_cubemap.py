...
class Runner:
    ...
    def __init__(
        ...
        progress_bar_ires = tqdm.tqdm(range(0, 12000), desc="Init Cubemapnet")
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            mask_gt_image = generate_circular_mask(fish_gt[0].shape, 450).cuda()

            #torchvision.utils.save_image(final_image*mask_gt_image, os.path.join(cfg.result_dir, f'final_fish_masked.png'))
    ...
    def eval(self, step: int, cubemap_net, rasterize_splats, stage: str = "val"):
        ...
        for i, data in enumerate(valloader):
            ...
            for img in img_list:
                ...
                intensity_final = torch.where(mask, intensity_img, intensity_final)  # Update intensity tracker
            fish_gt = (data['fisheye_image'].permute(0, 3, 1, 2)/255).cuda()
            mask_gt_image = generate_circular_mask(fish_gt[0].shape, 450).cuda().unsqueeze(0)
            colors = final_image.unsqueeze(0) * mask_gt_image
            pixels = mask_gt_image * fish_gt

            torch.cuda.synchronize()
            ellipse_time += time.time() - tic

            colors = torch.clamp(colors, 0.0, 1.0)
            canvas_list = [pixels.permute(0, 2, 3, 1), colors.permute(0, 2, 3, 1)]
            ...
            if world_rank == 0:
                ...
                pixels_p = pixels  # [1, 3, H, W]
                colors_p = colors # [1, 3, H, W]
