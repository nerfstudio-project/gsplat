...
def main():
    ...
    for i in range(len(dirs)):
      if i in clutters:
        os.rename(Path(root+dirs[i]), Path(root + '2clutter' + str(i).zfill(3) + '.' + file_type))
        if transforms is not None:
          transforms['frames'][i]['file_path'] = './images/2clutter' + str(i).zfill(3) + '.' + file_type
      elif i in extras:
        os.rename(Path(root+dirs[i]), Path(root + '1extra'+str(i).zfill(3) + '.' + file_type))    
        if transforms is not None:
          transforms['frames'][i]['file_path'] = './images/1extra' + str(i).zfill(3) + '.' + file_type
      elif i in cleans:
        os.rename(Path(root+dirs[i]), Path(root + '0clean'+str(i).zfill(3) + '.' + file_type))
        if transforms is not None:
          transforms['frames'][i]['file_path'] = './images/0clean' + str(i).zfill(3) + '.' + file_type
      else:
        os.rename(Path(root+dirs[i]), Path(root + '2clutter'+str(i).zfill(3) + '.' + file_type))    
        #if transforms is not None:
        #  transforms['frames'][i]['file_path'] = './images/2clutter' + str(i).zfill(3) + '.' + file_type
