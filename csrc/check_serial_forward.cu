...
// Host version of ref preprocessCUDA
__host__ void host_preprocessCUDA(
    ...
    if (cov3D_precomp != nullptr) {
        ...
        computeCov3D(
            scales[idx], scale_modifier, rotations[idx], cov3Ds + idx * 6
        );
        cov3D = cov3Ds + idx * 6;
    }
    ...
    if ((rect_max.x - rect_min.x) * (rect_max.y - rect_min.y) == 0)
        ...
        if (colors_precomp == nullptr) {
            ...
            // (glm::vec3*)orig_points, *cam_pos, shs, clamped); rgb[idx * C +
            // 0] = result.x; rgb[idx * C + 1] = result.y; rgb[idx * C + 2] =
            // result.z;
        }
    ...
    printf(
        "ref rast: point %d x %.2f y %.2f z %.2f, radius %d, # tiles %d, "
        "tile_min %d %d, tile_max %d %d \n",
        idx,
        point_image.x,
        point_image.y,
        depths[idx],
...
// Host version of diff_rast projection kernel
__host__ __device__ void host_project_gaussians_forward_kernel(
    ...
    // unsigned idx = cg::this_grid().thread_rank(); // idx of thread within
    // grid
    int idx = 0;
    radii[idx] = 0;
    num_tiles_hit[idx] = 0;
    ...
    xys[idx] = center;
    printf(
        "diff rast: point %d x %.2f y %.2f z %.2f, radius %d, # tiles %d, "
        "tile_min %d %d, tile_max %d %d \n",
        idx,
        center.x,
        center.y,
...
void compare_cov2d_forward() {
    ...
    float viewmatColumnMajor[] = {
        ...
        8.0f,
        1.0f};
    float cov3d[]{1.f, 0.f, 0.f, 1.f, 0.f, 1.f};
    float3 mean = {1.f, 1.f, 1.f};

    // diff rast
    float3 diff_rast_cov2d = project_cov3d_ewa(mean, cov3d, viewmat, fx, fy);
    std::cout << "diff rast cov2d: " << diff_rast_cov2d.x << " "
              << diff_rast_cov2d.y << " " << diff_rast_cov2d.z << std::endl;

    // ref rast
    float3 res = computeCov2D(
        mean, fx, fy, tan_fovx, tan_fovy, cov3d, viewmatColumnMajor
    );
    std::cout << "ref rast cov2d:  " << res.x << " " << res.y << " " << res.z
              << std::endl;
}
...
void compare_scale_rot_to_cov3d() {
    ...
    std::cout << "diff rast cov3d: " << cov3d[0] << " " << cov3d[1] << " "
              << cov3d[2] << std::endl;
    ...
    std::cout << "ref rast cov3d: " << ref_cov3d[0] << " " << ref_cov3d[1]
              << " " << ref_cov3d[2] << std::endl;
}
...
void compare_project_preprocess() {
    ...
    glm::vec4 quat_glm =
        glm::vec4({temp_quat.x, temp_quat.y, temp_quat.z, temp_quat.w});
    ...
    host_preprocessCUDA(
        ...
        rgb,
        conic_opacity,
        tile_bounds, // this is called "grid" in ref
        num_tiles_hit_ref
    );
}
int main() {
