...
__host__ __device__ float3 projectMean2DBackward(
    ...
    dL_dmean.x = (proj[0] * m_w - proj[3] * mul1) * dL_dmean2D.x +
                 (proj[1] * m_w - proj[3] * mul2) * dL_dmean2D.y;
    dL_dmean.y = (proj[4] * m_w - proj[7] * mul1) * dL_dmean2D.x +
                 (proj[5] * m_w - proj[7] * mul2) * dL_dmean2D.y;
    dL_dmean.z = (proj[8] * m_w - proj[11] * mul1) * dL_dmean2D.x +
                 (proj[9] * m_w - proj[11] * mul2) * dL_dmean2D.y;
    return dL_dmean;
}
...
computeConicBackward(const float3 &cov2D, const float3 &dL_dconic) {
    ...
    if (denom2inv != 0) {
        ...
        // matrix). e.g., dL / da = dL / d_conic_a * d_conic_a / d_a
...
__host__ __device__ void computeCov2DBackward(
    ...
    // cov2D = transpose(T) * transpose(Vrk) * T;
    dL_dcov[1] = 2 * T[0][0] * T[0][1] * dL_da +
                 (T[0][0] * T[1][1] + T[0][1] * T[1][0]) * dL_db +
                 2 * T[1][0] * T[1][1] * dL_dc;
    dL_dcov[2] = 2 * T[0][0] * T[0][2] * dL_da +
                 (T[0][0] * T[1][2] + T[0][2] * T[1][0]) * dL_db +
                 2 * T[1][0] * T[1][2] * dL_dc;
    dL_dcov[4] = 2 * T[0][2] * T[0][1] * dL_da +
...
__host__ __device__ void rasterizeBackward(
    ...
    for (int j = 0; j < N; j++) {
        ...
        for (int ch = 0; ch < C; ch++) {
            ...
            // atomicAdd(&(dL_dcolors[global_id * C + ch]), dchannel_dcolor *
            // dL_dchannel);
        }
        dL_dalpha *= T;
        // printf("ref %d dL_dalpha %.2e alpha %.2e T %.2e\n", j, dL_dalpha,
        // alpha, T); Update last alpha (to be used in the next iteration)
        last_alpha = alpha;
