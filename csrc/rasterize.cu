    ...
    ) 
{
    ...
    torch::Tensor out_img = torch::zeros({3, img_height, img_width}, means3d.options().dtype(torch::kFloat32));
    torch::Tensor out_radii = torch::zeros({num_points}, means3d.options().dtype(torch::kFloat32));
...
int rasterize_forward_impl(
    ...
    cudaMemcpy(means_d, means3d, num_points * sizeof(float3), cudaMemcpyHostToDevice);
    cudaMemcpy(rgbs_d, colors, num_points * sizeof(float3), cudaMemcpyHostToDevice);
    cudaMemcpy(opacities_d, opacity, num_points * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(quats_d, quats, num_points * sizeof(float4), cudaMemcpyHostToDevice);
    cudaMemcpy(viewmat_d, view_matrix, num_view * sizeof(float), cudaMemcpyHostToDevice);
    ...
    const dim3 tile_bounds = {(img_width + BLOCK_X - 1) / BLOCK_X, (img_height + BLOCK_Y - 1) / BLOCK_Y, 1};
    project_gaussians_forward_impl(
        num_points,
        means_d,
        scales_d,
    ...
    cudaMemcpy(out_radii, radii_d, num_points * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    const dim3 block = {BLOCK_X, BLOCK_Y, 1}; // TODO: make this a user custom setting.
