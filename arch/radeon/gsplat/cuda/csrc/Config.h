...
// __launch_bounds__ macro for the AMD-native (ROCm) Radeon target
// (gfx1100 / gfx1200, RDNA3+). Wave32 default → 256 threads/block = 8 waves.
// The MIN_WAVES_PER_EU=2 hint asks the compiler to keep VGPR allocation low
// enough that two 256-thread blocks (16 waves) can be co-resident per CU,
// which is the sweet spot for memory-bound projection / SH / intersect /
// adam kernels on RDNA3+ where occupancy hides GDDR latency.
//
// Defined here in Config.h so every .hip TU sees it via its existing
// `#include "./Config.h"`. AMD-native semantic: __launch_bounds__(MAX_THREADS,
// MIN_WAVES_PER_EU). This codebase is ROCm-only, so no NVIDIA fallback.
