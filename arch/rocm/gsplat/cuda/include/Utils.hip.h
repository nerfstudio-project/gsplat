...
// AMD-native ROCm output: no CUDA fallback. The libcu++ `<cuda/std/...>`
// headers are unavailable on ROCm; libstdc++'s `<limits>` and `<type_traits>`
// are __host__ __device__-friendly under amdclang++ and have no exception
// path. Map cuda::std::* aliases directly to ::std::* so existing call sites
// (cuda::std::numeric_limits<T>::epsilon(), cuda::std::is_floating_point_v<T>)
// keep working without per-call rewrites.
#include <limits>
#include <type_traits>
