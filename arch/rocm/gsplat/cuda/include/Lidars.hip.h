...
public:
    ...
    float shutter_relative_frame_time(const glm::fvec2 &image_point) const
    {
        ...
        // Access ANGLE_TO_PIXEL_SCALING_FACTOR through the qualified type
        // name, not the instance reference: amdclang++ is stricter than nvcc
        // about constexpr-ifying member access through a runtime object,
        // even when the member itself is `static constexpr`.
        constexpr float kToAngle =
            1.f / RowOffsetStructuredSpinningLidarModelParametersExtDevice::
