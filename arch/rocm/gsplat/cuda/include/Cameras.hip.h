...
template <class DerivedCameraModel> struct BaseCameraModel {
    ...
    shutter_relative_frame_time(glm::fvec2 const &image_point) const -> float {
        ...
        case ShutterType::ROLLING_TOP_TO_BOTTOM:
            t = ::floor(image_point.y) / (resolution[1] - 1);
            break;

        case ShutterType::ROLLING_LEFT_TO_RIGHT:
            t = ::floor(image_point.x) / (resolution[0] - 1);
            break;

        case ShutterType::ROLLING_BOTTOM_TO_TOP:
            t = (resolution[1] - ::ceil(image_point.y)) /
                (resolution[1] - 1);
            break;

        case ShutterType::ROLLING_RIGHT_TO_LEFT:
            t = (resolution[0] - ::ceil(image_point.x)) /
                (resolution[0] - 1);
            break;
        }
...
struct OpenCVPinholeCameraModel
    ...
    inline __device__ auto compute_distortion(glm::fvec2 const &uv
        ...
        auto const icD = (::fabs(icD_denominator) > 1e-8f)
    ...
    if (c == 0.0f) {
        ...
        if (delta >= 0.0f) {
            ...
            for (int i : {-1, 0, 1}) {
                ...
                if (s > 0.0f) {
                    // Use std::fmin instead of std::min: PyTorch's hipify
                    // rewrites std::min -> ::min, which then fails to compile
                    // when this header is included from .cpp host TUs
                    // (lesson cuda-translation-z3z).
...
struct OpenCVFisheyeCameraModel
    ...
    __host__ __device__ OpenCVFisheyeCameraModel(
        ...
        // std::fmax (not std::max): hipify rewrites std::max -> ::max
        // which fails to compile in .cpp host TUs (lesson cuda-translation-z3z).
...
public:
    ...
    __host__ __device__ FThetaCameraModel(
        ...
        // std::array::at() pulls in libstdc++'s __throw_out_of_range_fmt
        // helper which is __host__-only; amdclang++ rejects the reachability
        // chain in any __device__ function (lesson cuda_to_hip_porting
        // §4.8). Switch to operator[] — the indices are compile-time constants
        // here so the bounds check was redundant anyway.
