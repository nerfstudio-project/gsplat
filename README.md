...
- [Jul 2026] **Automatic feature-width planning** -- High-level rasterizers now choose the fewest compiled widths that exactly compose the total feature width; the largest compiled width limits one launch, not the input. `rasterization()` and `rasterization_2dgs()` no longer accept `channel_chunk`; callers should remove it. Default builds contain no widths above 32, while custom `NUM_CHANNELS` builds can add wider specializations. See the [developer documentation](docs/DEV.md#compiled-feature-widths) for low-level requirements and build-cost details.
