...
embedded as the first two columns of a rotation matrix :math:`R \in \R^{3x3}`, and a scale matrix :math:`S \in R^{3x3}`
...
by :math:`(WH)^{-1} \in R^{4x4}` and compute weights via ray-splat intersection. Then we follow the sort and rendering similar to 3DGS.  
...
and :math:`W \in \R{4x4}` is the transformation matrix from world space to image space.
...
to splat's tangent frame via the inverse transformation :math:`(WH)^{-1}`, where :math:`W \in R^{4x4}` is the transformation matrix
