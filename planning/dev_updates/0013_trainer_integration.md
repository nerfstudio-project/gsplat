...
- **Setup helpers** (CPU-friendly):
  - `build_splats_from_parser(parser, cfg, device)`: materialises the train-split frames, runs `multi_frame_depth_unprojection` over them (with the dataset's tissue-include mask as the keep gate), optional random subsample to `cfg.init_max_points`, then `knn_scale_init` with G-SHARP's `log(knn_dist * init_scale_multiplier)` rescaling. Quaternions random-normal + unit-normalised; opacities init in logit space so `sigmoid(...) == cfg.init_opacity`; colors taken from unprojected RGB. Returns a `ParameterDict` containing only the **per-Gaussian** trainables (`means`, `quats`, `scales`, `opacities`, `colors`) + matching optimizers. HexPlane / DeformNet trainables live in their own optimizers (see `build_deform_modules`); they are *not* added to the strategy `params` dict because gsplat's densification ops (`_duplicate` / `_split`) blindly per-Gaussian-index every entry and would crash on non-per-Gaussian tensors. `DynamicStrategy.check_sanity` therefore matches `DefaultStrategy.check_sanity` exactly — no extra keys are required.
