#!/usr/bin/env python3
"""
Create scatter plots for culling evaluation analysis with a different perspective:
- X-axis: Number of Gaussians after culling
...
This provides insights into quality vs computational efficiency trade-offs.
Modifications:
1. Remove pixel threshold size = 0.0
2. Reduce sphere sizes
3. Add uniform size option
4. Add area-based culling support
"""
...
def load_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, quality_metric):
    """
    Load all data for quality vs gaussians scatter plots.
...
def load_area_based_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, quality_metric):
    """
    Load all data for area-based quality vs gaussians scatter plots.
    ...
    for area_threshold in area_thresholds:
        ...
        if culling_method == "frustum_only":
            full_method_name = "frustum_only"
            area_threshold_for_point = 0.0  # No distance culling
        elif culling_method == "distance_only":
            full_method_name = f"distance_only_area_{area_threshold}"
            area_threshold_for_point = area_threshold
        elif culling_method == "both_culling":
        ...
        # Extract data for all poses with this area threshold
        for pose_id, metrics in data[full_method_name].items():
            distance = metrics["distance_to_bbox"]
            quality_value = metrics[quality_metric]
            num_gaussians = metrics["culled_count"]
            ...
            # Skip area threshold = 0.0 for distance_only and both_culling methods
            if area_threshold_for_point == 0.0 and culling_method in ["distance_only", "both_culling"]:
                continue
            ...
            all_data['quality_values'].append(quality_value)
            all_data['distances'].append(distance)
            all_data['area_thresholds'].append(area_threshold_for_point)
            all_data['culling_ratios'].append(culling_ratio)
    
    return all_data


def plot_quality_vs_gaussians_scatter(culling_evaluation_dir, save_dir, culling_method, quality_metric, quality_label, uniform_size=False):
    """
    ...
    scatter_data = load_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, quality_metric)
    if not scatter_data or len(scatter_data['num_gaussians']) == 0:
        print(f"No data found for {quality_metric} quality vs gaussians scatter plot in {culling_method}")
        return
    ...
    ax.set_xlabel('Number of Gaussians after Culling', fontsize=12)
    ax.set_ylabel(quality_label, fontsize=12)
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Quality vs Gaussians ({size_type}): {quality_label} - {culling_method.replace("_", " ").title()}', fontsize=14)
...
def plot_culling_efficiency_vs_gaussians(culling_evaluation_dir, save_dir, culling_method, uniform_size=False):
    """
    Create scatter plot focused on culling efficiency vs computational cost:
    - X-axis: Number of Gaussians after culling  
    ...
    scatter_data = load_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, "psnr")
    if not scatter_data or len(scatter_data['num_gaussians']) == 0:
        print(f"No data found for culling efficiency vs gaussians scatter plot in {culling_method}")
        return
    ...
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Culling Efficiency vs Computational Cost ({size_type}) - {culling_method.replace("_", " ").title()}', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Save plot with bbox_inches='tight' but include legends in the bounding box
    suffix = "_uniform_size" if uniform_size else ""
    plot_path = os.path.join(save_dir, f"culling_efficiency_vs_gaussians{suffix}.png")
    if legend is not None:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', bbox_extra_artists=[legend])
    else:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Culling efficiency vs Gaussians scatter plot saved to: {plot_path}")
    plt.close()


def plot_area_based_quality_vs_gaussians_scatter(culling_evaluation_dir, save_dir, culling_method, quality_metric, quality_label, uniform_size=False):
    """
    Create area-based scatter plot with:
    - X-axis: Number of Gaussians after culling
    - Y-axis: Quality metric value
    - Color: Distance to bounding box center
    - Size: Area threshold (variable) or uniform size
    """
    
    scatter_data = load_area_based_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, quality_metric)
    if not scatter_data or len(scatter_data['num_gaussians']) == 0:
        print(f"No area-based data found for {quality_metric} quality vs gaussians scatter plot in {culling_method}")
        return
    
    num_gaussians = np.array(scatter_data['num_gaussians'])
    quality_values = np.array(scatter_data['quality_values'])
    distances = np.array(scatter_data['distances'])
    area_thresholds = np.array(scatter_data['area_thresholds'])
    ...
    # Get unique area thresholds and assign different sizes
    unique_thresholds = sorted(np.unique(area_thresholds))
    
    # Create size mapping for area thresholds
    if uniform_size:
        # Use uniform size for all points
        normalized_sizes = np.full_like(area_thresholds, 50)
        size_range_min, size_range_max = 50, 50
    elif len(unique_thresholds) > 1:
        size_range_min, size_range_max = 20, 120  # Reduced from 30-300 to 20-120
        threshold_min, threshold_max = np.min(area_thresholds), np.max(area_thresholds)
        
        # Normalize area thresholds to size range
        if threshold_max > threshold_min:
            normalized_sizes = (area_thresholds - threshold_min) / (threshold_max - threshold_min) * (size_range_max - size_range_min) + size_range_min
        else:
            normalized_sizes = np.full_like(area_thresholds, (size_range_min + size_range_max) / 2)
    else:
        # If only one threshold, use medium size
        normalized_sizes = np.full_like(area_thresholds, 50)  # Reduced from 100 to 50
        size_range_min, size_range_max = 50, 50
    ...
    ax.set_ylabel(quality_label, fontsize=12)
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Area-Based Quality vs Gaussians ({size_type}): {quality_label} - {culling_method.replace("_", " ").title()}', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Create size legend for area thresholds
    legend = None
    if uniform_size:
        # For uniform size, omit the area threshold legend since all points are the same size
        pass
    elif len(unique_thresholds) > 1:
        # Create legend showing different area threshold sizes
        size_legend_elements = []
        ...
        for threshold in legend_thresholds:
            ...
            size_legend_elements.append(
                plt.scatter([], [], s=size, c='gray', alpha=0.7, edgecolors='black', linewidth=0.5,
                           label=f'Area Threshold: {threshold} px²')
            )
        
        legend = ax.legend(handles=size_legend_elements, title='Area Threshold (Size)', 
                          loc='upper left', bbox_to_anchor=(1.15, 0.95),
                          frameon=True, fancybox=True, shadow=True)
    else:
        # Single threshold case
        legend_elements = [plt.scatter([], [], s=normalized_sizes[0], c='gray', alpha=0.7, 
                                     edgecolors='black', linewidth=0.5,
                                     label=f'Area Threshold: {unique_thresholds[0]} px²')]
        legend = ax.legend(handles=legend_elements, title='Area Threshold (Size)', 
                          loc='upper left', bbox_to_anchor=(1.15, 0.95),
                          frameon=True, fancybox=True, shadow=True)
    ...
    # Save plot with bbox_inches='tight' but include legends in the bounding box
    suffix = "_uniform_size" if uniform_size else ""
    plot_path = os.path.join(save_dir, f"area_based_quality_vs_gaussians_{quality_metric}{suffix}.png")
    if legend is not None:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', bbox_extra_artists=[legend])
    else:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Area-based Quality vs Gaussians scatter plot for {quality_label} saved to: {plot_path}")
    plt.close()


def plot_area_based_culling_efficiency_vs_gaussians(culling_evaluation_dir, save_dir, culling_method, uniform_size=False):
    """
    Create area-based scatter plot focused on culling efficiency vs computational cost:
    - X-axis: Number of Gaussians after culling  
    - Y-axis: Culling ratio (percentage of gaussians removed)
    - Color: Distance to bounding box center
    - Size: Area threshold or uniform
    """
    
    scatter_data = load_area_based_quality_vs_gaussians_data(culling_evaluation_dir, culling_method, "psnr")
    if not scatter_data or len(scatter_data['num_gaussians']) == 0:
        print(f"No area-based data found for culling efficiency vs gaussians scatter plot in {culling_method}")
        return
    ...
    ax.set_ylabel('Culling Ratio (%)', fontsize=12)
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Area-Based Culling Efficiency vs Computational Cost ({size_type}) - {culling_method.replace("_", " ").title()}', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Create size legend for area thresholds (similar logic as above)
    legend = None
    if uniform_size:
        # For uniform size, omit the area threshold legend since all points are the same size
        pass
    ...
    # Save plot with bbox_inches='tight' but include legends in the bounding box
    suffix = "_uniform_size" if uniform_size else ""
    plot_path = os.path.join(save_dir, f"area_based_culling_efficiency_vs_gaussians{suffix}.png")
    if legend is not None:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', bbox_extra_artists=[legend])
    else:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Area-based Culling efficiency vs Gaussians scatter plot saved to: {plot_path}")
    plt.close()


def generate_all_quality_vs_gaussians_plots(culling_evaluation_dir, base_save_dir, culling_method):
    """Generate all quality vs gaussians plots for a specific culling method."""
    
    # Create method-specific subdirectory
    method_save_dir = os.path.join(base_save_dir, f"{culling_method}_quality_vs_gaussians")
    os.makedirs(method_save_dir, exist_ok=True)
    
    print(f"Generating quality vs gaussians plots for {culling_method.replace('_', ' ').title()} in {method_save_dir}")
    ...
    for metric_name, metric_label in quality_metrics:
        # Variable size plots
        plot_quality_vs_gaussians_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=False)
        # Uniform size plots
        plot_quality_vs_gaussians_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=True)
    
    # Generate culling efficiency vs gaussians plots (both versions)
    plot_culling_efficiency_vs_gaussians(culling_evaluation_dir, method_save_dir, culling_method, uniform_size=False)
    plot_culling_efficiency_vs_gaussians(culling_evaluation_dir, method_save_dir, culling_method, uniform_size=True)


def generate_all_area_based_quality_vs_gaussians_plots(culling_evaluation_dir, base_save_dir, culling_method):
    """Generate all area-based quality vs gaussians plots for a specific culling method."""
    
    # Create method-specific subdirectory for area-based plots
    method_save_dir = os.path.join(base_save_dir, f"{culling_method}_area_based_quality_vs_gaussians")
    os.makedirs(method_save_dir, exist_ok=True)
    
    print(f"Generating area-based quality vs gaussians plots for {culling_method.replace('_', ' ').title()} in {method_save_dir}")
    ...
    for metric_name, metric_label in quality_metrics:
        # Variable size plots
        plot_area_based_quality_vs_gaussians_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=False)
        # Uniform size plots
        plot_area_based_quality_vs_gaussians_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=True)
    
    # Generate area-based culling efficiency vs gaussians plots (both versions)
    plot_area_based_culling_efficiency_vs_gaussians(culling_evaluation_dir, method_save_dir, culling_method, uniform_size=False)
    plot_area_based_culling_efficiency_vs_gaussians(culling_evaluation_dir, method_save_dir, culling_method, uniform_size=True)
...
if __name__ == "__main__":
    ...
    for culling_method in culling_methods:
        ...
        generate_all_quality_vs_gaussians_plots(pixel_culling_evaluation_dir, base_save_dir, culling_method)
        
        print(f"Completed pixel-based quality vs gaussians plots for {culling_method}")
    ...
    for culling_method in culling_methods:
        ...
        generate_all_area_based_quality_vs_gaussians_plots(area_culling_evaluation_dir, base_save_dir, culling_method)
        
        print(f"Completed area-based quality vs gaussians plots for {culling_method}")
    
    print(f"\n{'='*80}")
    print("ALL QUALITY VS GAUSSIANS PLOTTING COMPLETED!")
