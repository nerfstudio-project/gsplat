...
- Size: Pixel threshold or Area threshold (with options for uniform size)
...
4. Add separate area-based plotting functions and data loading
"""
...
def load_area_based_gaussians_vs_quality_data(culling_evaluation_dir, culling_method, quality_metric):
    """
    Load all data for area-based gaussians vs quality scatter plots.
...
def plot_area_based_gaussians_vs_quality_scatter(culling_evaluation_dir, save_dir, culling_method, quality_metric, quality_label, uniform_size=False):
    """
    Create scatter plot for area-based culling with:
    - X-axis: Quality metric value
    - Y-axis: Number of Gaussians after culling
    - Color: Distance to bounding box center
    ...
    scatter_data = load_area_based_gaussians_vs_quality_data(culling_evaluation_dir, culling_method, quality_metric)
    if not scatter_data or len(scatter_data['quality_values']) == 0:
        print(f"No area-based data found for {quality_metric} gaussians vs quality scatter plot in {culling_method}")
        return
    ...
    ax.set_ylabel('Number of Gaussians after Culling', fontsize=12)
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Area-Based Gaussians vs Quality ({size_type}): {quality_label} - {culling_method.replace("_", " ").title()}', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Save plot with bbox_inches='tight' but include legends in the bounding box
    suffix = "_uniform_size" if uniform_size else ""
    plot_path = os.path.join(save_dir, f"area_based_gaussians_vs_quality_{quality_metric}{suffix}.png")
    if legend is not None:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', bbox_extra_artists=[legend])
    else:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Area-based Gaussians vs Quality scatter plot for {quality_label} saved to: {plot_path}")
    plt.close()
...
def plot_area_based_culling_ratio_vs_quality(culling_evaluation_dir, save_dir, culling_method, quality_metric, quality_label, uniform_size=False):
    """
    Create scatter plot focused on area-based culling efficiency vs quality:
    - X-axis: Quality metric value 
    - Y-axis: Culling ratio (percentage of gaussians removed)
    - Color: Distance to bounding box center
    ...
    scatter_data = load_area_based_gaussians_vs_quality_data(culling_evaluation_dir, culling_method, quality_metric)
    if not scatter_data or len(scatter_data['quality_values']) == 0:
        print(f"No area-based data found for culling ratio vs quality scatter plot in {culling_method}")
        return
    ...
    ax.set_ylabel('Culling Ratio (%)', fontsize=12)
    size_type = "Uniform Size" if uniform_size else "Variable Size"
    ax.set_title(f'Area-Based Culling Ratio vs Quality ({size_type}): {quality_label} - {culling_method.replace("_", " ").title()}', fontsize=14)
    ax.grid(True, alpha=0.3)
    ...
    # Save plot with bbox_inches='tight' but include legends in the bounding box
    suffix = "_uniform_size" if uniform_size else ""
    plot_path = os.path.join(save_dir, f"area_based_culling_ratio_vs_quality_{quality_metric}{suffix}.png")
    if legend is not None:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight', bbox_extra_artists=[legend])
    else:
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Area-based Culling ratio vs Quality scatter plot saved to: {plot_path}")
    plt.close()
...
def generate_all_area_based_gaussians_vs_quality_plots(culling_evaluation_dir, base_save_dir, culling_method):
    """Generate all area-based gaussians vs quality plots for a specific culling method."""
    
    # Create method-specific subdirectory for area-based plots
    method_save_dir = os.path.join(base_save_dir, f"{culling_method}_area_based_gaussians_vs_quality")
    os.makedirs(method_save_dir, exist_ok=True)
    
    print(f"Generating area-based gaussians vs quality plots for {culling_method.replace('_', ' ').title()} in {method_save_dir}")
    ...
    for metric_name, metric_label in quality_metrics:
        # Variable size plots
        plot_area_based_gaussians_vs_quality_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=False)
        # Uniform size plots
        plot_area_based_gaussians_vs_quality_scatter(culling_evaluation_dir, method_save_dir, culling_method, 
                                         metric_name, metric_label, uniform_size=True)
        
        # Area-based culling ratio vs quality plots for each metric
        plot_area_based_culling_ratio_vs_quality(culling_evaluation_dir, method_save_dir, culling_method,
                                     metric_name, metric_label, uniform_size=False)
        plot_area_based_culling_ratio_vs_quality(culling_evaluation_dir, method_save_dir, culling_method,
                                     metric_name, metric_label, uniform_size=True)
...
if __name__ == "__main__":
    ...
    for culling_method in culling_methods:
        ...
        generate_all_gaussians_vs_quality_plots(pixel_culling_evaluation_dir, base_save_dir, culling_method)
        
        print(f"Completed pixel-based gaussians vs quality plots for {culling_method}")
    ...
    for culling_method in culling_methods:
        ...
        generate_all_area_based_gaussians_vs_quality_plots(area_culling_evaluation_dir, base_save_dir, culling_method)
        
        print(f"Completed area-based gaussians vs quality plots for {culling_method}")
