#!/usr/bin/env python3
"""
Evaluate the quantitative quality (PSNR and SSIM) of area-based culling methods against ground truth.
This script compares area-based distance culling with pixel size-based culling using different area thresholds.
"""
...
from stream.culling import frustum_culling, distance_culling, distance_culling_area
...
def render_image(
...
) -> Tuple[torch.Tensor, torch.Tensor, int, int, float, np.ndarray]: # [1, H, W, 3], [1, H, W, 1], int, int, float, np.ndarray
    ...
    Args:
        ...
        pixel_area_threshold: float - pixel area threshold in pixels²
    Returns:
        ...
        pixel_areas: np.ndarray - pixel areas of each Gaussian [N] (only for area culling)
    """
    ...
    # Apply area-based distance culling if requested
    pixel_areas = None
    if apply_distance_culling:
        culling_mask, pixel_areas = distance_culling_area(
            render_means, render_quats, render_scales, render_opacities,
            viewmat, K_matrix, width, height, 
            pixel_area_threshold=cfg.pixel_threshold, return_pixel_areas=True
        )
        render_means = render_means[culling_mask]
        render_quats = render_quats[culling_mask]
        render_scales = render_scales[culling_mask]
    ...
    if render_means.shape[0] == 0:
        ...
        num_after_culling = 0
        culling_ratio = 100.0
        return background_image, background_alpha, num_original, num_after_culling, culling_ratio, pixel_areas
    ...
    return rendered_image, rendered_alpha, num_original, num_after_culling, culling_ratio, pixel_areas

def evaluate_culling_quality(frame_id, iter, cfg: Config, exp_name, sub_exp_name=None, save_images=True, measure_lpips=False):
    """Main evaluation function for area-based culling quality."""
    ...
    eval_configs = [
        ...
        {"name": f"distance_only_area_{cfg.pixel_threshold}", "frustum": False, "distance": True},
        {"name": f"both_culling_area_{cfg.pixel_threshold}", "frustum": True, "distance": True},
    ]
    ...
    if save_images:
        images_output_dir = os.path.join(poses_dir, f"render_hxw_{cfg.render_width}x{cfg.render_height}", "static_dist_area_culling", "evaluation_images")
    ...
    print(f"\nStarting evaluation on {len(poses)} poses, area threshold: {cfg.pixel_threshold} px² ...")
    ...
    for i, pose_data in enumerate(poses):
        ...
        # Render ground truth (no culling) - this will be the same for all configs
        ...
        for config in eval_configs:
            culled_image, _, _, num_after_culling, culling_ratio, pixel_areas = render_image(
                means, quats, scales, opacities, colors, pose_data, device, cfg,
                apply_frustum_culling=config["frustum"],
                apply_distance_culling=config["distance"],
            ) # [1, H, W, 3], [1, H, W, 1]
            
            # Save stacked comparison image (only if saving images and for distance culling)
            if save_images and config["name"] == f"distance_only_area_{cfg.pixel_threshold}":
    ...
    pixel_areas_dir = os.path.join(poses_dir, f"render_hxw_{cfg.render_width}x{cfg.render_height}", "static_dist_area_culling", "pixel_areas")
    os.makedirs(pixel_areas_dir, exist_ok=True)
    for config_name, pose_results in results.items():
        if config_name == f"distance_only_area_{cfg.pixel_threshold}":
            for pose_id, pose_data in pose_results.items():
                # Pixel areas are same across any pixel_threshold, so we overwrite the file
                pixel_areas = pose_data["pixel_areas"]
                if pixel_areas is not None:
                    np.save(os.path.join(pixel_areas_dir, f"{pose_id}.npy"), pixel_areas)
                    print(f"Saved pixel areas for pose {pose_id} to {os.path.join(pixel_areas_dir, f'{pose_id}.npy')}")

        # remove pixel_areas from pose_data to save space
        for pose_id in pose_results:
            del pose_results[pose_id]["pixel_areas"]
    ...
    output_dir = os.path.join(poses_dir, f"render_hxw_{cfg.render_width}x{cfg.render_height}", "static_dist_area_culling", "culling_evaluation")
    os.makedirs(output_dir, exist_ok=True)
    
    # Save detailed results (pose_id-keyed format for easy lookup)
    detailed_file = os.path.join(output_dir, f"detailed_results_area_{cfg.pixel_threshold}.json")
    with open(detailed_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save summary
    summary_file = os.path.join(output_dir, f"summary_area__{cfg.pixel_threshold}.json")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Pixel area threshold for distance culling (default: None)"
    )
    parser.add_argument(
        "--save-images", 
        action="store_true", 
    ...
    if args.area_threshold is not None:
        cfg.pixel_threshold = args.area_threshold
    ...
    print(f"Baseline pixel area threshold: {cfg.pixel_threshold}")
...
python scripts/culling/evaluate_area_based_culling.py --area-threshold 4.0      # Set pixel area threshold for culling
python scripts/culling/evaluate_area_based_culling.py --save-images             # Explicitly save images  
python scripts/culling/evaluate_area_based_culling.py --no-save-images          # Skip saving images
python scripts/culling/evaluate_area_based_culling.py --lpips                   # To measure LPIPS

This will:
1. Load the config and checkpoint
2. Read poses from {result_dir}/viewer_poses/viewer_poses.json
3. For each pose, render images with different culling configurations:
   - Ground truth (no culling)
   - Frustum culling only
   - Area-based distance culling only  
   - Both frustum + area-based distance culling
4. Calculate PSNR, SSIM, LPIPS, masked PSNR, masked SSIM, and cropped LPIPS metrics comparing culled images to ground truth
5. Use alpha channel from ground truth (no culling) render as mask for masked PSNR calculation
6. Optionally save ground truth masks to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_area_culling/evaluation_images/gt_masks/ (if --save-images)
7. Optionally save stacked comparison images (GT|Culled|Difference|Heatmap) to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_area_culling/evaluation_images/ organized by culling type (if --save-images)
8. Save detailed results (pose_id-keyed with psnr, ssim, lpips, masked_psnr, masked_ssim, cropped_lpips, culling_ratio, distance_to_bbox, num_valid_pixels) and summary statistics to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_area_culling/culling_evaluation/
"""
