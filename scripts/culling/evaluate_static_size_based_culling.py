...
def render_image(
    ...
    Returns:
        ...
        pixel_sizes: np.ndarray - pixel sizes of each Gaussian [N]
    """
    ...
    if apply_distance_culling:
        culling_mask, pixel_sizes = distance_culling(
    ...
    if render_means.shape[0] == 0:
        ...
        num_after_culling = 0
        culling_ratio = 100.0
        return background_image, background_alpha, num_original, num_after_culling, culling_ratio, pixel_sizes
    ...
    return rendered_image, rendered_alpha, num_original, num_after_culling, culling_ratio, pixel_sizes
...
def evaluate_culling_quality(frame_id, iter, cfg: Config, exp_name, sub_exp_name=None, save_images=True, measure_lpips=False):
    ...
    print(f"\nStarting evaluation on {len(poses)} poses, pixel threshold: {cfg.pixel_threshold} px ...")
    ...
    for config_name, pose_results in results.items():
        ...
        # remove pixel_sizes from pose_data to save space
...
python scripts/culling/evaluate_quality_of_culling.py --pixel-threshold 0.01   # Set pixel size threshold to 0.01, override the default value in the config file
python scripts/culling/evaluate_quality_of_culling.py --save-images            # Explicitly save images  
python scripts/culling/evaluate_quality_of_culling.py --no-save-images         # Skip saving images
python scripts/culling/evaluate_quality_of_culling.py --lpips                  # To measure LPIPS, you need to pass --lpips flag. This takes a lot of time.
...
6. Optionally save ground truth masks to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_culling/evaluation_images/gt_masks/ (if --save-images)
7. Optionally save stacked comparison images (GT|Culled|Difference|Heatmap) to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_culling/evaluation_images/ organized by culling type (if --save-images)
8. Save detailed results (pose_id-keyed with psnr, ssim, lpips, masked_psnr, masked_ssim, cropped_lpips, culling_ratio, distance_to_bbox, num_valid_pixels) and summary statistics to {result_dir}/viewer_poses/render_hxw_{cfg.render_width}x{cfg.render_height}/static_dist_culling/culling_evaluation/
"""
