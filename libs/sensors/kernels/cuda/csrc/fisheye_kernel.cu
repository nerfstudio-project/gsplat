...
__device__ __forceinline__ void fisheye_save_proj_state_8(
    ...
    scratch[off + 3] = fisheye_pack_flags(
        state.behind_camera, state.angle_clamped, state.oob, state.xy_norm_clamped);
    scratch[off + 4] = 0.0f;
    scratch[off + 5] = 0.0f;
    scratch[off + 6] = 0.0f;
    scratch[off + 7] = 0.0f;
...
__global__ void camera_rays_to_image_points_opencv_fisheye_no_external_forward_kernel(
    ...
    bool valid = false;
    float2 const img = fisheye_project_ray(ray, params, state, valid);
...
__global__ void image_points_to_camera_rays_opencv_fisheye_no_external_forward_kernel(
    ...
    float3 const ray =
        fisheye_backproject_image_point(image_point, params, state);
...
__global__ void project_world_points_mean_pose_opencv_fisheye_no_external_forward_kernel(
    ...
    FisheyeProjectState state;
    bool valid = false;
    float2 const img = fisheye_project_ray(cam_pt, params, state, valid);
    image_points[idx * 2 + 0] = img.x;
    image_points[idx * 2 + 1] = img.y;
    valid_flags[idx] = valid;
    ...
    if (scratch != nullptr) {
        ...
        scratch[off + 9] = fisheye_pack_flags(
            state.behind_camera, state.angle_clamped, state.oob, state.xy_norm_clamped);
        scratch[off + 10] = 0.0f;
        scratch[off + 11] = 0.0f;
        scratch[off + 12] = 0.0f;
...
__global__ void camera_rays_to_image_points_opencv_fisheye_bivariate_windshield_forward_kernel(
    ...
    FisheyeProjectState state;
    bool valid = false;
    float2 const img = fisheye_project_ray(distorted_ray, params, state, valid);
    image_points[idx * 2 + 0] = img.x;
    image_points[idx * 2 + 1] = img.y;
    valid_flags[idx] = valid;
    if (scratch != nullptr) {
...
__global__ void image_points_to_camera_rays_opencv_fisheye_bivariate_windshield_forward_kernel(
    ...
    float3 const distorted_ray =
        fisheye_backproject_image_point(image_point, params, state);
    float3 const unnorm_out =
        apply_bivariate_distortion(distorted_ray, bivariate_params);
    float3 const ray = normalize3(unnorm_out);
    write_vec3(camera_rays, idx, ray);
    if (scratch != nullptr) {
        int64_t off = idx * 12;
...
__global__ void project_world_points_mean_pose_opencv_fisheye_bivariate_windshield_forward_kernel(
    ...
    float3 p_rel = sub3(world_point, pose_t);
    float3 cam_pt = quat_inverse_rotate_xyzw_geom(pose_r_xyzw, p_rel);
    float3 const distorted = apply_bivariate_distortion(cam_pt, bivariate_params);
    FisheyeProjectState state;
    bool valid = false;
    float2 const img = fisheye_project_ray(distorted, params, state, valid);
    image_points[idx * 2 + 0] = img.x;
    image_points[idx * 2 + 1] = img.y;
    valid_flags[idx] = valid;
    if (timestamps_us != nullptr) {
...
__global__ void image_points_to_world_rays_static_pose_opencv_fisheye_bivariate_windshield_forward_kernel(
    ...
    float3 const unnorm_out =
        apply_bivariate_distortion(distorted_ray, bivariate_params);
    float3 const camera_ray = normalize3(unnorm_out);
    float3 pose_t = read_vec3(translations, 0);
    float4 pose_r_xyzw = read_quat_xyzw_from_wxyz(rotations, 0);
    float3 origin = pose_t;
    float3 direction = quat_rotate_xyzw_geom(pose_r_xyzw, camera_ray);
...
project_world_points_shutter_pose_opencv_fisheye_no_external_forward_kernel(
    ...
    for (int64_t iter = 0; iter < max_iterations; ++iter) {
        ...
        float2 const img = fisheye_project_ray(cam_pt, params, iter_state, fwd_valid);
        state = iter_state;
        cur_img = img;
        ...
        float const new_t = compute_relative_frame_time_opencv(
        ...
        float2 const dpx = make_float2(img.x - prev_img.x, img.y - prev_img.y);
        float const pixel_err = sqrtf(dpx.x * dpx.x + dpx.y * dpx.y);
        if (iter > 0 && pixel_err < stop_delta_mean_error_px) {
            break;
        }
        float const dt = fabsf(new_t - relative_time);
        float const approx_dpx = dt
...
__global__ void image_points_to_world_rays_shutter_pose_opencv_fisheye_no_external_forward_kernel(
    ...
    float2 image_point = make_float2(
        image_points[idx * 2 + 0], image_points[idx * 2 + 1]);
    float alpha = compute_relative_frame_time_opencv(
        image_point, projection.width, projection.height, shutter_type);

    float3 pose_t = lerp3(
        read_vec3(start_translation, 0), read_vec3(end_translation, 0), alpha);
    float4 rot0 = read_quat_xyzw_from_wxyz(start_rotation, 0);
    float4 rot1 = read_quat_xyzw_from_wxyz(end_rotation, 0);
    float qx, qy, qz, qw;
    trajectory_cuda::quat_slerp_pair_fwd_f(
    ...
    if (timestamps_us != nullptr) {
        timestamps_us[idx] = timestamp_from_relative_time(
            alpha, start_timestamp_us, end_timestamp_us);
    }
    if (pose_translations != nullptr) {
        write_vec3(pose_translations, idx, pose_t);
    }
...
__global__ void project_world_points_shutter_pose_opencv_fisheye_bivariate_windshield_forward_kernel(
    ...
    for (int64_t iter = 0; iter < max_iterations; ++iter) {
        ...
        FisheyeProjectState iter_state;
        bool fwd_valid = false;
        float2 const img = fisheye_project_ray(distorted, params, iter_state, fwd_valid);
        state = iter_state;
        cur_img = img;
