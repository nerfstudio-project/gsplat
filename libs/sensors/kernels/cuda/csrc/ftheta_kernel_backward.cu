...
__global__ void camera_rays_to_image_points_ftheta_bivariate_windshield_backward_kernel(
    ...
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    FThetaParams params = load_ftheta_params(projection);
    BivariateWindshieldParams biv_params =
    ...
    if (idx < count) {
        ...
        ftheta_project_ray_bwd(
            distorted_ray, params, state, d_img, d_distorted_ray, d_params);

        float3 d_ray = make_float3(0.0f, 0.0f, 0.0f);
        apply_bivariate_distortion_bwd(
            ray, biv_params, d_distorted_ray, d_ray, d_biv);
        if (grad_camera_rays != nullptr) {
            write_vec3(grad_camera_rays, idx, d_ray);
        }
    }
...
__global__ void image_points_to_camera_rays_ftheta_bivariate_windshield_backward_kernel(
    ...
    if (idx < count) {
        ...
        apply_bivariate_distortion_bwd(
            ftheta_ray, biv_params, d_unnorm_out, d_ftheta_ray, d_biv);
        float2 d_img = make_float2(0.0f, 0.0f);
        ftheta_backproject_image_point_bwd(
            img, params, state, d_ftheta_ray, d_img, d_params);
        if (grad_image_points != nullptr) {
            grad_image_points[idx * 2 + 0] = d_img.x;
            grad_image_points[idx * 2 + 1] = d_img.y;
        }
...
__global__ void project_world_points_mean_pose_ftheta_bivariate_windshield_backward_kernel(
    ...
    if (idx < count) {
        ...
        if (!state.behind_camera) {
            ...
            float3 d_distorted_ray = make_float3(0.0f, 0.0f, 0.0f);
            ftheta_project_ray_bwd(
                distorted_ray, params, replayed, d_img, d_distorted_ray, d_params);
            float3 d_camera_ray = make_float3(0.0f, 0.0f, 0.0f);
            apply_bivariate_distortion_bwd(
                camera_ray, biv_params, d_distorted_ray, d_camera_ray, d_biv);
...
__global__ void image_points_to_world_rays_static_pose_ftheta_bivariate_windshield_backward_kernel(
    ...
    if (idx < count) {
        ...
        apply_bivariate_distortion_bwd(
            ftheta_ray_norm, biv_params, d_unnorm_out, d_ftheta_ray_norm, d_biv);

        float2 d_img = make_float2(0.0f, 0.0f);
        ftheta_backproject_image_point_bwd(
            img, params, state, d_ftheta_ray_norm, d_img, d_params);
        if (grad_image_points != nullptr) {
            grad_image_points[idx * 2 + 0] = d_img.x;
            grad_image_points[idx * 2 + 1] = d_img.y;
        }
...
__global__ void image_points_to_world_rays_shutter_pose_ftheta_no_external_backward_kernel(
    ...
    if (idx < count) {
        ...
        float3 d_camera_ray = make_float3(0.0f, 0.0f, 0.0f);
        quat_rotate_bwd_xyzw_geom(
            pose_r_xyzw, camera_ray, d_direction, d_pose_r, d_camera_ray);
        ...
        trajectory_cuda::quat_slerp_pair_bwd_f(
            ...
            rot1.x, rot1.y, rot1.z, rot1.w,
            alpha, rx, ry, rz, rw,
            d_pose_r.x, d_pose_r.y, d_pose_r.z, d_pose_r.w,
            &gq0x, &gq0y, &gq0z, &gq0w,
            &gq1x, &gq1y, &gq1z, &gq1w,
            &ga_unused);
        float4 d_r0 = make_float4(gq0x, gq0y, gq0z, gq0w);
...
void camera_rays_to_image_points_ftheta_no_external_backward_launch(
    ...
    camera_rays_to_image_points_ftheta_no_external_backward_kernel
...
void image_points_to_camera_rays_ftheta_no_external_backward_launch(
    ...
    image_points_to_camera_rays_ftheta_no_external_backward_kernel
        <<<grid_for_count(count), kThreads, 0, stream>>>(
            count,
            projection,
            image_points,
...
void project_world_points_mean_pose_ftheta_no_external_backward_launch(
    ...
    project_world_points_mean_pose_ftheta_no_external_backward_kernel
        <<<grid_for_count(count), kThreads, 0, stream>>>(
            count,
            projection,
            world_points,
