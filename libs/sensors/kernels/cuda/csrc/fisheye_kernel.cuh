...
__device__ __forceinline__ OpenCVFisheyeParams load_opencv_fisheye_params(
...
__device__ __forceinline__ float fisheye_eval_forward_poly(
    float const theta,
    float const k[4]) {
...
__device__ __forceinline__ float fisheye_eval_dforward_poly(
    float const theta,
    float const k[4]) {
    float const t2 = theta * theta;
...
__device__ __forceinline__ float2 fisheye_project_ray(
    ...
    float const raw_xy_norm =
        sqrtf(camera_ray.x * camera_ray.x + camera_ray.y * camera_ray.y);
    ...
    float const raw_theta = atan2f(state.ray_xy_norm, camera_ray.z);
    ...
    float const scale = p.focal_length.x * state.delta / state.ray_xy_norm;
    float const scale_y = p.focal_length.y * state.delta / state.ray_xy_norm;
    float2 const img_point = make_float2(
        scale * camera_ray.x + p.principal_point.x,
        scale_y * camera_ray.y + p.principal_point.y);

    if (img_point.x < 0.0f || img_point.x >= static_cast<float>(p.width) ||
        img_point.y < 0.0f || img_point.y >= static_cast<float>(p.height)) {
        state.oob = true;
        valid_out = false;
        return make_float2(0.0f, 0.0f);
    }
...
__device__ __forceinline__ void fisheye_project_ray_bwd(
    float3 const camera_ray,
    OpenCVFisheyeParams const& p,
    FisheyeProjectState const& state,
    float2 const d_image_point,
    float3& d_camera_ray,                // accumulated
    OpenCVFisheyeParamGrads& d_params) { // accumulated
    ...
    float const inv_xy_norm = 1.0f / state.ray_xy_norm;
    float const q = state.delta * inv_xy_norm; // delta / ray_xy_norm

    d_params.focal[0] += d_image_point.x * q * camera_ray.x;
    d_params.focal[1] += d_image_point.y * q * camera_ray.y;

    float const dA = d_image_point.x * p.focal_length.x;
    float const dB = d_image_point.y * p.focal_length.y;

    float const d_q = dA * camera_ray.x + dB * camera_ray.y;
    d_camera_ray.x += dA * q;
    d_camera_ray.y += dB * q;

    float const d_delta = d_q * inv_xy_norm;
    ...
    float const t2 = state.theta * state.theta;
    float const t3 = state.theta * t2;
    float const t5 = t3 * t2;
    float const t7 = t5 * t2;
    float const t9 = t7 * t2;
    ...
    float const u = state.ray_xy_norm;
    float const v = camera_ray.z;
    float const denom = u * u + v * v;
    float const inv_denom = (denom > 0.0f) ? (1.0f / denom) : 0.0f;
    ...
    float const inv_raw = 1.0f / state.ray_xy_norm;
...
// operating band.
__device__ __forceinline__ float fisheye_solve_fw_newton(
    float const delta,
    OpenCVFisheyeParams const& p) {
    float x = p.approx_bw_factor * delta;
    for (int i = 0; i < p.newton_iterations; ++i) {
        float const df_x = fisheye_eval_dforward_poly(x, p.forward_poly);
        float const residual = fisheye_eval_forward_poly(x, p.forward_poly) - delta;
        x = x - residual / df_x;
    }
    return x;
}
...
__device__ __forceinline__ float3 fisheye_backproject_image_point(
    ...
    state.delta = sqrtf(
        state.normalized.x * state.normalized.x +
        state.normalized.y * state.normalized.y);
    ...
    float const scale = sin_theta / state.delta;
    state.ray_raw = make_float3(
        state.normalized.x * scale,
        state.normalized.y * scale,
        cos_theta);
...
__device__ __forceinline__ void fisheye_backproject_image_point_bwd(
    float2 const image_point,
    OpenCVFisheyeParams const& p,
    FisheyeBackprojectState const& state,
    float3 const d_camera_ray_out,
    ...
    float3 const d_ray_raw = normalize3_bwd(state.ray_raw, d_camera_ray_out);
    ...
    float const inv_delta = 1.0f / state.delta;
    float const scale = sin_theta * inv_delta;
    float2 d_normalized = make_float2(d_ray_raw.x * scale, d_ray_raw.y * scale);
    float const d_scale =
        d_ray_raw.x * state.normalized.x + d_ray_raw.y * state.normalized.y;

    float const d_sin_theta = d_scale * inv_delta;
    ...
    float const x_star = state.theta;
    float const df_val = fisheye_eval_dforward_poly(x_star, p.forward_poly);
    if (fabsf(df_val) > FISHEYE_IFT_DF_EPSILON) {
        float const inv_df = 1.0f / df_val;
        d_delta += d_theta * inv_df;
        float const x2 = x_star * x_star;
        float const x3 = x_star * x2;
        float const x5 = x3 * x2;
        float const x7 = x5 * x2;
        float const x9 = x7 * x2;
    ...
    float const inv_fx = 1.0f / p.focal_length.x;
    float const inv_fy = 1.0f / p.focal_length.y;
