...
class SpinningDirection(IntEnum):
    ...
    ``libs/sensors/kernels/cuda/csrc/lidar_params.h``. The C++ definition is the
...
def _verify_spinning_direction_matches_cpp() -> None:
    cpp_enum = getattr(_C, "SpinningDirection", None)
    if cpp_enum is None:
        raise RuntimeError(
    ...
    for member in SpinningDirection:
        ...
        if cpp_member is None:
            raise RuntimeError(
                ...
                "Python SpinningDirection in libs/sensors/kernels/lidars/types.py "
                "and C++ in libs/sensors/kernels/cuda/csrc/lidar_params.h are "
                "out of sync."
            )
        if int(cpp_member) != member.value:
            raise RuntimeError(
                ...
                "libs/sensors/kernels/cuda/csrc/lidar_params.h)."
            )
